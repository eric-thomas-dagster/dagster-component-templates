...
|---|---|---|
| `asset_key` | `str` | Asset key to attach the freshness policy to (e.g. 'warehouse/orders') |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `policy_type` | `Literal['time_window', 'cron']` | `"time_window"` | time_window: rolling window SLA. cron: deadline-based SLA. |
| `fail_window_hours` | `float` | — | [time_window] Hours after which the asset is considered stale and the check fails. Must exceed 1 minute. |
| `warn_window_hours` | `float` | — | [time_window] Hours after which a warning is raised. Must be shorter than fail_window_hours. |
| `deadline_cron` | `str` | — | [cron] Cron expression for the materialization deadline (e.g. '0 10 * * *'). |
| `lower_bound_delta_hours` | `float` | — | [cron] How many hours before the deadline the materialization window opens. |
| `timezone` | `str` | — | [cron] Timezone for the cron expression (e.g. 'America/New_York'). Defaults to UTC. |
