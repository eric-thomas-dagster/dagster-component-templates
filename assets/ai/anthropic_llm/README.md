...
|---|---|---|
| `asset_name` | `str` | Name of the asset that will hold the enriched data |
| `api_key` | `str` | Anthropic API key. Use ${ANTHROPIC_API_KEY} for environment variables. |
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with text to process |

### Execution

| Field | Type | Default | Description |
|---|---|---|---|
| `batch_size` | `int` | `10` | Number of rows to process in parallel for batch operations |
...
| `group_name` | `str` | `"anthropic_llm"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `model` | `str` | `"claude-3-5-sonnet-20241022"` | Claude model: claude-3-5-sonnet-20241022, claude-3-opus-20240229, claude-3-sonnet-20240229, claude-3-haiku-20240307 |
| `input_column` | `Union[str, int]` | — | Column name containing input text (for batch processing) |
| `output_column` | `Union[str, int]` | `"claude_response"` | Column name for Claude responses |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `system_prompt` | `str` | — | System prompt to set context and behavior for Claude |
| `user_prompt_template` | `str` | — | User prompt template with {column_name} placeholders for DataFrame columns |
| `max_tokens` | `int` | `1024` | Maximum tokens in response (required by Anthropic API) |
| `temperature` | `float` | `1.0` | Temperature for response randomness (0.0-1.0). Lower = more deterministic. |
| `top_p` | `float` | — | Nucleus sampling parameter (0.0-1.0). Alternative to temperature. |
| `top_k` | `int` | — | Top-K sampling parameter. Only consider top K tokens. |
| `tools` | `str` | — | JSON array of tool definitions for tool use |
| `tool_choice` | `str` | — | Control tool use: 'auto', 'any', 'tool_name', or {'type': 'tool', 'name': 'tool_name'} |
| `enable_prompt_caching` | `bool` | `false` | Enable prompt caching to reduce costs for repeated contexts (saves 90% on cached tokens) |
| `rate_limit_delay` | `float` | `0.2` | Delay in seconds between API calls to respect rate limits |
| `enable_caching` | `bool` | `true` | Cache responses to avoid redundant API calls |
| `cache_dir` | `str` | — | Directory for cache files. Default: /tmp/anthropic_llm_cache |
| `track_costs` | `bool` | `true` | Track token usage and estimated costs |
