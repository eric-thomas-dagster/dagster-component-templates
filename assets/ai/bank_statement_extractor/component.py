...
Extract account info and transactions from bank statements using an LLM via litellm.
"""
...
@dataclass
class BankStatementExtractorComponent(Component, Model, Resolvable):
    """Component for extracting structured data from bank statements using LLMs.

    Each row in the upstream DataFrame is treated as one bank statement. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (account_number, transactions, balances, etc.)
    ...
    input_column: str = Field(
        default="text",
        description="Column with bank statement content (text or file path)",
    )
    ...
    output_fields: List[str] = Field(
        ...
        description="Fields to extract from each bank statement",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "bank_statement_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: pd.DataFrame) -> pd.DataFrame:
            ...
            total = len(df)
            context.log.info(
                f"Extracting bank statement fields from {total} rows using {model}"
            )
