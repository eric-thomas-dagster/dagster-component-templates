"""Committed regression tests for BankStatementExtractorComponent's
input_type='file' content-extraction fix.

Before this fix, every file (PDF, image, anything) was opened in TEXT mode
with errors='replace', so a real bank statement PDF or scanned image got
decoded as raw-bytes-as-UTF-8 garbage and fed straight into the LLM prompt
-- silently not extracting anything real for the component's stated
primary use case. These tests exercise the fix against REAL files on disk
(a real PNG via Pillow, a real PDF via reportlab, a real .txt file) and
mock only the litellm.completion call -- no real API spend, no fake media.

BankStatementExtractorComponent-specific behavior (differs from
invoice_extractor):
- passes response_format={"type": "json_object"} to litellm.completion
- does NOT strip markdown code fences (relies on response_format instead)
- default input_column is "text"
- text prompt uses "Document:\n{content}" and "Return ONLY a JSON object..."
  (uppercase "ONLY")
"""
import base64
import sys
...
def fake_litellm(monkeypatch):
    ...
    behavior per row."""
    calls = []
    state = {"response_text": '{"account_number": "12345"}'}
...
def _materialize_one_row(component, file_path, input_column="text"):
...
def _make_component(mod, **overrides):
    kwargs = dict(
        ...
        input_type="file",
        input_column="text",
        output_fields=["account_number"],
    )
    kwargs.update(overrides)
    return mod.BankStatementExtractorComponent(**kwargs)
...
def test_image_file_reaches_prompt_as_real_vision_block(mod, fake_litellm, tmp_path):
    ...
    img_path = tmp_path / "statement.png"
...
def test_pdf_with_real_text_reaches_prompt(mod, fake_litellm, tmp_path):
    ...
    c.drawString(100, 700, "STATEMENT ACCT #55512345 -- Closing Balance: $1,204.11")
    c.save()
    ...
    assert "STATEMENT ACCT #55512345" in prompt_text
    assert "Closing Balance: $1,204.11" in prompt_text
    assert "Document:\n" in prompt_text
...
def test_scanned_pdf_with_no_text_fails_cleanly_not_silently(mod, fake_litellm, tmp_path):
    ...
    assert df.loc[0, "account_number"] is None
...
def test_non_dict_llm_response_rejected_not_crashed(mod, fake_litellm, tmp_path):
    ...
    txt_path.write_text("some real bank statement content")
...
def test_llm_max_retries_and_response_format_forwarded(mod, fake_litellm, tmp_path):
    calls, _ = fake_litellm
    txt_path = tmp_path / "doc.txt"
    txt_path.write_text("some real bank statement content")
