...
class CatalogAgentComponent(dg.Component, dg.Model, dg.Resolvable):
    ...
    codegen_output_dir: Optional[str] = Field(
        default=None,
        description=(
            "If set, after synthesis the component writes one ready-to-commit "
            "defs.yaml per successful pick to this directory. Point at your "
            "project's `src/<pkg>/defs/` folder. The generated YAMLs reproduce "
            "the agent's trajectory as permanent Dagster assets — drop the "
            "catalog_agent config afterwards and materialize directly (zero "
            "LLM cost from that point). Preview of the equivalent YAML is "
            "ALWAYS included in the synthesis asset's metadata regardless."
        ),
    )
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        ...
        def _plan_and_execute_step(context, iteration: int, prior_step_outputs: List[Any]):
            ...
            planner_prompt = (
                ...
                f"\n"
                f"INTERPRETING VAGUE TASKS — insert derivation steps when needed:\n"
                f"  • If the task mentions grouping/filtering by 'month', 'year', 'quarter', "
                f"    'week', 'day-of-week', or similar, and the data only has a date/timestamp "
                f"    column (not the derived one), insert a `formula` step FIRST to derive it "
                f"    (e.g. expressions: {{'month': 'order_date.dt.month'}}) before grouping.\n"
                f"  • If the task mentions grouping by a top-level of a nested field (e.g. "
                f"    'by domain' when the column is email), derive it via `formula` first "
                f"    (e.g. expressions: {{'domain': \"email.str.split('@').str[1]\"}}).\n"
                f"  • If the task mentions filtering by 'last N days' or a date range and the "
                f"    data has a raw timestamp, use `filter` with a pandas-query condition like "
                f"    'order_date >= \"2026-01-01\"'.\n"
                f"  • If the task mentions writing to a file (CSV/parquet/json), the final "
                f"    step should be a sink component (dataframe_to_csv/parquet/json) — don't "
                f"    end on a transform.\n"
                f"  • When in doubt about a derived column, PICK the formula step — insertion "
                f"    is cheap; missing it means the pipeline can't group/filter correctly."
            )
        ...
        def _synthesis_asset(context: dg.AssetExecutionContext, **kwargs):
            ...
            steps_desc = []
            successful_picks: List[Dict[str, Any]] = []  # for codegen
            n_ok = 0
            for _n in step_keys:
                _o = kwargs.get(_n)
            ...
            # ── Codegen: turn successful picks into permanent defs.yaml ──
            # The picks contain everything needed to reproduce the pipeline
            # as static Dagster assets. Emit one defs.yaml per pick so users
            # can commit them and drop the agent (zero LLM cost from then on).
            def _generate_defs_yaml(pick: Dict[str, Any]) -> str:
                _cfg = json.loads(pick["config"]) if isinstance(pick.get("config"), str) else (pick.get("config") or {})
                _lines = [
                    f"# generated by CatalogAgentComponent — {pick.get('asset_name')}",
                    f"# original reason: {pick.get('reason', '')[:120]}",
                    f"type: {pick['component_type']}",
                    "attributes:",
                ]

                def _emit(key, val, indent=2):
                    _pad = " " * indent
                    if isinstance(val, dict):
                        _lines.append(f"{_pad}{key}:")
                        ...
                        for v in val:
                            if isinstance(v, (dict, list)):
                                _lines.append(f"{_pad}  -")
                                for kk, vv in (v.items() if isinstance(v, dict) else enumerate(v)):
                                    _emit(str(kk), vv, indent + 4)
                            else:
                                _lines.append(f"{_pad}  - {json.dumps(v)}")
                    else:
                        # scalar — use JSON serialization for correct quoting
                        _lines.append(f"{_pad}{key}: {json.dumps(val)}")
            ...
            codegen_docs = []
            for _p in successful_picks:
                _asn = _p.get("asset_name") or f"step_{_p['iteration']}"
                _yaml = _generate_defs_yaml(_p)
                codegen_docs.append((_asn, _yaml))

            if _self.codegen_output_dir and codegen_docs:
                _out_root = _self.codegen_output_dir
                os.makedirs(_out_root, exist_ok=True)
                for _asn, _yaml in codegen_docs:
                    _dir = os.path.join(_out_root, _asn)
                    os.makedirs(_dir, exist_ok=True)
                    with open(os.path.join(_dir, "defs.yaml"), "w") as fh:
                        fh.write(_yaml)
                context.log.info(
                    f"[codegen] wrote {len(codegen_docs)} defs.yaml files to {_out_root}"
                )
            ...
            _md_parts = []
            for _asn, _yaml in codegen_docs:
                _md_parts.append(f"### `{_asn}/defs.yaml`\n\n```yaml\n{_yaml}```\n")
            _codegen_md = (
                "**Graduation path**: commit these to `src/<pkg>/defs/<asset_name>/defs.yaml` "
                "in your Dagster project. Then remove this catalog_agent config — the "
                "pipeline becomes permanent Dagster assets and you pay zero LLM cost.\n\n"
                + "\n".join(_md_parts)
            ) if codegen_docs else "_(no successful picks to codegen)_"

            context.add_output_metadata({
                "task": dg.MetadataValue.text(_task),
                "n_success": dg.MetadataValue.int(n_ok),
                "answer": dg.MetadataValue.md(answer),
                "codegen": dg.MetadataValue.md(_codegen_md),
                "codegen_written_to": dg.MetadataValue.text(_self.codegen_output_dir or "(not set — see codegen preview above)"),
            })
            return df
