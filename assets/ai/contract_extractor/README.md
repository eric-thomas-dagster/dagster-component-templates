...
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with contract content |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `input_column` | `Union[str, int]` | `"text"` | Column with contract content (text or file path) |
| `input_type` | `str` | `"text"` | Input type: 'text' (raw text) or 'file' (read file content) |
| `model` | `str` | `"gpt-4o-mini"` | LiteLLM model string |
| `output_fields` | `List[str]` | `['contract_type', 'parties', 'effective_date', 'expiration_date', 'governing_law', 'payment_terms', 'termination_clause', 'liability_cap', 'signatures']` | Fields to extract from each contract |
