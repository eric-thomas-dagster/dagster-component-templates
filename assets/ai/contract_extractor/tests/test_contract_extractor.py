"""Committed regression tests for ContractExtractorComponent's
input_type='file' content-extraction fix.

Before this fix, every file (PDF, image, anything) was opened in TEXT mode
with errors='replace', so a real contract PDF or scanned image got
...
def fake_litellm(monkeypatch):
    """Captures every completion() call's messages; returns a canned valid
    JSON response by default. Tests mutate `calls`/`next_response` to steer
    behavior per row."""
    calls = []
    state = {"response_text": '{"contract_type": "value_a"}'}
...
def test_pdf_with_real_text_reaches_prompt(mod, fake_litellm, tmp_path):
    ...
    c.drawString(100, 700, "MASTER SERVICES AGREEMENT -- Effective Date: 2024-01-15")
    c.save()
    ...
    component = mod.ContractExtractorComponent(
        ...
        input_type="file",
        input_column="local_path",
        output_fields=["contract_type", "effective_date"],
    )
    ...
    assert "MASTER SERVICES AGREEMENT" in prompt_text
    assert "Effective Date: 2024-01-15" in prompt_text
...
def test_scanned_pdf_with_no_text_fails_cleanly_not_silently(mod, fake_litellm, tmp_path):
    ...
    assert df.loc[0, "contract_type"] is None
...
def test_non_dict_llm_response_rejected_not_crashed(mod, fake_litellm, tmp_path):
    ...
    txt_path.write_text("some real contract content")
