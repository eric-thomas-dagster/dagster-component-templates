...
|---|---|---|---|
| `chunk_size` | `int` | `1000` | Target chunk size in characters (or tokens for token_aware strategy) |
...
| `group_name` | `str` | `"document_processing"` | Asset group for organization |
...
| `tokenizer_model` | `str` | — | Model for token counting (e.g., 'gpt-3.5-turbo', 'cl100k_base'). Required for token_aware strategy. |
| `min_chunk_size` | `int` | `100` | Minimum chunk size to avoid very small chunks |
| `merge_small_chunks` | `bool` | `true` | Merge chunks smaller than min_chunk_size with adjacent chunks |
| `preserve_sentences` | `bool` | `true` | Avoid splitting sentences when possible (for fixed and token_aware) |
