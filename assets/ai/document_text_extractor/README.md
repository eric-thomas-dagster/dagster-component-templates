...
| Field | Type | Default | Description |
|---|---|---|---|
| `output_format` | `str` | `"text"` | Output format: 'text', 'json' (with metadata), 'markdown' |
| `output_path` | `str` | — | Path to save extracted text (required if save_to_file is True) |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `extraction_method` | `str` | `"auto"` | Extraction method: 'auto', 'pypdf', 'pdfplumber', 'pytesseract' (OCR), 'docx', 'markdown' |
| `ocr_enabled` | `bool` | `false` | Enable OCR for scanned PDFs or images |
| `preserve_formatting` | `bool` | `false` | Attempt to preserve document formatting (paragraphs, whitespace) |
| `extract_metadata` | `bool` | `true` | Extract document metadata (author, creation date, etc.) |
| `page_range` | `str` | — | Page range to extract (e.g., '1-5', '1,3,5'). Only for PDFs |
| `save_to_file` | `bool` | `false` | Save extracted text to a file |
