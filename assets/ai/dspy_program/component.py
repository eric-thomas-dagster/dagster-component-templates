...
class DspyProgramComponent(Component, Model, Resolvable):
    ...
    Configures dspy.settings with the chosen LLM, instantiates the selected program type
    with the given signature, and runs .forward() on each row. Output fields from the
    signature are written as new columns.
    ...
    output_column: Union[str, int] = Field(default="dspy_output", description="Primary output column (first output field)")
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "dspy_program"  # component directory name
        _kind_map = {
...
group_name=group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            try:
                ...
                raise ImportError("pip install dspy-ai>=2.0.0")

            import os

            df = upstream.copy()
            context.log.info(f"Running DSPy {program_type} with signature='{signature}' on {len(df)} rows")

            # Configure LLM
            openai_kwargs: dict = {"model": model}
            if api_key_env_var:
                openai_kwargs["api_key"] = os.environ[api_key_env_var]

            lm = dspy.LM(**openai_kwargs)
            dspy.configure(lm=lm)

            # Parse signature to get output field names
            # Format: "input1, input2 -> output1, output2"
            if "->" in signature:
                _, output_part = signature.split("->", 1)
                output_fields = [f.strip() for f in output_part.split(",")]
            else:
                output_fields = [output_column]

            # Instantiate program type
            program_map = {
            ...
            program_cls = program_map.get(program_type, dspy.ChainOfThought)
            program = program_cls(signature)

            # Parse input field name from signature
            if "->" in signature:
                input_part, _ = signature.split("->", 1)
                input_fields = [f.strip() for f in input_part.split(",")]
                primary_input = input_fields[0] if input_fields else "input"
            else:
                primary_input = "input"

            results_by_field: dict = {f: [] for f in output_fields}

            for i, row in df.iterrows():
                text = str(row[text_column])
                prediction = program.forward(**{primary_input: text})

                for field in output_fields:
                    results_by_field[field].append(getattr(prediction, field, None))

                if (i + 1) % 20 == 0:
                    context.log.info(f"Processed {i + 1}/{len(df)} rows")

            for field in output_fields:
                df[field] = results_by_field[field]
