...
|---|---|---|---|
| `api_key` | `str` | — | API key for OpenAI/Cohere. Use ${API_KEY_NAME} for env vars. |

### Execution

| Field | Type | Default | Description |
|---|---|---|---|
| `batch_size` | `int` | `100` | Batch size for API calls (larger = faster but more memory) |
...
| `group_name` | `str` | `"embeddings"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `normalize_embeddings` | `bool` | `true` | Normalize embeddings to unit length (recommended for cosine similarity) |
| `dimension_reduction` | `str` | — | Reduce dimensions: 'pca', 'umap', or None |
| `compute_similarity` | `bool` | `false` | Compute pairwise cosine similarity matrix |
| `similarity_threshold` | `float` | — | If set, only store similarities above this threshold |
| `cache_embeddings` | `bool` | `false` | Cache embeddings to file for reuse |
| `cache_path` | `str` | — | Path to cache embeddings (parquet format) |
| `rate_limit_delay` | `float` | `0.0` | Delay between batches in seconds (for API rate limits) |
| `track_costs` | `bool` | `true` | Track API costs (OpenAI, Cohere) |
