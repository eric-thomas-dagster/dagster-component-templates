...
|---|---|---|
| `asset_name` | `str` | Name of the asset that will hold extracted entities |
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with text to extract entities from |
...
| `group_name` | `str` | `"entity_extraction"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `input_column` | `str` | `"text"` | Column name containing text to extract entities from |
| `output_column` | `str` | `"entities"` | Column name for the extracted entities (used when output_format='structured'). |
| `output_format` | `str` | `"structured"` | Output format: structured (dict per row) or flat (one column per entity type) |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `method` | `str` | `"spacy"` | Extraction method: llm (GPT/Claude), spacy, or transformer |
| `llm_provider` | `str` | — | LLM provider: openai or anthropic (for method=llm) |
| `llm_model` | `str` | — | LLM model name (for method=llm) |
| `spacy_model` | `str` | — | spaCy model: en_core_web_sm, en_core_web_md, en_core_web_lg (for method=spacy) |
| `transformer_model` | `str` | — | Transformer NER model (for method=transformer) |
| `entity_types` | `str` | `"person,organization,location,email,phone,product,order_id,date,money"` | Comma-separated list of entity types to extract |
| `custom_entities` | `str` | — | JSON string of custom entity patterns: {"type": ["pattern1", "pattern2"]} |
| `link_entities` | `bool` | `false` | Enable entity linking/resolution (LLM only) |
| `include_confidence` | `bool` | `true` | Include confidence scores for entities |
| `include_spans` | `bool` | `false` | Include character span positions (start, end) |
| `deduplicate` | `bool` | `true` | Remove duplicate entities within the same text |
