...
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with expense report content |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `input_column` | `Union[str, int]` | `"text"` | Column with expense report content (text or file path) |
| `input_type` | `str` | `"text"` | Input type: 'text' (raw text) or 'file' (read file content) |
| `model` | `str` | `"gpt-4o-mini"` | LiteLLM model string |
| `output_fields` | `List[str]` | `['employee_name', 'employee_id', 'department', 'report_date', 'period_start', 'period_end', 'line_items', 'total_amount', 'currency', 'approver', 'status']` | Fields to extract from each expense report |
