...
Extract structured data from employee expense reports using an LLM via litellm.
"""
...
@dataclass
class ExpenseReportExtractorComponent(Component, Model, Resolvable):
    """Component for extracting structured data from employee expense reports using LLMs.

    Each row in the upstream DataFrame is treated as one expense report. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (employee_name, line_items, total_amount, etc.)
    ...
    input_column: str = Field(
        default="text",
        description="Column with expense report content (text or file path)",
    )
    ...
    output_fields: List[str] = Field(
        ...
        description="Fields to extract from each expense report",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "expense_report_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: pd.DataFrame) -> pd.DataFrame:
            ...
            total = len(df)
            context.log.info(
                f"Extracting expense report fields from {total} rows using {model}"
            )
