"""Committed regression tests for ExpenseReportExtractorComponent's
input_type='file' content-extraction fix.

Before this fix, every file (PDF, image, anything) was opened in TEXT mode
with errors='replace', so a real expense report PDF or scanned image got
...
ExpenseReportExtractorComponent-specific behavior (differs from
invoice_extractor):
...
def fake_litellm(monkeypatch):
    ...
    behavior per row."""
    calls = []
    state = {"response_text": '{"employee_name": "Jane Doe"}'}
...
def _make_component(mod, **overrides):
    kwargs = dict(
        ...
        input_type="file",
        input_column="text",
        output_fields=["employee_name"],
    )
    kwargs.update(overrides)
    return mod.ExpenseReportExtractorComponent(**kwargs)
...
def test_pdf_with_real_text_reaches_prompt(mod, fake_litellm, tmp_path):
    ...
    c.drawString(100, 700, "EXPENSE REPORT #778 -- Total Reimbursement: $312.90")
    c.save()
    ...
    assert "EXPENSE REPORT #778" in prompt_text
    assert "Total Reimbursement: $312.90" in prompt_text
    assert "Document:\n" in prompt_text
...
def test_scanned_pdf_with_no_text_fails_cleanly_not_silently(mod, fake_litellm, tmp_path):
    ...
    assert df.loc[0, "employee_name"] is None
...
def test_non_dict_llm_response_rejected_not_crashed(mod, fake_litellm, tmp_path):
    ...
    txt_path.write_text("some real expense report content")
