...
|---|---|---|
| `asset_key` | `str` | Dagster asset key (e.g. 'hf/chat/summary'). |
| `model` | `str` | HF Hub model id available on the router (e.g. 'moonshotai/Kimi-K2-Instruct-0905', 'meta-llama/Llama-3.3-70B-Instruct', 'mistralai/Mistral-7B-Instruct-v0.3'). Aliased to `model:` in YAML. |

### Connection

| Field | Type | Default | Description |
|---|---|---|---|
| `base_url` | `str` | `HF_ROUTER_BASE_URL` | OpenAI-compatible base URL. Default is the HF router. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `prompt` | `str` | — | Simple single-user-message prompt. Mutually exclusive with `messages`. Use this for the common 'one question, one answer' shape. |
| `messages` | `List[Dict[str, str]]` | — | OpenAI-shaped chat messages: [{role, content}, ...]. Mutually exclusive with `prompt`. |
| `hf_token_env_var` | `str` | `"HF_TOKEN"` | Env var with HuggingFace token (default 'HF_TOKEN'). |
| `temperature` | `float` | — | Sampling temperature (0.0–2.0). |
| `max_tokens` | `int` | — | Max output tokens. |
| `top_p` | `float` | — | Nucleus sampling top_p. |
| `extra_kwargs` | `Dict[str, Any]` | — | Additional kwargs forwarded to chat.completions.create() (e.g. stop, response_format). |
