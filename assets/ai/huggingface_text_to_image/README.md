...
|---|---|---|
| `asset_key` | `str` | Dagster asset key (e.g. 'hf/images/marketing_hero'). |
| `model` | `str` | Text-to-image HF Hub model id (e.g. 'black-forest-labs/FLUX.1-dev', 'stabilityai/stable-diffusion-xl-base-1.0'). Aliased to `model:` in YAML. |
| `prompts` | `List[str]` | List of text prompts. One PNG per prompt. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `output_dir` | `str` | `"./generated_images"` | Directory to write generated PNGs (relative to working dir). |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `provider` | `str` | — | Inference provider to route through. Common options: 'wavespeed', 'falai', 'replicate', 'together'. Leave unset for HF default routing. |
| `filename_template` | `str` | `"{asset}_{index}_{slug}.png"` | Filename template. Placeholders: {asset} (sanitized asset_key), {index} (zero-padded), {slug} (slugged prompt). |
| `hf_token_env_var` | `str` | `"HF_TOKEN"` | Env var with HF token. |
| `generation_kwargs` | `Dict[str, Any]` | — | Extra kwargs for text_to_image() — e.g. {'guidance_scale': 7.5, 'num_inference_steps': 30, 'width': 1024, 'height': 1024}. |
