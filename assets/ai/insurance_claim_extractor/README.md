# Insurance Claim Extractor Component

Extract structured data from insurance claim documents using an LLM via litellm. Supports any OpenAI-compatible model and returns extracted data as additional DataFrame columns.

## Overview

The Insurance Claim Extractor Component uses a large language model to parse unstructured insurance claim text and extract key fields such as claim ID, policy number, incident details, damage type, claimed amount, and claim status. It works with any model accessible through litellm (OpenAI, Anthropic, Azure, etc.).
...
1. **Claims Processing Automation**: Extract claim data for adjudication systems
2. **Fraud Detection Pipelines**: Surface suspicious patterns across claims
3. **Adjuster Workflow Enrichment**: Pre-populate adjuster review screens
4. **Insurance Analytics**: Aggregate claims by type, amount, and status
...
### Basic Insurance Claim Extraction

```yaml
type: dagster_component_templates.InsuranceClaimExtractorComponent
attributes:
  asset_name: parsed_insurance_claims
  upstream_asset_key: raw_insurance_claims
  input_column: text
  model: gpt-4o-mini
...
| `input_column` | string | `text` | Column with claim content |
