...
Extract structured data from insurance claim documents using an LLM via litellm.
"""
...
class InsuranceClaimExtractorComponent(Component, Model, Resolvable):
    """Component for extracting structured data from insurance claim documents using LLMs.

    Each row in the upstream DataFrame is treated as one insurance claim. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (claim_id, policy_number, damage_type, etc.)
    ...
    upstream_asset_key: str = Field(
        description="Upstream asset key providing a DataFrame with insurance claim content"
    )
    input_column: Union[str, int] = Field(
        default="text",
        description="Column with insurance claim content (text or file path)",
    )
    ...
    output_fields: List[str] = Field(
        ...
        description="Fields to extract from each insurance claim",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "insurance_claim_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            df = upstream.copy()
            results = []
            _row_success: list[bool] = []
            total = len(df)
            context.log.info(
                f"Extracting insurance claim fields from {total} rows using {model}"
            )
                ...
                for _, row in batch.iterrows():
                    ...
                    try:
                        ...
                        if image_block is not None:
                            ...
                            message_content: Any = [{"type": "text", "text": prompt}, image_block]
                        else:
                            prompt = (
                                f"Extract the following fields from this document as JSON: {output_fields}\n\n"
                                f"Document:\n{content}\n\n"
                        ...
                        context.log.warning(f"Extraction failed for {file_ref!r}: {e}")
                        extracted = {f: None for f in output_fields}

                    results.append(extracted)
                    _row_success.append(success)

            extracted_df = pd.DataFrame(results)
            for col in extracted_df.columns:
                df[col] = extracted_df[col].values

            extraction_failures = sum(1 for ok in _row_success if not ok)
