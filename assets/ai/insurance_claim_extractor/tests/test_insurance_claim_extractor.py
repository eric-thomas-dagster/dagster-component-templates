"""Committed regression tests for InsuranceClaimExtractorComponent's
input_type='file' content-extraction fix.

Before this fix, every file (PDF, image, anything) was opened in TEXT mode
with errors='replace', so a real insurance claim PDF or scanned image got
...
def test_image_file_reaches_prompt_as_real_vision_block(mod, fake_litellm, tmp_path):
    ...
    img_path = tmp_path / "claim.png"
    Image.new("RGB", (20, 20), color=(255, 0, 0)).save(img_path)

    component = mod.InsuranceClaimExtractorComponent(
...
def test_pdf_with_real_text_reaches_prompt(mod, fake_litellm, tmp_path):
    ...
    c.drawString(100, 700, "CLAIM #98765 -- Damage Type: Water Damage")
    c.save()
    ...
    assert "CLAIM #98765" in prompt_text
    assert "Water Damage" in prompt_text
...
def test_non_dict_llm_response_rejected_not_crashed(mod, fake_litellm, tmp_path):
    ...
    txt_path.write_text("some real claim content")
