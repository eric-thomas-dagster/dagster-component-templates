...
Extract structured data from job postings and descriptions using an LLM via litellm.
"""
...
class JobPostingExtractorComponent(Component, Model, Resolvable):
    """Component for extracting structured data from job postings using LLMs.

    Each row in the upstream DataFrame is treated as one job posting. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (job_title, company, salary_range, skills, etc.)
    ...
    upstream_asset_key: str = Field(
        description="Upstream asset key providing a DataFrame with job posting content"
    )
    input_column: Union[str, int] = Field(
        default="text",
        description="Column with job posting content (text or file path)",
    )
    ...
    output_fields: List[str] = Field(
        ...
        description="Fields to extract from each job posting",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "job_posting_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            context.log.info(
                f"Extracting job posting fields from {total} rows using {model}"
            )
                ...
                for _, row in batch.iterrows():
                    ...
                    try:
                        ...
                        if image_block is not None:
                            prompt = (
                                f"Extract the following fields from this job posting image as JSON: {output_fields}\n\n"
