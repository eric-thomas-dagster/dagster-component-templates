...
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with legal document content |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `input_column` | `Union[str, int]` | `"text"` | Column with legal document content (text or file path) |
| `input_type` | `str` | `"text"` | Input type: 'text' (raw text) or 'file' (read file content) |
| `model` | `str` | `"gpt-4o-mini"` | LiteLLM model string |
| `output_fields` | `List[str]` | `['document_type', 'jurisdiction', 'court', 'case_number', 'parties', 'filing_date', 'key_dates', 'relief_sought', 'defined_terms', 'obligations', 'penalties']` | Fields to extract from each legal document |
