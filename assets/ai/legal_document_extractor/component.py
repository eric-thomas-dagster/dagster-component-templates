...
Extract key information from legal documents (NDAs, agreements, pleadings, briefs)
...
class LegalDocumentExtractorComponent(Component, Model, Resolvable):
    ...
    Each row in the upstream DataFrame is treated as one legal document. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (document_type, parties, obligations, penalties, etc.)
    ...
    - Litigation support pipelines
    - Compliance monitoring
    - Contract repository enrichment
    """

    model_config = ConfigDict(populate_by_name=True)
    asset_name: str = Field(description="Output Dagster asset name")
    upstream_asset_key: str = Field(
        description="Upstream asset key providing a DataFrame with legal document content"
    )
    input_column: Union[str, int] = Field(
        default="text",
        description="Column with legal document content (text or file path)",
    )
    ...
    output_fields: List[str] = Field(
        ...
        description="Fields to extract from each legal document",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "legal_document_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            total = len(df)
            context.log.info(
                f"Extracting legal document fields from {total} rows using {model}"
            )
            ...
            for i in range(0, total, batch_size):
                ...
                for _, row in batch.iterrows():
                    ...
                    try:
                        ...
                        # what the model chose to include.
