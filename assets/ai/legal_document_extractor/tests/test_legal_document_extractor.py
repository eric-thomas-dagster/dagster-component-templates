"""Committed regression tests for LegalDocumentExtractorComponent's
input_type='file' content-extraction fix.

Before this fix, every file (PDF, image, anything) was opened in TEXT mode
with errors='replace', so a real legal-document PDF or scanned image got
...
def fake_litellm(monkeypatch):
    ...
    behavior per row."""
    calls = []
    state = {"response_text": '{"document_type": "value_a"}'}
...
def test_image_file_reaches_prompt_as_real_vision_block(mod, fake_litellm, tmp_path):
    ...
    img_path = tmp_path / "brief_scan.png"
    Image.new("RGB", (20, 20), color=(255, 0, 0)).save(img_path)

    component = mod.LegalDocumentExtractorComponent(
...
def test_pdf_with_real_text_reaches_prompt(mod, fake_litellm, tmp_path):
    ...
    c.drawString(100, 700, "IN THE SUPERIOR COURT -- Case No. CV-2024-001122")
    c.save()
    ...
    component = mod.LegalDocumentExtractorComponent(
        ...
        input_type="file",
        input_column="local_path",
        output_fields=["document_type", "case_number"],
    )
    ...
    assert "IN THE SUPERIOR COURT" in prompt_text
    assert "Case No. CV-2024-001122" in prompt_text
...
def test_scanned_pdf_with_no_text_fails_cleanly_not_silently(mod, fake_litellm, tmp_path):
    ...
    assert df.loc[0, "document_type"] is None
...
def test_non_dict_llm_response_rejected_not_crashed(mod, fake_litellm, tmp_path):
    ...
    txt_path.write_text("some real legal document content")
