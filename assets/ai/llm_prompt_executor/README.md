...
| `provider` | `str` | LLM provider: 'openai', 'anthropic', 'cohere', or 'huggingface' |
| `model` | `str` | Model name (e.g., 'gpt-4', 'claude-3-5-sonnet-20241022', 'command-r-plus') |

### Connection

| Field | Type | Default | Description |
|---|---|---|---|
| `api_key` | `str` | — | API key (use ${VAR_NAME} for environment variable, e.g., ${OPENAI_API_KEY}) |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `system_prompt` | `str` | — | System prompt to set context for the LLM |
| `user_prompt_template` | `str` | — | User prompt template with {column_name} placeholders for DataFrame columns. If not set, input_column value is used directly as the prompt. |
| `temperature` | `float` | `0.7` | Temperature for response randomness (0.0-2.0) |
| `max_tokens` | `int` | — | Maximum tokens in the response |
| `response_format` | `str` | `"text"` | Response format: 'text', 'json', or 'markdown' |
| `json_schema` | `str` | — | JSON schema for structured output (JSON string) |
