...
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with medical record content |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `input_column` | `Union[str, int]` | `"text"` | Column with medical record content (text or file path) |
| `input_type` | `str` | `"text"` | Input type: 'text' (raw text) or 'file' (read file content) |
| `model` | `str` | `"gpt-4o-mini"` | LiteLLM model string |
| `output_fields` | `List[str]` | `['patient_name', 'dob', 'provider', 'visit_date', 'chief_complaint', 'diagnoses', 'icd_codes', 'medications', 'procedures', 'cpt_codes', 'follow_up']` | Fields to extract from each medical record |
