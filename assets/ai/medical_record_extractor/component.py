...
class MedicalRecordExtractorComponent(Component, Model, Resolvable):
    ...
    Each row in the upstream DataFrame is treated as one medical record. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (diagnoses, medications, icd_codes, etc.)
    ...
    upstream_asset_key: str = Field(
        description="Upstream asset key providing a DataFrame with medical record content"
    )
    input_column: Union[str, int] = Field(
        default="text",
        description="Column with medical record content (text or file path)",
    )
    ...
    output_fields: List[str] = Field(
        ...
        description="Fields to extract from each medical record",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "medical_record_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            total = len(df)
            context.log.info(
                f"Extracting medical record fields from {total} rows using {model}"
            )
