...
| `api_key` | `str` | OpenAI API key. Use ${OPENAI_API_KEY} for environment variables. |
...
| `group_name` | `str` | `"openai_llm"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `system_prompt` | `str` | — | System prompt to set context and behavior for the LLM |
| `user_prompt_template` | `str` | — | User prompt template with {column_name} placeholders for DataFrame columns |
| `temperature` | `float` | `0.7` | Temperature for response randomness (0.0-2.0). Lower = more deterministic. |
| `max_tokens` | `int` | — | Maximum tokens in response. Default uses model's max. |
| `top_p` | `float` | — | Nucleus sampling parameter (0.0-1.0). Alternative to temperature. |
| `frequency_penalty` | `float` | `0.0` | Reduce repetition of token sequences (-2.0 to 2.0) |
| `presence_penalty` | `float` | `0.0` | Increase likelihood of new topics (-2.0 to 2.0) |
| `functions` | `str` | — | JSON array of function definitions for function calling |
| `function_call` | `str` | — | Control function calling: 'auto', 'none', or {'name': 'function_name'} |
| `response_format` | `str` | — | Response format: 'text' or 'json_object' for structured outputs |
| `rate_limit_delay` | `float` | `0.1` | Delay in seconds between API calls to respect rate limits |
| `enable_caching` | `bool` | `true` | Cache responses to avoid redundant API calls |
| `cache_dir` | `str` | — | Directory for cache files. Default: /tmp/openai_llm_cache |
