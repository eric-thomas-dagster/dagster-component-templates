...
| `entity_types` | `List[str]` | — | Filter to these entity types e.g. ['PERSON','EMAIL_ADDRESS','PHONE_NUMBER']. Default=all. |
| `language` | `str` | `"en"` | Language of the text |
| `score_threshold` | `float` | `0.5` | Minimum confidence score for detected entities |
| `add_count_column` | `bool` | `true` | Add a {output_column}_count column with the number of detected entities |
