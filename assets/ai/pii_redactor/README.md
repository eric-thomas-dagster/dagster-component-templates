...
| `input_column` | `Union[str, int]` | Column containing text to redact PII from |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `output_column` | `Union[str, int]` | — | Column to write redacted text. Defaults to overwriting input_column. |
...
| `entity_types` | `List[str]` | — | Filter to these entity types. Default=all. |
| `replacement_style` | `str` | `"placeholder"` | How to replace PII: placeholder=<PERSON>, mask=****, hash=SHA256 prefix, synthetic=fake value |
| `language` | `str` | `"en"` | Language of the text |
| `score_threshold` | `float` | `0.5` | Minimum confidence score for detected entities |
