...
class PurchaseOrderExtractorComponent(Component, Model, Resolvable):
    ...
    Each row in the upstream DataFrame is treated as one purchase order. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (po_number, vendor, line_items, total, etc.)
    ...
    - Spend analytics
    - Supplier management
    - ERP system enrichment
    """

    model_config = ConfigDict(populate_by_name=True)
    asset_name: str = Field(description="Output Dagster asset name")
    upstream_asset_key: str = Field(
        description="Upstream asset key providing a DataFrame with purchase order content"
    )
    input_column: Union[str, int] = Field(
        default="text",
        description="Column with purchase order content (text or file path)",
    )
    ...
    output_fields: List[str] = Field(
        ...
        description="Fields to extract from each purchase order",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "purchase_order_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            total = len(df)
            context.log.info(
                f"Extracting purchase order fields from {total} rows using {model}"
            )
