"""Committed regression tests for PurchaseOrderExtractorComponent's
input_type='file' content-extraction fix.

Before this fix, every file (PDF, image, anything) was opened in TEXT mode
with errors='replace', so a real purchase-order PDF or scanned image got
...
def fake_litellm(monkeypatch):
    ...
    behavior per row."""
    calls = []
    state = {"response_text": '{"po_number": "value_a"}'}
...
def test_pdf_with_real_text_reaches_prompt(mod, fake_litellm, tmp_path):
    ...
    c.drawString(100, 700, "PURCHASE ORDER #98765 -- Total Due: $1,250.00")
    c.save()
    ...
    assert "PURCHASE ORDER #98765" in prompt_text
    assert "Total Due: $1,250.00" in prompt_text
...
def test_scanned_pdf_with_no_text_fails_cleanly_not_silently(mod, fake_litellm, tmp_path):
    ...
    assert df.loc[0, "po_number"] is None
...
def test_non_dict_llm_response_rejected_not_crashed(mod, fake_litellm, tmp_path):
    ...
    txt_path.write_text("some real purchase order content")
