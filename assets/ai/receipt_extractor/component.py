...
Extract structured fields from retail receipts using an LLM via litellm.
"""
...
class ReceiptExtractorComponent(Component, Model, Resolvable):
    ...
    Each row in the upstream DataFrame is treated as one receipt. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (merchant_name, total, items, etc.)
    ...
    - Loyalty program data extraction
    """

    asset_name: str = Field(description="Output Dagster asset name")
    upstream_asset_key: str = Field(
        description="Upstream asset key providing a DataFrame with receipt content"
    )
    input_column: str = Field(
        default="text",
        description="Column with receipt content (text or file path)",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "receipt_extractor"  # component directory name
        _kind_map = {
