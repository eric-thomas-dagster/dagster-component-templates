"""Committed regression tests for ReceiptExtractorComponent's
input_type='file' content-extraction fix.

Before this fix, every file (PDF, image, anything) was opened in TEXT mode
with errors='replace', so a real receipt PDF or scanned image got decoded
...
ReceiptExtractorComponent-specific behavior (differs from invoice_extractor):
...
def fake_litellm(monkeypatch):
    ...
    behavior per row."""
    calls = []
    state = {"response_text": '{"merchant_name": "Acme"}'}
...
def _make_component(mod, **overrides):
    ...
    return mod.ReceiptExtractorComponent(**kwargs)
...
def test_pdf_with_real_text_reaches_prompt(mod, fake_litellm, tmp_path):
    ...
    c.drawString(100, 700, "RECEIPT #98765 -- Total: $17.25")
    c.save()
    ...
    assert "RECEIPT #98765" in prompt_text
    assert "Total: $17.25" in prompt_text
    assert "Document:\n" in prompt_text
...
def test_scanned_pdf_with_no_text_fails_cleanly_not_silently(mod, fake_litellm, tmp_path):
    ...
    assert df.loc[0, "merchant_name"] is None
...
def test_non_dict_llm_response_rejected_not_crashed(mod, fake_litellm, tmp_path):
    ...
    txt_path.write_text("some real receipt content")
