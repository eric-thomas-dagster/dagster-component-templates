...
Extract structured profile data from resumes and CVs using an LLM via litellm.
"""
...
class ResumeExtractorComponent(Component, Model, Resolvable):
    """Component for extracting structured data from resumes and CVs using LLMs.

    Each row in the upstream DataFrame is treated as one resume. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (name, skills, experience, education, etc.)
    ...
    Use Cases:
    - Applicant tracking system enrichment
    ...
    upstream_asset_key: str = Field(
        description="Upstream asset key providing a DataFrame with resume content"
    )
    input_column: Union[str, int] = Field(
        default="text",
        description="Column with resume content (text or file path)",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "resume_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            total = len(df)
            context.log.info(
                f"Extracting resume fields from {total} rows using {model}"
            )
