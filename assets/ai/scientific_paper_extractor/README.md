...
| Field | Type | Default | Description |
|---|---|---|---|
| `input_column` | `Union[str, int]` | `"text"` | Column with scientific paper content (text or file path) |
...
- `max_content_chars` truncates an oversized real text file's content (simulating a long paper body) to the configured length before it reaches the prompt; the image row's vision content block is untouched by truncation.
