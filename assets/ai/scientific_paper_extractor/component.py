...
Extract metadata and findings from scientific papers and research articles using an LLM via litellm.
"""
...
class ScientificPaperExtractorComponent(Component, Model, Resolvable):
    """Component for extracting structured data from scientific papers using LLMs.

    Each row in the upstream DataFrame is treated as one scientific paper. The LLM
    extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (title, authors, doi, key_findings, etc.)
    ...
    upstream_asset_key: str = Field(
        description="Upstream asset key providing a DataFrame with scientific paper content"
    )
    input_column: Union[str, int] = Field(
        default="text",
        description="Column with scientific paper content (text or file path)",
    )
    ...
    output_fields: List[str] = Field(
        ...
        description="Fields to extract from each scientific paper",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "scientific_paper_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            context.log.info(
                f"Extracting scientific paper fields from {total} rows using {model}"
            )
                ...
                for _, row in batch.iterrows():
                    ...
                    try:
                        ...
                        if image_block is not None:
                            prompt = (
                                f"Extract the following fields from this scientific paper image as JSON: {output_fields}\n\n"
