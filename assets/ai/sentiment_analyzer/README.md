...
|---|---|---|
| `asset_name` | `str` | Name of the asset that will hold the sentiment analysis results |
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with text to analyze |
...
| `group_name` | `str` | `"sentiment_analysis"` | Asset group for organization |
...
| `detect_emotions` | `bool` | `false` | Detect specific emotions (joy, anger, sadness, etc.) - LLM only |
| `emotion_categories` | `str` | `"joy,anger,sadness,fear,surprise"` | Comma-separated emotion categories to detect |
| `return_reasoning` | `bool` | `false` | Return reasoning/explanation for sentiment (LLM only) |
...
| `max_tokens` | `int` | `100` | Max tokens for LLM response |
