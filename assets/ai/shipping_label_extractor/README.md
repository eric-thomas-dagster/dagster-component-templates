...
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with shipping label content |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `input_column` | `Union[str, int]` | `"text"` | Column with shipping label content (text or file path) |
| `input_type` | `str` | `"text"` | Input type: 'text' (raw text) or 'file' (read file content) |
| `model` | `str` | `"gpt-4o-mini"` | LiteLLM model string |
| `output_fields` | `List[str]` | `['tracking_number', 'carrier', 'service_type', 'sender_name', 'sender_address', 'recipient_name', 'recipient_address', 'weight', 'dimensions', 'ship_date', 'estimated_delivery']` | Fields to extract from each shipping label |
