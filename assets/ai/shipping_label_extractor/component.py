...
class ShippingLabelExtractorComponent(Component, Model, Resolvable):
    ...
    Each row in the upstream DataFrame is treated as one shipping label or packing slip.
    The LLM extracts the requested fields and returns them as additional columns.

    Features:
    - Configurable output fields (tracking_number, carrier, addresses, etc.)
    ...
    - Shipment tracking enrichment
    - Last-mile delivery analytics
    - Returns processing pipelines
    """

    model_config = ConfigDict(populate_by_name=True)
    asset_name: str = Field(description="Output Dagster asset name")
    upstream_asset_key: str = Field(
        description="Upstream asset key providing a DataFrame with shipping label content"
    )
    input_column: Union[str, int] = Field(
        default="text",
        description="Column with shipping label content (text or file path)",
    )
    ...
    output_fields: List[str] = Field(
        ...
        description="Fields to extract from each shipping label",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "shipping_label_extractor"  # component directory name
        _kind_map = {
...
group_name=self.group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            total = len(df)
            context.log.info(
                f"Extracting shipping label fields from {total} rows using {model}"
            )
