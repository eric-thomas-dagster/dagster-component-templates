"""Committed regression tests for ShippingLabelExtractorComponent's
input_type='file' content-extraction fix.

Before this fix, every file (PDF, image, anything) was opened in TEXT mode
with errors='replace', so a real shipping-label PDF or scanned image got
...
def fake_litellm(monkeypatch):
    ...
    behavior per row."""
    calls = []
    state = {"response_text": '{"tracking_number": "value_a"}'}
...
def test_pdf_with_real_text_reaches_prompt(mod, fake_litellm, tmp_path):
    ...
    c.drawString(100, 700, "TRACKING #1Z999AA10123456784 -- Carrier: UPS")
    c.save()
    ...
    component = mod.ShippingLabelExtractorComponent(
        ...
        input_type="file",
        input_column="local_path",
        output_fields=["tracking_number", "carrier"],
    )
    ...
    assert "TRACKING #1Z999AA10123456784" in prompt_text
    assert "Carrier: UPS" in prompt_text
...
def test_scanned_pdf_with_no_text_fails_cleanly_not_silently(mod, fake_litellm, tmp_path):
    ...
    assert df.loc[0, "tracking_number"] is None
...
def test_non_dict_llm_response_rejected_not_crashed(mod, fake_litellm, tmp_path):
    ...
    txt_path.write_text("some real shipping label content")
