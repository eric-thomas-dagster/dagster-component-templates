...
| `question_column` | `Union[str, int]` | Column containing natural language questions |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `output_column` | `Union[str, int]` | `"generated_sql"` | Column to write generated SQL queries |
| `model` | `str` | `"gpt-4o-mini"` | LLM model to use for SQL generation |
...
| `schema_context` | `str` | — | DDL or table description to include in the system prompt |
| `dialect` | `str` | `"generic"` | SQL dialect to generate for |
| `validate_sql` | `bool` | `false` | Run basic syntax check using sqlparse and add a validation column |
