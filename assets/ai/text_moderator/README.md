...
|---|---|---|
| `asset_name` | `str` | Name of the asset that will hold moderated content |
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with text to moderate |

### Connection

| Field | Type | Default | Description |
|---|---|---|---|
| `api_key` | `str` | — | API key for OpenAI, Anthropic, or Perspective API |
...
| `group_name` | `str` | `"content_moderation"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `input_column` | `str` | `"text"` | Column name containing text to moderate |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `method` | `str` | `"openai_moderation"` | Moderation method: openai_moderation, perspective_api, transformer, or llm |
| `llm_provider` | `str` | — | LLM provider: openai or anthropic (for method=llm) |
| `llm_model` | `str` | — | LLM model name (for method=llm) |
| `transformer_model` | `str` | — | Transformer model: unitary/toxic-bert, s-nlp/roberta-toxicity-classifier (for method=transformer) |
| `categories` | `str` | `"toxicity,hate_speech,pii,profanity,sexual,violence"` | Comma-separated moderation categories to check |
| `threshold` | `float` | `0.7` | Confidence threshold for flagging content (0.0-1.0) |
| `redact_pii` | `bool` | `false` | Redact/mask detected PII in output |
| `include_scores` | `bool` | `true` | Include confidence scores for each category |
| `flag_column` | `str` | `"flagged"` | Column name for overall flag (true if any category exceeds threshold) |
...
| `track_costs` | `bool` | `true` | Track API costs (for paid APIs) |
| `custom_profanity_list` | `str` | — | Comma-separated list of custom profanity words to detect |
