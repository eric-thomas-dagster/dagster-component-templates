...
|---|---|---|
| `asset_name` | `str` | Name of the asset that will hold classified ticket data |
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with ticket text to classify |

### Connection

| Field | Type | Default | Description |
|---|---|---|---|
| `api_key` | `str` | — | API key for LLM method. Use ${OPENAI_API_KEY} or ${ANTHROPIC_API_KEY} |
...
| `group_name` | `str` | `"ticket_classification"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `input_column` | `str` | `"ticket_text"` | Column name containing ticket text |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `method` | `str` | `"llm"` | Classification method: llm (GPT/Claude) or transformer (BERT, RoBERTa) |
| `llm_provider` | `str` | — | LLM provider: openai or anthropic (for method=llm) |
| `llm_model` | `str` | — | LLM model: gpt-4o-mini, gpt-4, claude-3-haiku, claude-3-5-sonnet (for method=llm) |
| `transformer_model` | `str` | — | Transformer model: bert-base-uncased, distilbert-base-uncased, roberta-base (for method=transformer) |
| `categories` | `str` | `"technical,billing,account,product,general,other"` | Comma-separated list of ticket categories |
| `urgency_levels` | `str` | `"low,medium,high,critical"` | Comma-separated list of urgency levels |
| `departments` | `str` | — | Comma-separated list of departments for routing (optional) |
| `classify_sentiment` | `bool` | `true` | Include sentiment classification (positive, neutral, negative) |
| `include_confidence` | `bool` | `true` | Include confidence scores for classifications |
| `include_reasoning` | `bool` | `false` | Include reasoning/explanation for classifications (LLM only) |
| `multi_label` | `bool` | `false` | Allow multiple categories per ticket (e.g., technical+billing) |
| `confidence_threshold` | `float` | — | Minimum confidence threshold (0.0-1.0). Below this = 'uncertain' |
| `temperature` | `float` | `0.0` | Temperature for LLM (0.0 = deterministic) |
| `max_tokens` | `int` | `200` | Max tokens for LLM response |
| `rate_limit_delay` | `float` | `0.1` | Delay in seconds between API calls |
| `enable_caching` | `bool` | `true` | Cache API responses to avoid redundant calls |
| `track_costs` | `bool` | `true` | Track token usage and costs (LLM only) |
| `include_metadata_fields` | `str` | — | Comma-separated list of metadata fields to include in classification (e.g., 'customer_tier,channel') |
