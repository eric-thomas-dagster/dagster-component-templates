...
class TopicModelerComponent(Component, Model, Resolvable):
    ...
    text_column: Union[str, int] = Field(description="Column containing text to model topics from")
    output_column: Union[str, int] = Field(default="topic_id", description="Column to write integer topic ID")
    topic_label_column: Union[str, int] = Field(
        default="topic_keywords",
        description="Column to write top keywords string for the assigned topic",
    )
    n_topics: int = Field(default=10, description="Number of topics to discover")
    method: str = Field(default="lda", description="Topic modeling method: lda or nmf")
    max_features: int = Field(default=1000, description="Vocabulary size for TF-IDF vectorizer")
    top_words_per_topic: int = Field(default=5, description="Number of top keywords to include in topic label")
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        topic_label_column = self.topic_label_column
        ...
        _comp_name = "topic_modeler"  # component directory name
        _kind_map = {
...
group_name=group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            if method == "lda":
                ...
                raise ValueError(f"Unknown method: {method}. Choose from lda or nmf.")

            topic_document_matrix = model.fit_transform(doc_term_matrix)
            feature_names = vectorizer.get_feature_names_out()

            # Build topic label lookup
            topic_labels = {}
            for topic_idx, topic_weights in enumerate(model.components_):
                top_indices = topic_weights.argsort()[::-1][:top_words_per_topic]
                top_words = [feature_names[i] for i in top_indices]
                topic_labels[topic_idx] = ", ".join(top_words)

            # Assign dominant topic per row
            topic_ids = topic_document_matrix.argmax(axis=1)
            df[output_column] = topic_ids
            df[topic_label_column] = [topic_labels[tid] for tid in topic_ids]
