...
|---|---|---|
| `asset_name` | `str` | Name of the asset that will hold image analysis results |
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with image references |
| `provider` | `str` | Vision model provider: openai, anthropic |
| `model` | `str` | Model name (e.g., 'gpt-4o', 'gpt-4-vision-preview', 'claude-3-opus-20240229') |
| `api_key` | `str` | API key for provider. Use ${API_KEY_NAME} for env vars. |
| `prompt` | `str` | Analysis prompt/instructions for the vision model |

### Execution

| Field | Type | Default | Description |
|---|---|---|---|
| `batch_size` | `int` | `5` | Number of images to process in parallel |
...
| `group_name` | `str` | `"vision"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `output_column` | `str` | `"vision_analysis"` | Column name for analysis results |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `image_column` | `str` | `"image_url"` | Column name containing image URLs or file paths |
| `image_type` | `str` | `"url"` | Image input type: url, path, base64 |
| `detail_level` | `str` | `"auto"` | Detail level for GPT-4 Vision: low, high, auto |
| `max_images_per_request` | `int` | `1` | Maximum images per API request (1-10 for multi-image analysis) |
| `temperature` | `float` | `0.3` | Temperature for response randomness (0.0-1.0) |
...
| `include_image_metadata` | `bool` | `true` | Include image metadata (size, format) in results |
