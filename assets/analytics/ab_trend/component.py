...
class ABTrendComponent(Component, Model, Resolvable):
    """Compute conversion-rate trend per variant over time buckets (daily, weekly, monthly)."""
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        @asset(
            ...
            description=self.description or "Compute conversion-rate trend per variant over time buckets (daily, weekly, monthly).",
            tags=tags_dict,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            df = upstream
            df = df.copy()
            df[_self.date_column] = pd.to_datetime(df[_self.date_column])
            freq = {"day": "D", "week": "W", "month": "ME"}.get(_self.bucket, "D")
            df["_bucket"] = df[_self.date_column].dt.to_period(freq[0]).dt.to_timestamp()
            grp = df.groupby(["_bucket", _self.variant_column])[_self.converted_column].agg(["count", "sum"]).reset_index()
            grp.columns = ["date_bucket", "variant", "n", "conversions"]
            grp["conversion_rate"] = grp["conversions"] / grp["n"].clip(lower=1)
            out_df = grp.sort_values(["date_bucket", "variant"]).reset_index(drop=True)
