...
| `platform` | `Literal['hubspot', 'salesforce', 'pipedrive']` | Source CRM platform to standardize |
| `resource_type` | `Literal['contacts', 'companies', 'deals', 'activities']` | Type of CRM resource to standardize |
...
| `group_name` | `str` | `"crm"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `filter_status` | `str` | — | Filter by status/stage (comma-separated) |
| `filter_owner` | `str` | — | Filter by owner name or ID |
...
| `record_id_field` | `str` | — | Field name for record ID (auto-detected if not provided) |
| `name_field` | `str` | — | Field name for name/title (auto-detected if not provided) |
| `email_field` | `str` | — | Field name for email (auto-detected if not provided) |
