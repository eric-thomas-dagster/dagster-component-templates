...
|---|---|---|
| `asset_name` | `str` | Name of the funnel analysis asset to create |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `event_data_asset_key` | `str` | — | Event/activity data with user actions and timestamps |
| `user_data_asset_key` | `str` | — | User/customer data for segmentation (optional) |
| `funnel_type` | `str` | `"linear"` | Funnel type: linear (sequential) or flexible (any order) |
| `stage_1_event` | `str` | `"page_view"` | Event name for first funnel stage |
| `stage_1_name` | `str` | `"Awareness"` | Display name for first stage |
| `stage_2_event` | `str` | `"signup"` | Event name for second funnel stage |
| `stage_2_name` | `str` | `"Signup"` | Display name for second stage |
| `stage_3_event` | `str` | — | Event name for third funnel stage (optional) |
| `stage_3_name` | `str` | — | Display name for third stage |
| `stage_4_event` | `str` | — | Event name for fourth funnel stage (optional) |
| `stage_4_name` | `str` | — | Display name for fourth stage |
| `stage_5_event` | `str` | — | Event name for fifth funnel stage (optional) |
| `stage_5_name` | `str` | — | Display name for fifth stage |
| `funnel_window_days` | `int` | `30` | Maximum days between first and last stage to count as conversion |
| `analysis_period_days` | `int` | `90` | Number of days of data to analyze |
| `group_by_cohort` | `bool` | `true` | Group results by weekly/monthly cohorts |
| `cohort_period` | `str` | `"weekly"` | Cohort period: daily, weekly, or monthly |
| `segment_by_source` | `bool` | `true` | Segment funnel by traffic source/campaign |
| `segment_by_attribute` | `str` | — | Additional attribute to segment by (e.g., plan_type, country) |
| `calculate_time_to_convert` | `bool` | `true` | Calculate median time between stages |
| `identify_drop_offs` | `bool` | `true` | Flag high drop-off stages |
| `drop_off_threshold` | `float` | `0.5` | Drop-off rate threshold to flag (e.g., 0.5 = 50% drop) |
| `include_user_level_data` | `bool` | `false` | Include individual user progression (large output) |
