"""KCentroidsDiagnosticsComponent.
...
class KCentroidsDiagnosticsComponent(Component, Model, Resolvable):
    """Compute silhouette + elbow scores across a range of k values to choose the optimal cluster count."""
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        @asset(
            ...
            description=self.description or "Compute silhouette + elbow scores across a range of k values to choose the optimal cluster count.",
            tags=tags_dict,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            rows = []
            for k in range(_self.k_min, _self.k_max + 1):
                km = KMeans(n_clusters=k, random_state=_self.random_state, n_init=10)
                labels = km.fit_predict(X)
                row = {
                    "k": int(k),
                    "inertia": float(km.inertia_),
                    "silhouette": float(silhouette_score(X, labels)) if k > 1 and k < len(X) else float("nan"),
                    "calinski_harabasz": float(calinski_harabasz_score(X, labels)) if k > 1 else float("nan"),
                    "davies_bouldin": float(davies_bouldin_score(X, labels)) if k > 1 else float("nan"),
                }
                rows.append(row)
