...
| Field | Type | Description |
|---|---|---|
| `asset_name` | `str` | Name of the lead scoring asset to create |
...
| `geography_weight` | `float` | `0.15` | Weight for geography match in fit score |
| `budget_weight` | `float` | `0.1` | Weight for budget indicators in fit score |
| `email_engagement_weight` | `float` | `0.25` | Weight for email engagement in intent score |
| `website_activity_weight` | `float` | `0.3` | Weight for website visits in intent score |
| `content_consumption_weight` | `float` | `0.2` | Weight for content downloads in intent score |
| `product_interest_weight` | `float` | `0.25` | Weight for product page views in intent score |
| `mql_threshold` | `float` | `50.0` | Score threshold for Marketing Qualified Lead |
| `sql_threshold` | `float` | `70.0` | Score threshold for Sales Qualified Lead |
| `hot_lead_threshold` | `float` | `75.0` | Score threshold for hot lead classification |
| `warm_lead_threshold` | `float` | `50.0` | Score threshold for warm lead classification |
| `include_score_breakdown` | `bool` | `true` | Include fit and intent score breakdown |
| `calculate_lead_grade` | `bool` | `true` | Calculate letter grade (A-F) in addition to score |
| `apply_time_decay` | `bool` | `true` | Apply time decay to behavioral signals |
| `time_decay_days` | `int` | `30` | Number of days for behavioral signals to decay to 50% |
