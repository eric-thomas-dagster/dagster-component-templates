...
class LogisticRegressionModelComponent(Component, Model, Resolvable):
    ...
    target_column: str = Field(description="Column name of the target class label")
    ...
    def get_description(cls) -> str:
        return "Fit a logistic regression classifier and output predicted classes and/or probabilities."
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "logistic_regression_model"  # component directory name
        _kind_map = {
...
group_name=group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: pd.DataFrame) -> pd.DataFrame:
            ...
            if _missing_feats:
                context.log.warning(
                    f"logistic_regression: feature_columns {_missing_feats} not in upstream; "
                    f"using {_present_feats or '(none)'} only."
                )
            if not _present_feats or target_column not in df.columns:
                context.log.warning(
                    "logistic_regression: no usable features or target_column missing; "
            ...
            if len(X) < 5:
                context.log.warning(
                    f"logistic_regression: only {len(X)} rows available; skipping "
            ...
            if output_predictions:
                df["predicted_class"] = model.predict(X_full)
