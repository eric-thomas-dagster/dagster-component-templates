...
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with transaction/order data |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `prediction_period_months` | `int` | `24` | Months to project forward for LTV prediction |
| `cohort_analysis` | `bool` | `true` | Include cohort-based analysis (by customer signup month) |
| `include_confidence_intervals` | `bool` | `true` | Include confidence intervals for predictions |
| `min_transactions_required` | `int` | `2` | Minimum transactions required to calculate LTV (filters out one-time buyers) |
| `customer_id_field` | `str` | — | Customer ID column name (auto-detected if not specified) |
| `transaction_date_field` | `str` | — | Transaction date column (auto-detected if not specified) |
| `amount_field` | `str` | — | Transaction amount column (auto-detected if not specified) |
