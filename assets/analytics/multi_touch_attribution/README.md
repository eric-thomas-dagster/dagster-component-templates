...
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with marketing touchpoint data |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `attribution_model` | `str` | `"linear"` | Attribution model: first_touch, last_touch, linear, time_decay, u_shaped, w_shaped |
| `lookback_window_days` | `int` | `30` | Days to look back for touchpoints before conversion |
| `time_decay_half_life_days` | `float` | `7.0` | Half-life for time decay model (days) |
| `include_channel_performance` | `bool` | `true` | Include aggregated channel performance metrics |
| `include_journey_details` | `bool` | `true` | Include detailed customer journey paths |
| `customer_id_field` | `str` | — | Customer ID column name (auto-detected if not specified) |
| `touchpoint_date_field` | `str` | — | Touchpoint timestamp column (auto-detected if not specified) |
| `channel_field` | `str` | — | Marketing channel column (auto-detected if not specified) |
| `conversion_date_field` | `str` | — | Conversion timestamp column (auto-detected if not specified) |
| `conversion_value_field` | `str` | — | Conversion value column (auto-detected if not specified) |
