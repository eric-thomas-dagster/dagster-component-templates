...
| `upstream_asset_key` | `str` | Upstream asset key providing a DataFrame with order/transaction data |
...
| `group_name` | `str` | `"product_analytics"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `recommendation_type` | `str` | `"collaborative"` | Type: collaborative, popular, frequently_bought_together |
| `num_recommendations` | `int` | `10` | Number of recommendations to generate per product |
| `min_co_occurrence` | `int` | `3` | Minimum times products must be purchased together (collaborative filtering) |
| `lookback_days` | `int` | — | Only consider recent purchases (optional, use all data if not set) |
| `customer_id_field` | `str` | — | Customer ID column (auto-detected if not specified) |
| `product_id_field` | `str` | — | Product ID column (auto-detected if not specified) |
| `order_id_field` | `str` | — | Order ID column (auto-detected if not specified) |
| `date_field` | `str` | — | Transaction date column (auto-detected if not specified) |
| `quantity_field` | `str` | — | Quantity purchased column (auto-detected if not specified) |
| `product_name_field` | `str` | — | Product name column (optional, for readable output) |
| `category_field` | `str` | — | Product category column (optional, for category-based recommendations) |
