...
|---|---|---|
| `asset_name` | `str` | Name of the product usage analytics asset to create |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `event_data_asset_key` | `str` | — | Product usage events with user_id, event_name, timestamp |
| `user_data_asset_key` | `str` | — | User data for additional context (optional) |
| `analysis_period_days` | `int` | `30` | Number of days to analyze |
| `core_feature_events` | `str` | — | Comma-separated list of core feature event names |
| `calculate_dau_mau` | `bool` | `true` | Calculate DAU/MAU ratio (stickiness) |
| `calculate_feature_adoption` | `bool` | `true` | Calculate feature adoption rates |
| `identify_power_users` | `bool` | `true` | Identify power users based on usage |
| `power_user_threshold` | `int` | `20` | Number of actions to qualify as power user |
