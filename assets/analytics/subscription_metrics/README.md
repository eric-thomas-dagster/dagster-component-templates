...
| Field | Type | Description |
|---|---|---|
| `asset_name` | `str` | Name of the subscription metrics output asset |
...
| `group_name` | `str` | `"subscription_analytics"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `stripe_data_asset_key` | `str` | — | Stripe data asset with subscriptions (automatically set via lineage) |
| `revenue_data_asset_key` | `str` | — | Revenue data for enhanced LTV calculations (optional) |
| `calculation_period` | `Literal['daily', 'weekly', 'monthly']` | `"monthly"` | Time period for metrics calculation |
| `ltv_method` | `Literal['historical', 'predictive']` | `"historical"` | Method for calculating LTV (historical average or predictive) |
| `lookback_months` | `int` | `12` | Months to look back for historical calculations |
