...
Ingest project management data from Asana (tasks, projects, teams) using dlt's Asana source.

## Overview

This component uses [dlt (data load tool)](https://dlthub.com) to extract data from Asana.
...
**Available Resources:**
Tasks, Projects, Teams, Tags, Portfolios
...
See `schema.json` for complete list of authentication and configuration parameters specific to Asana.
...
Refer to `schema.json` and the [dlt documentation](https://dlthub.com/docs) for authentication requirements specific to Asana.
...
- `dlt[asana]` - Installs dlt with Asana source
