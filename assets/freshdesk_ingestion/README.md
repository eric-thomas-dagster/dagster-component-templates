...
Ingest support data from Freshdesk (tickets, contacts, agents) using dlt's Freshdesk source.

## Overview

This component uses [dlt (data load tool)](https://dlthub.com) to extract data from Freshdesk.
...
**Available Resources:**
Tickets, Contacts, Agents, Companies, Conversations
...
See `schema.json` for complete list of authentication and configuration parameters specific to Freshdesk.
...
Refer to `schema.json` and the [dlt documentation](https://dlthub.com/docs) for authentication requirements specific to Freshdesk.
...
- `dlt[freshdesk]` - Installs dlt with Freshdesk source
