...
|---|---|---|---|
| `name` | `str` | no | Human label (logged on apply) |
| `selection` | `str` or `AssetSelection` | yes | See [Selection language](#selection-language) |
| **Shape (pick ONE):** | | | |
| `cron` | `str` | one-of | `"0 9 * * *"` → `on_cron(cron)` |
| `ignore_selection` | `str` | only with `cron` | Passed to `.ignore()` |
| `allow_selection` | `str` | only with `cron` | Passed to `.allow()` |
| `preset` | `str` | one-of | Name of any zero-arg `AutomationCondition.*` factory (`eager`, `on_missing`, `any_downstream_conditions`, etc.) |
| `derive_from_upstreams` | `bool` | one-of | `true` → derive from upstream cron metadata |
| `strategy` | `str` | with derive | `most_frequent` (default) / `least_frequent` / `tiered` / `staggered` / `any_dep_updated` / `all_deps_updated` |
| `offset_minutes` | `int` | for `staggered` | Required; positive only; cross-day-boundary rejected |
| `python` | `str` | one-of | `"module.path:function_name"` — universal escape |
| **Modifiers (compose on top):** | | | |
| `label` | `str` | no | Override auto-generated label |
| `business_hours_only` | `bool` | no | Constrain cron to business hours/days |
| `business_hours` | `str` | with `business_hours_only` | Default `"9-17"`; cron hour-field syntax |
| `business_days` | `str` | with `business_hours_only` | Default `"1-5"`; cron day-of-week syntax |
| `min_interval_minutes` | `int` | no | Debounce floor — coarsen cron if faster than this |

### Strategy summary

| Strategy | Cron output | Use when |
|---|---|---|
| `most_frequent` | Fastest upstream's cron, ignore slower | Slow deps nice-to-have, shouldn't block |
| `least_frequent` | Slowest upstream's cron, require all | Must see every dep's update before firing |
| `tiered` | Fastest cron + per-tier `allow`-gating | Daily heartbeat with periodic boundary waits |
| `staggered` | Fastest cron shifted by `offset_minutes` | Pipeline pacing (buffer after upstream) |
| `any_dep_updated` | None — event-driven | Fire whenever any upstream changes |
| `all_deps_updated` | None — strict join | Wait for all upstreams to refresh |
