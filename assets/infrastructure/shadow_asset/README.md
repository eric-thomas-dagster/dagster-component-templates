...
|---|---|---|
| [`shadow_asset.md`](https://github.com/eric-thomas-dagster/dagster-community-components-cli/blob/main/examples/shadow_asset.md) | [`setup_shadow_asset_demo.sh`](https://github.com/eric-thomas-dagster/dagster-community-components-cli/blob/main/examples/setup_shadow_asset_demo.sh) | 100% offline. Demonstrates BOTH shapes side by side: (1) `@shadow` Python decorator dual-runs primary + shadow on a `@dg.asset` (last row diverges → `shadow_match=false`, `diff_rows=1`), (2) `ShadowAssetComponent { wraps: PrimaryComp, shadow_wraps: AltComp }` — zero Python, two SyntheticDataGenerators side-by-side with different `random_state` values (perfect fit for vendor swaps like `wraps: SnowflakeQueryComponent, shadow_wraps: BigqueryQueryComponent`). Both emit the same `shadow_match` observations. |
