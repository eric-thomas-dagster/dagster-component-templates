...
|---|---|---|
| [`smart_retry.md`](https://github.com/eric-thomas-dagster/dagster-community-components-cli/blob/main/examples/smart_retry.md) | [`setup_smart_retry_demo.sh`](https://github.com/eric-thomas-dagster/dagster-community-components-cli/blob/main/examples/setup_smart_retry_demo.sh) | 100% offline. Demonstrates BOTH shapes: (1) `@smart_retry` Python decorator over `@dg.asset` — 3 modes (`success` → 1 attempt OK; `flaky` → 2× `ConnectionError` classified transient + succeeds on attempt 3 via real Dagster `STEP_RESTARTED`; `permanent` → `ValueError` classified permanent → `dg.Failure` immediately), (2) `SmartRetryComponent { wraps: SyntheticDataGeneratorComponent }` — the "money shot" composability where classification-aware retry layers over another DCC component with zero Python. Both shapes share the same classification engine. Core value over Dagster's built-in `RetryPolicy`: classification prevents thrashing on non-retryable failures. |
