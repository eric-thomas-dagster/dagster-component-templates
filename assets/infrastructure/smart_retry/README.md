...
|---|---|---|
| [`smart_retry.md`](https://github.com/eric-thomas-dagster/dagster-community-components-cli/blob/main/examples/smart_retry.md) | [`setup_smart_retry_demo.sh`](https://github.com/eric-thomas-dagster/dagster-community-components-cli/blob/main/examples/setup_smart_retry_demo.sh) | 100% offline. Three runs of one `@smart_retry`-wrapped asset, controlled by `MODE`: `success` → 1 attempt OK; `flaky` → 2× `ConnectionError` (classified transient) + succeeds on attempt 3 via real Dagster `STEP_RESTARTED`; `permanent` → `ValueError` (classified permanent) → `dg.Failure` immediately, no retry. Demonstrates the core value over Dagster's built-in `RetryPolicy`: classification prevents thrashing on non-retryable failures. |
