"""ActiveCampaignIngestionComponent -- Ingest ActiveCampaign marketing/CRM data using dlt's generic REST API source.

Ingest ActiveCampaign data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). ActiveCampaign has no official
...
class ActiveCampaignIngestionComponent(Component, Model, Resolvable):
    ...
    account: str = Field(description='ActiveCampaign account subdomain (from https://{account}.api-us1.com).')

    api_token: str = Field(description='ActiveCampaign API token. Use ${ACTIVECAMPAIGN_API_TOKEN} for env vars.')

    resources: str = Field(
        default="contacts,deals,accounts,lists,campaigns",
        description="Comma-separated list of resources to extract: contacts, deals, accounts, lists, campaigns"
    )
    ...
    group_name: Optional[str] = Field(default="activecampaign_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest ActiveCampaign marketing/CRM data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["activecampaign", "python"]
        ...
        def activecampaign_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting ActiveCampaign ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://{self.account}.api-us1.com/api/3",
                    "auth": {"type": "api_key", "api_key": self.api_token, "name": "Api-Token", "location": "header"},
                },
                "resources": [],
            }
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": r, "data_selector": r}})
            ...
            context.log.info("Creating ActiveCampaign REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting ActiveCampaign data...")
            load_info = pipeline.run(source)
            context.log.info(f"ActiveCampaign data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from ActiveCampaign.")
        ...
        return Definitions(assets=[activecampaign_ingestion_asset])
