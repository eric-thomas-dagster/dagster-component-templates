"""AcuitySchedulingIngestionComponent -- Ingest Acuity Scheduling appointment, client, and calendar data using dlt's generic REST API source.

Ingest Acuity Scheduling data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Acuity Scheduling has no official
...
class AcuitySchedulingIngestionComponent(Component, Model, Resolvable):
    ...
    user_id: str = Field(description='Acuity numeric user ID, used as the HTTP Basic auth username.')

    api_key: str = Field(description='Acuity API key, used as the HTTP Basic auth password.')

    resources: str = Field(
        default="appointments,clients",
        description="Comma-separated list of resources to extract: appointments, clients, calendars, appointment_types"
    )
    ...
    group_name: Optional[str] = Field(default="acuity_scheduling_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Acuity Scheduling appointment, client, and calendar data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["acuity_scheduling", "python"]
        ...
        def acuity_scheduling_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Acuity Scheduling ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://acuityscheduling.com/api/v1",
                    "auth": {"type": "http_basic", "username": self.user_id, "password": self.api_key},
                },
                "resources": [],
            }
            _appt_params = {}
            if context.has_partition_key:
                try:
                    ...
                    _appt_params["minDate"] = _window.start.date().isoformat()
                    _appt_params["maxDate"] = _window.end.date().isoformat()
                except Exception:
                    pass
            _map = {
                "appointments": {"name": "appointments", "endpoint": {"path": "appointments", "data_selector": "$", "params": _appt_params}},
                "clients": {"name": "clients", "endpoint": {"path": "clients", "data_selector": "$"}},
                "calendars": {"name": "calendars", "endpoint": {"path": "calendars", "data_selector": "$"}},
                "appointment_types": {"name": "appointment_types", "endpoint": {"path": "appointment-types", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Acuity Scheduling REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Acuity Scheduling data...")
            load_info = pipeline.run(source)
            context.log.info(f"Acuity Scheduling data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Acuity Scheduling.")
        ...
        return Definitions(assets=[acuity_scheduling_ingestion_asset])
