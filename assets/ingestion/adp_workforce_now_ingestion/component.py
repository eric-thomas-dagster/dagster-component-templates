"""AdpWorkforceNowIngestionComponent -- Ingest ADP Workforce Now HR data using dlt's generic REST API source.

Ingest ADP Workforce Now data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). ADP Workforce Now has no official
...
class AdpWorkforceNowIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='ADP OAuth2 access token (obtained via mutual-TLS client credentials flow -- see README). Use ${ADP_ACCESS_TOKEN} for env vars.')

    resources: str = Field(
        default="workers,work_assignments",
        description="Comma-separated list of resources to extract: workers, work_assignments, associate_contacts, job_requisitions"
    )
    ...
    group_name: Optional[str] = Field(default="adp_workforce_now_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest ADP Workforce Now HR data using dlt's generic REST API source."
        ...
        def adp_workforce_now_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting ADP Workforce Now ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "workers": {"name": "workers", "endpoint": {"path": "hr/v2/workers", "data_selector": "workers"}},
                "work_assignments": {"name": "work_assignments", "endpoint": {"path": "hr/v2/work-assignments", "data_selector": "workAssignments"}},
                "associate_contacts": {"name": "associate_contacts", "endpoint": {"path": "hr/v2/associate-contacts", "data_selector": "associateContacts"}},
                "job_requisitions": {"name": "job_requisitions", "endpoint": {"path": "hr/v2/job-requisitions", "data_selector": "jobRequisitions"}},
            }
            ...
            context.log.info("Creating ADP Workforce Now REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting ADP Workforce Now data...")
            load_info = pipeline.run(source)
            context.log.info(f"ADP Workforce Now data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from ADP Workforce Now.")
        ...
        return Definitions(assets=[adp_workforce_now_ingestion_asset])
