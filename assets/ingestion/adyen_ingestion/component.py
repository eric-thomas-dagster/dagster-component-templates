"""AdyenIngestionComponent -- Ingest Adyen merchant and store data via the Management API using dlt's generic REST API source.

Ingest Adyen data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Adyen has no official
...
class AdyenIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='Adyen API key, sent as the X-API-Key header.')

    is_live: Optional[bool] = Field(default=None, description='Set true for the live environment. Defaults to the test environment.')

    live_url_prefix: Optional[str] = Field(default=None, description='Your company-specific live URL prefix (required if is_live=true -- found in the Adyen Customer Area).')

    merchant_id: Optional[str] = Field(default=None, description='Merchant account ID. Required for the stores resource.')

    resources: str = Field(
        default="merchants",
        description="Comma-separated list of resources to extract: merchants (account-level); stores (requires merchant_id)"
    )
    ...
    group_name: Optional[str] = Field(default="adyen_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Adyen merchant and store data via the Management API using dlt's generic REST API source."
        ...
        def adyen_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Adyen ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            if self.is_live:
                if not self.live_url_prefix:
                    raise ValueError("Adyen: live_url_prefix is required when is_live=true.")
                _base = f"https://{self.live_url_prefix}-management-live.adyenpayments.com/v3"
            else:
                _base = "https://management-test.adyen.com/v3"

            config = {
                "client": {
                    "base_url": _base,
                    "auth": {"type": "api_key", "api_key": self.api_key, "name": "X-API-Key", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "merchants": {"name": "merchants", "endpoint": {"path": "merchants", "data_selector": "data"}},
            }
            if self.merchant_id:
                _map["stores"] = {"name": "stores", "endpoint": {"path": f"merchants/{self.merchant_id}/stores", "data_selector": "data"}}
            for r in resources_list:
                if r in _map:
                    config["resources"].append(_map[r])
                elif r == "stores":
                    context.log.warning("Resource 'stores' requires merchant_id to be set; skipping.")
            ...
            context.log.info("Creating Adyen REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Adyen data...")
            load_info = pipeline.run(source)
            context.log.info(f"Adyen data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Adyen.")
        ...
        return Definitions(assets=[adyen_ingestion_asset])
