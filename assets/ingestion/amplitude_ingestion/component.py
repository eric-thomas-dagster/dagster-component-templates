"""AmplitudeIngestionComponent -- Ingest Amplitude event definitions and cohort metadata using dlt's generic REST API source.

Ingest Amplitude data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Amplitude has no official
...
class AmplitudeIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: Optional[str] = Field(default=None, description='Amplitude project API key, used as the HTTP Basic auth username. Required unless resource_key is set.')

    secret_key: Optional[str] = Field(default=None, description='Amplitude project secret key, used as the HTTP Basic auth password. Always required inline -- NOT sourced from resource_key, because AmplitudeResourceComponent only stores an API-key env var (for the HTTP V2 event-track API) and has no secret-key field.')
    ...
    eu_residency: Optional[bool] = Field(default=None, description='Set true if your Amplitude project uses EU data residency.')

    resources: str = Field(
        default="events_list",
        description="Comma-separated list of resources to extract: events_list, cohorts"
    )
    ...
    group_name: Optional[str] = Field(default="amplitude_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        component = self
        if not self.resource_key and not (self.api_key and self.secret_key):
            raise ValueError(
                "AmplitudeIngestionComponent: supply resource_key (a registered "
                "AmplitudeResourceComponent) OR both of api_key/secret_key."
            )
        if not self.secret_key:
            raise ValueError(
                "AmplitudeIngestionComponent: secret_key must always be supplied inline -- "
                "it is not available from AmplitudeResourceComponent (which only stores an "
                "API-key env var, not a secret key)."
            )
        asset_name = self.asset_name
        description = self.description or "Ingest Amplitude event definitions and cohort metadata using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["amplitude", "python"]
        ...
        def amplitude_ingestion_asset(context: AssetExecutionContext):
            ...
            secret_key = component.secret_key

            context.log.info(f"Starting Amplitude ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            _base = "https://analytics.eu.amplitude.com" if component.eu_residency else "https://amplitude.com"
            config = {
                "client": {
                    "base_url": _base,
            ...
            _map = {
                "events_list": {"name": "events_list", "endpoint": {"path": "api/2/events/list", "data_selector": "data"}},
                "cohorts": {"name": "cohorts", "endpoint": {"path": "api/3/cohorts", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Amplitude REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Amplitude data...")
            load_info = pipeline.run(source)
            context.log.info(f"Amplitude data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Amplitude.")
        ...
        return Definitions(assets=[amplitude_ingestion_asset])
