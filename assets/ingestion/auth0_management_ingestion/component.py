"""Auth0ManagementIngestionComponent -- Ingest Auth0 org data (users, roles, organizations) via the Management API using dlt's generic REST API source. Distinct from auth0_logs_ingestion, which covers audit/log-stream data only.

Ingest Auth0 (Management API) data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Auth0 (Management API) has no official
...
class Auth0ManagementIngestionComponent(Component, Model, Resolvable):
    ...
    tenant_domain: str = Field(description='Your Auth0 tenant domain.')

    client_id: str = Field(description='Auth0 Machine-to-Machine application client ID (authorized for the Management API).')

    client_secret: str = Field(description='Auth0 M2M application client secret.')

    resources: str = Field(
        default="users,roles",
        description="Comma-separated list of resources to extract: users, roles, organizations"
    )
    ...
    group_name: Optional[str] = Field(default="auth0_management_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Auth0 org data (users, roles, organizations) via the Management API using dlt's generic REST API source. Distinct from auth0_logs_ingestion, which covers audit/log-stream data only."
        ...
        def auth0_management_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Auth0 (Management API) ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            config = {
                "client": {
                    "base_url": f"https://{self.tenant_domain}/api/v2",
                    "auth": {"type": "bearer", "token": _access_token},
                },
                "resources": [],
            }
            _map = {
                "users": {"name": "users", "endpoint": {"path": "users", "data_selector": "$"}},
                "roles": {"name": "roles", "endpoint": {"path": "roles", "data_selector": "$"}},
                "organizations": {"name": "organizations", "endpoint": {"path": "organizations", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Auth0 (Management API) REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Auth0 (Management API) data...")
            load_info = pipeline.run(source)
            context.log.info(f"Auth0 (Management API) data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Auth0 (Management API).")
        ...
        return Definitions(assets=[auth0_management_ingestion_asset])
