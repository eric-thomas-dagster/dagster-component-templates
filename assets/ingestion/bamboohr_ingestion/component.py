"""BambooHrIngestionComponent -- Ingest BambooHR employee data using dlt's generic REST API source.

Ingest BambooHR data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). BambooHR has no official
...
class BambooHrIngestionComponent(Component, Model, Resolvable):
    ...
    company_domain: str = Field(description='BambooHR company subdomain (from https://{company_domain}.bamboohr.com).')

    api_key: str = Field(description='BambooHR API key (HTTP Basic username, blank password). Use ${BAMBOOHR_API_KEY} for env vars.')

    resources: str = Field(
        default="employees",
        description="Comma-separated list of resources to extract: employees (directory), changed_employees"
    )
    ...
    group_name: Optional[str] = Field(default="bamboohr_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest BambooHR employee data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["bamboohr", "python"]
        ...
        def bamboohr_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting BambooHR ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://{self.company_domain}.bamboohr.com/api/v1",
                    "auth": {"type": "http_basic", "username": self.api_key, "password": ""},
                },
                "resources": [],
            }
            _map = {
                "employees": {"name": "employees", "endpoint": {"path": "employees/directory", "data_selector": "employees"}},
                "changed_employees": {"name": "changed_employees", "endpoint": {"path": "employees/changed"}},
            }
            ...
            context.log.info("Creating BambooHR REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting BambooHR data...")
            load_info = pipeline.run(source)
            context.log.info(f"BambooHR data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from BambooHR.")
        ...
        return Definitions(assets=[bamboohr_ingestion_asset])
