"""BasecampIngestionComponent -- Ingest Basecamp projects/todos data using dlt's generic REST API source.

Ingest Basecamp data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Basecamp has no official
...
class BasecampIngestionComponent(Component, Model, Resolvable):
    ...
    account_id: str = Field(description='Basecamp account ID (from your Basecamp URL).')

    access_token: str = Field(description='Basecamp OAuth2 access token. Use ${BASECAMP_ACCESS_TOKEN} for env vars.')

    resources: str = Field(
        default="projects",
        description="Comma-separated list of resources to extract: projects, todos (todos requires project_id + todolist_id)"
    )
    ...
    group_name: Optional[str] = Field(default="basecamp_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Basecamp projects/todos data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["basecamp", "python"]
        ...
        def basecamp_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Basecamp ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://3.basecampapi.com/{self.account_id}",
                    "auth": {"type": "bearer", "token": self.access_token},
                },
                "resources": [],
            }
            if "projects" in resources_list:
                config["resources"].append({"name": "projects", "endpoint": {"path": "projects.json"}})
            ...
            context.log.info("Creating Basecamp REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Basecamp data...")
            load_info = pipeline.run(source)
            context.log.info(f"Basecamp data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Basecamp.")
        ...
        return Definitions(assets=[basecamp_ingestion_asset])
