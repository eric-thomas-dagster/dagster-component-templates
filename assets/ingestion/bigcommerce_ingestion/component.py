"""BigCommerceIngestionComponent -- Ingest BigCommerce product, customer, and category data using dlt's generic REST API source.

Ingest BigCommerce data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). BigCommerce has no official
...
class BigCommerceIngestionComponent(Component, Model, Resolvable):
    ...
    store_hash: str = Field(description="BigCommerce store hash (from your store's API path).")

    access_token: str = Field(description='BigCommerce API access token, sent as the X-Auth-Token header.')

    resources: str = Field(
        default="products,customers",
        description="Comma-separated list of resources to extract: products, customers, categories"
    )
    ...
    group_name: Optional[str] = Field(default="bigcommerce_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest BigCommerce product, customer, and category data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["bigcommerce", "python"]
        ...
        def bigcommerce_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting BigCommerce ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://api.bigcommerce.com/stores/{self.store_hash}/v3",
                    "auth": {"type": "api_key", "api_key": self.access_token, "name": "X-Auth-Token", "location": "header"},
                    "headers": {"Accept": "application/json"},
                },
                "resources": [],
            }
            _map = {
                "products": {"name": "products", "endpoint": {"path": "catalog/products", "data_selector": "data"}},
                "customers": {"name": "customers", "endpoint": {"path": "customers", "data_selector": "data"}},
                "categories": {"name": "categories", "endpoint": {"path": "catalog/categories", "data_selector": "data"}},
            }
            ...
            context.log.info("Creating BigCommerce REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting BigCommerce data...")
            load_info = pipeline.run(source)
            context.log.info(f"BigCommerce data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from BigCommerce.")
        ...
        return Definitions(assets=[bigcommerce_ingestion_asset])
