"""BillComIngestionComponent -- Ingest Bill.com AP/AR data using dlt's generic REST API source.

Ingest Bill.com data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Bill.com has no official
...
class BillComIngestionComponent(Component, Model, Resolvable):
    ...
    username: str = Field(description='Bill.com login username.')

    password: str = Field(description='Bill.com login password.')

    organization_id: str = Field(description='Bill.com organization ID.')

    dev_key: str = Field(description='Bill.com developer key issued to your integration.')

    resources: str = Field(
        default="invoices,vendors",
        description="Comma-separated list of resources to extract: invoices, vendors, customers, bills"
    )
    ...
    group_name: Optional[str] = Field(default="bill_com_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Bill.com AP/AR data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["bill_com", "python"]
        ...
        def bill_com_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Bill.com ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _login_resp = _requests.post(
                "https://gateway.bill.com/connect/v3/login",
                json={
            ...
            _login_resp.raise_for_status()
            _session_id = _login_resp.json()["sessionId"]

            config = {
                "client": {
                    "base_url": "https://gateway.bill.com/connect/v3",
                    "auth": {"type": "api_key", "api_key": _session_id, "name": "sessionId", "location": "header"},
                    "headers": {"devKey": self.dev_key},
                },
                "resources": [],
            }
            _map = {
                "invoices": {"name": "invoices", "endpoint": {"path": "invoices", "data_selector": "results"}},
                "vendors": {"name": "vendors", "endpoint": {"path": "vendors", "data_selector": "results"}},
                "customers": {"name": "customers", "endpoint": {"path": "customers", "data_selector": "results"}},
                "bills": {"name": "bills", "endpoint": {"path": "bills", "data_selector": "results"}},
            }
            ...
            context.log.info("Creating Bill.com REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Bill.com data...")
            load_info = pipeline.run(source)
            context.log.info(f"Bill.com data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Bill.com.")
        ...
        return Definitions(assets=[bill_com_ingestion_asset])
