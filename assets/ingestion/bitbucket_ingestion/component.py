"""BitbucketIngestionComponent -- Ingest Bitbucket Cloud repository data using dlt's generic REST API source.

Ingest Bitbucket data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Bitbucket has no official
...
class BitbucketIngestionComponent(Component, Model, Resolvable):
    ...
    api_token: str = Field(description='Atlassian API token (scoped), used as the Basic auth password.')

    repo_slug: Optional[str] = Field(default=None, description='Repository slug. Required for pullrequests/commits/issues resources; not needed for repositories.')

    resources: str = Field(
        default="repositories",
        description="Comma-separated list of resources to extract: repositories (workspace-level); pullrequests, commits, issues (require repo_slug)"
    )
    ...
    group_name: Optional[str] = Field(default="bitbucket_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Bitbucket Cloud repository data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["bitbucket", "python"]
        ...
        def bitbucket_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Bitbucket ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "repositories": {"name": "repositories", "endpoint": {"path": f"repositories/{self.workspace}", "data_selector": "values"}},
            }
            if self.repo_slug:
                _map.update({
                    "pullrequests": {"name": "pullrequests", "endpoint": {"path": f"repositories/{self.workspace}/{self.repo_slug}/pullrequests", "data_selector": "values", "params": {"state": "ALL"}}},
                    "commits": {"name": "commits", "endpoint": {"path": f"repositories/{self.workspace}/{self.repo_slug}/commits", "data_selector": "values"}},
                    "issues": {"name": "issues", "endpoint": {"path": f"repositories/{self.workspace}/{self.repo_slug}/issues", "data_selector": "values"}},
                })
            for r in resources_list:
                if r in _map:
                    config["resources"].append(_map[r])
                else:
                    context.log.warning(f"Resource {r!r} requires repo_slug to be set; skipping.")
            ...
            context.log.info("Creating Bitbucket REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Bitbucket data...")
            load_info = pipeline.run(source)
            context.log.info(f"Bitbucket data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Bitbucket.")
        ...
        return Definitions(assets=[bitbucket_ingestion_asset])
