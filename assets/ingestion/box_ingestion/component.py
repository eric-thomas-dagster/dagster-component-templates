"""BoxIngestionComponent -- Ingest Box file/folder listings and users using dlt's generic REST API source, authenticated via Client Credentials Grant (service account).

Ingest Box data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Box has no official
...
class BoxIngestionComponent(Component, Model, Resolvable):
    ...
    client_id: str = Field(description='Box app client ID (configured for Client Credentials Grant).')

    client_secret: str = Field(description='Box app client secret.')

    box_subject_type: str = Field(description="Box CCG subject type: 'enterprise' or 'user'.")

    box_subject_id: str = Field(description='Box enterprise ID (if subject_type=enterprise) or user ID (if subject_type=user).')

    folder_id: Optional[str] = Field(default=None, description="Box folder ID to list children of. Defaults to '0' (root) if unset.")

    resources: str = Field(
        default="folder_items,users",
        description="Comma-separated list of resources to extract: folder_items (children of folder_id, default root), users"
    )
    ...
    group_name: Optional[str] = Field(default="box_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Box file/folder listings and users using dlt's generic REST API source, authenticated via Client Credentials Grant (service account)."
        ...
        def box_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Box ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _token_resp = _requests.post(
                ...
                data={
                    ...
                    "box_subject_type": self.box_subject_type,
                    "box_subject_id": self.box_subject_id,
                },
            )
            ...
            _map = {
                "folder_items": {"name": "folder_items", "endpoint": {"path": f"folders/{self.folder_id or '0'}/items", "data_selector": "entries"}},
                "users": {"name": "users", "endpoint": {"path": "users", "data_selector": "entries"}},
            }
            ...
            context.log.info("Creating Box REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Box data...")
            load_info = pipeline.run(source)
            context.log.info(f"Box data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Box.")
        ...
        return Definitions(assets=[box_ingestion_asset])
