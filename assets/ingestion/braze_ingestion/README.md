...
| `rest_endpoint` | one of rest_endpoint/api_key OR resource_key | Account-specific Braze REST endpoint (from the Braze dashboard). |
| `api_key` | one of rest_endpoint/api_key OR resource_key | Braze REST API key. Use ${BRAZE_API_KEY} for env vars. |
| `resource_key` | no | Resource key registered by a BrazeResourceComponent. When set, rest_endpoint/api_key are read from that resource at run time -- lets one Braze credential serve both this connector and the dataframe_to_braze reverse-ETL sink without configuring it twice. |
...
|---|---|---|---|
| `api_key` | `str` | — | Braze REST API key. Use ${BRAZE_API_KEY} for env vars. Required unless resource_key is set. |
| `resource_key` | `str` | — | Optional resource key registered by a BrazeResourceComponent. When set, credentials are read from that resource at run time instead of rest_endpoint/api_key above -- lets one Braze credential serve both this ingestion co… _(full docs in schema.json + component README)_ |
...
| `group_name` | `str` | `"braze_ingestion"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `rest_endpoint` | `str` | — | Account-specific Braze REST endpoint (from the Braze dashboard). Required unless resource_key is set. |
| `resources` | `str` | `"segments,campaigns,canvases,content_blocks"` | Comma-separated list of resources to extract: segments, campaigns, canvases, content_blocks |
...
## Example — sharing credentials with a BrazeResourceComponent

```yaml
type: dagster_component_templates.BrazeIngestionComponent
attributes:
  asset_name: braze_ingestion
  resource_key: braze   # matches BrazeResourceComponent's default resource_key
  resources: "segments,campaigns,canvases,content_blocks"
```
