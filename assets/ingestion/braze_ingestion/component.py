"""BrazeIngestionComponent -- Ingest Braze marketing automation data using dlt's generic REST API source.

Ingest Braze data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Braze has no official
...
class BrazeIngestionComponent(Component, Model, Resolvable):
    ...
    resources: str = Field(
        ...
        description="Comma-separated list of resources to extract: segments, campaigns, canvases, content_blocks"
    )
    ...
    group_name: Optional[str] = Field(default="braze_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        def braze_ingestion_asset(context: AssetExecutionContext):
            ...
            _map = {
                "segments": {"name": "segments", "endpoint": {"path": "segments/list", "data_selector": "segments"}},
                "campaigns": {"name": "campaigns", "endpoint": {"path": "campaigns/list", "data_selector": "campaigns"}},
                "canvases": {"name": "canvases", "endpoint": {"path": "canvas/list", "data_selector": "canvases"}},
                "content_blocks": {"name": "content_blocks", "endpoint": {"path": "content_blocks/list", "data_selector": "content_blocks"}},
            }
            ...
            context.log.info("Creating Braze REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Braze data...")
            load_info = pipeline.run(source)
            context.log.info(f"Braze data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Braze.")
        ...
        return Definitions(assets=[braze_ingestion_asset])
