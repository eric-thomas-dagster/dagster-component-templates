"""BuildkiteIngestionComponent -- Ingest Buildkite organization, pipeline, and build data using dlt's generic REST API source.

Ingest Buildkite data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Buildkite has no official
...
class BuildkiteIngestionComponent(Component, Model, Resolvable):
    ...
    api_token: str = Field(description='Buildkite API access token.')

    org_slug: Optional[str] = Field(default=None, description='Organization slug. Required for pipelines/builds resources; not needed for organizations.')

    resources: str = Field(
        default="organizations,pipelines",
        description="Comma-separated list of resources to extract: organizations (account-level); pipelines, builds (require org_slug)"
    )
    ...
    group_name: Optional[str] = Field(default="buildkite_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Buildkite organization, pipeline, and build data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["buildkite", "python"]
        ...
        def buildkite_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Buildkite ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.buildkite.com/v2",
                    "auth": {"type": "bearer", "token": self.api_token},
                },
                "resources": [],
            }
            _map = {
            ...
            if self.org_slug:
                _map.update({
                    "pipelines": {"name": "pipelines", "endpoint": {"path": f"organizations/{self.org_slug}/pipelines", "data_selector": "$"}},
                    "builds": {"name": "builds", "endpoint": {"path": f"organizations/{self.org_slug}/builds", "data_selector": "$"}},
                })
            for r in resources_list:
                if r in _map:
                    config["resources"].append(_map[r])
                else:
                    context.log.warning(f"Resource {r!r} requires org_slug to be set; skipping.")
            ...
            context.log.info("Creating Buildkite REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Buildkite data...")
            load_info = pipeline.run(source)
            context.log.info(f"Buildkite data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Buildkite.")
        ...
        return Definitions(assets=[buildkite_ingestion_asset])
