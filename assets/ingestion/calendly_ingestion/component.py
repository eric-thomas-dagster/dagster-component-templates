"""CalendlyIngestionComponent -- Ingest Calendly scheduling data using dlt's generic REST API source.

Ingest Calendly data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Calendly has no official
...
class CalendlyIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Calendly personal access token. Use ${CALENDLY_ACCESS_TOKEN} for env vars.')

    organization_uri: Optional[str] = Field(default=None, description='Organization URI to scope events/memberships to (optional).')

    resources: str = Field(
        default="scheduled_events,event_types",
        description="Comma-separated list of resources to extract: scheduled_events, event_types, organization_memberships"
    )
    ...
    group_name: Optional[str] = Field(default="calendly_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Calendly scheduling data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["calendly", "python"]
        ...
        def calendly_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Calendly ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _org_params = {"organization": self.organization_uri} if self.organization_uri else {}
            _events_params = dict(_org_params)
            if context.has_partition_key:
                try:
                    _window = context.partition_time_window
                    _events_params["min_start_time"] = _window.start.isoformat() + "Z"
                    _events_params["max_start_time"] = _window.end.isoformat() + "Z"
                except Exception:
                    pass
            _map = {
                "scheduled_events": {"name": "scheduled_events", "endpoint": {"path": "scheduled_events", "data_selector": "collection", "params": _events_params}},
                "event_types": {"name": "event_types", "endpoint": {"path": "event_types", "data_selector": "collection", "params": _org_params}},
                "organization_memberships": {"name": "organization_memberships", "endpoint": {"path": "organization_memberships", "data_selector": "collection"}},
            }
            ...
            context.log.info("Creating Calendly REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Calendly data...")
            load_info = pipeline.run(source)
            context.log.info(f"Calendly data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Calendly.")
        ...
        return Definitions(assets=[calendly_ingestion_asset])
