"""ChargebeeIngestionComponent -- Ingest Chargebee subscription billing data using dlt's generic REST API source.

Ingest Chargebee data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Chargebee has no official
...
class ChargebeeIngestionComponent(Component, Model, Resolvable):
    ...
    site: str = Field(description='Chargebee site name (for mysite.chargebee.com).')

    api_key: str = Field(description='Chargebee API key, used as the HTTP Basic auth username (password is blank).')

    resources: str = Field(
        default="customers,subscriptions,invoices",
        description="Comma-separated list of resources to extract: customers, subscriptions, invoices, transactions, credit_notes"
    )
    ...
    group_name: Optional[str] = Field(default="chargebee_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Chargebee subscription billing data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["chargebee", "python"]
        ...
        def chargebee_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Chargebee ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://{self.site}.chargebee.com/api/v2",
                    "auth": {"type": "http_basic", "username": self.api_key, "password": ""},
                },
                "resources": [],
            }
            _date_params = {}
            if context.has_partition_key:
                try:
                    ...
                    _date_params = {
                        "created_at[after]": int(_window.start.timestamp()),
                        "created_at[before]": int(_window.end.timestamp()),
                    }
                except Exception:
                    pass
            _map = {
                "customers": {"name": "customers", "endpoint": {"path": "customers", "data_selector": "list"}},
                "subscriptions": {"name": "subscriptions", "endpoint": {"path": "subscriptions", "data_selector": "list"}},
                "invoices": {"name": "invoices", "endpoint": {"path": "invoices", "data_selector": "list", "params": _date_params}},
                "transactions": {"name": "transactions", "endpoint": {"path": "transactions", "data_selector": "list", "params": _date_params}},
                "credit_notes": {"name": "credit_notes", "endpoint": {"path": "credit_notes", "data_selector": "list"}},
            }
            ...
            context.log.info("Creating Chargebee REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Chargebee data...")
            load_info = pipeline.run(source)
            context.log.info(f"Chargebee data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Chargebee.")
        ...
        return Definitions(assets=[chargebee_ingestion_asset])
