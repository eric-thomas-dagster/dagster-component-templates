"""ChargifyIngestionComponent -- Ingest Chargify/Maxio Advanced Billing subscription, customer, and invoice data using dlt's generic REST API source.

Ingest Chargify (Maxio Advanced Billing) data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Chargify (Maxio Advanced Billing) has no official
...
class ChargifyIngestionComponent(Component, Model, Resolvable):
    ...
    subdomain: str = Field(description='Chargify subdomain (for mysite.chargify.com).')

    api_key: str = Field(description="Chargify API key, used as the HTTP Basic auth username (password is the literal string 'x').")

    resources: str = Field(
        default="subscriptions,customers",
        description="Comma-separated list of resources to extract: subscriptions, customers, invoices, products"
    )
    ...
    group_name: Optional[str] = Field(default="chargify_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Chargify/Maxio Advanced Billing subscription, customer, and invoice data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["chargify", "python"]
        ...
        def chargify_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Chargify (Maxio Advanced Billing) ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://{self.subdomain}.chargify.com",
                    "auth": {"type": "http_basic", "username": self.api_key, "password": "x"},
                },
                "resources": [],
            }
            _map = {
                "subscriptions": {"name": "subscriptions", "endpoint": {"path": "subscriptions.json", "data_selector": "$"}},
                "customers": {"name": "customers", "endpoint": {"path": "customers.json", "data_selector": "$"}},
                "invoices": {"name": "invoices", "endpoint": {"path": "invoices.json", "data_selector": "$"}},
                "products": {"name": "products", "endpoint": {"path": "products.json", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Chargify (Maxio Advanced Billing) REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Chargify (Maxio Advanced Billing) data...")
            load_info = pipeline.run(source)
            context.log.info(f"Chargify (Maxio Advanced Billing) data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Chargify (Maxio Advanced Billing).")
        ...
        return Definitions(assets=[chargify_ingestion_asset])
