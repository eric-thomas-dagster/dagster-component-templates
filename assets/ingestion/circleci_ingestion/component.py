"""CircleCIIngestionComponent -- Ingest CircleCI pipeline and workflow data using dlt's generic REST API source.

Ingest CircleCI data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). CircleCI has no official
...
class CircleCIIngestionComponent(Component, Model, Resolvable):
    ...
    api_token: str = Field(description='CircleCI personal API token.')

    project_slug: Optional[str] = Field(default=None, description="Project slug (e.g. 'gh/my-org/my-repo'). Required for the project_pipelines resource.")

    resources: str = Field(
        default="pipelines",
        description="Comma-separated list of resources to extract: pipelines (account-level); project_pipelines (requires project_slug)"
    )
    ...
    group_name: Optional[str] = Field(default="circleci_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest CircleCI pipeline and workflow data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["circleci", "python"]
        ...
        def circleci_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting CircleCI ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://circleci.com/api/v2",
                    "auth": {"type": "api_key", "api_key": self.api_token, "name": "Circle-Token", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "pipelines": {"name": "pipelines", "endpoint": {"path": "pipeline", "data_selector": "items"}},
            }
            if self.project_slug:
                _map["project_pipelines"] = {"name": "project_pipelines", "endpoint": {"path": f"project/{self.project_slug}/pipeline", "data_selector": "items"}}
            for r in resources_list:
                if r in _map:
                    config["resources"].append(_map[r])
                elif r == "project_pipelines":
                    context.log.warning("Resource 'project_pipelines' requires project_slug to be set; skipping.")
            ...
            context.log.info("Creating CircleCI REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting CircleCI data...")
            load_info = pipeline.run(source)
            context.log.info(f"CircleCI data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from CircleCI.")
        ...
        return Definitions(assets=[circleci_ingestion_asset])
