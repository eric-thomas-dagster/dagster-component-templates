"""ClerkIngestionComponent -- Ingest Clerk user and organization data using dlt's generic REST API source.

Ingest Clerk data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Clerk has no official
...
class ClerkIngestionComponent(Component, Model, Resolvable):
    ...
    secret_key: str = Field(description='Clerk secret key (starts with sk_).')

    resources: str = Field(
        default="users,organizations",
        description="Comma-separated list of resources to extract: users, organizations, sessions"
    )
    ...
    group_name: Optional[str] = Field(default="clerk_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Clerk user and organization data using dlt's generic REST API source."
        ...
        def clerk_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Clerk ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.clerk.com/v1",
                    "auth": {"type": "bearer", "token": self.secret_key},
                },
                "resources": [],
            }
            _map = {
                ...
                "sessions": {"name": "sessions", "endpoint": {"path": "sessions", "data_selector": "data"}},
            }
            ...
            context.log.info("Creating Clerk REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Clerk data...")
            load_info = pipeline.run(source)
            context.log.info(f"Clerk data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Clerk.")
        ...
        return Definitions(assets=[clerk_ingestion_asset])
