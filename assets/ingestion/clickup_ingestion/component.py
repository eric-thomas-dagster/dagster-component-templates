"""ClickUpIngestionComponent -- Ingest ClickUp project management data using dlt's generic REST API source.

Ingest ClickUp data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). ClickUp has no official
...
class ClickUpIngestionComponent(Component, Model, Resolvable):
    ...
    api_token: str = Field(description='ClickUp personal API token or OAuth2 token. Use ${CLICKUP_API_TOKEN} for env vars.')

    resources: str = Field(
        default="spaces,folders,lists",
        description="Comma-separated list of resources to extract: spaces, folders, lists, tasks"
    )
    ...
    group_name: Optional[str] = Field(default="clickup_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest ClickUp project management data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["clickup", "python"]
        ...
        def clickup_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting ClickUp ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "spaces": {"name": "spaces", "endpoint": {"path": f"team/{self.team_id}/space", "data_selector": "spaces"}},
                "folders": {"name": "folders", "endpoint": {"path": f"team/{self.team_id}/space", "data_selector": "spaces"}},
                "lists": {"name": "lists", "endpoint": {"path": f"team/{self.team_id}/space", "data_selector": "spaces"}},
                "tasks": {"name": "tasks", "endpoint": {"path": f"team/{self.team_id}/task", "data_selector": "tasks"}},
            }
            ...
            context.log.info("Creating ClickUp REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting ClickUp data...")
            load_info = pipeline.run(source)
            context.log.info(f"ClickUp data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from ClickUp.")
        ...
        return Definitions(assets=[clickup_ingestion_asset])
