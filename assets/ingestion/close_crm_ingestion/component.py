"""CloseCrmIngestionComponent -- Ingest Close CRM data using dlt's generic REST API source.

Ingest Close CRM data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Close CRM has no official
...
class CloseCrmIngestionComponent(Component, Model, Resolvable):
    """Ingest Close CRM data using dlt's generic REST API source.
    ...
    api_key: str = Field(description='Close API key (used as HTTP Basic username, blank password). Use ${CLOSE_API_KEY} for env vars.')

    resources: str = Field(
        default="leads,contacts,activities,opportunities",
        description="Comma-separated list of resources to extract: leads, contacts, activities, opportunities"
    )
    ...
    group_name: Optional[str] = Field(default="close_crm_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Close CRM data using dlt's generic REST API source."
        ...
        def close_crm_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Close CRM ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {"leads": "lead", "contacts": "contact", "activities": "activity", "opportunities": "opportunity"}
            for r in resources_list:
                if r in _map:
                    config["resources"].append({"name": r, "endpoint": {"path": f"{_map[r]}/", "data_selector": "data"}})
            ...
            context.log.info("Creating Close CRM REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Close CRM data...")
            load_info = pipeline.run(source)
            context.log.info(f"Close CRM data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Close CRM.")
        ...
        return Definitions(assets=[close_crm_ingestion_asset])
