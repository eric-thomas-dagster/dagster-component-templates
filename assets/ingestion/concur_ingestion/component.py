"""ConcurIngestionComponent -- Ingest SAP Concur travel/expense data using dlt's generic REST API source.

Ingest SAP Concur data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). SAP Concur has no official
...
class ConcurIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='SAP Concur OAuth2 access token. Use ${CONCUR_ACCESS_TOKEN} for env vars.')

    datacenter_url: Optional[str] = Field(default=None, description='Region-specific Concur API base (e.g. us/eu/cn datacenter). Defaults to the US datacenter.')

    resources: str = Field(
        default="users,expense",
        description="Comma-separated list of resources to extract: users, vendors, expense"
    )
    ...
    group_name: Optional[str] = Field(default="concur_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest SAP Concur travel/expense data using dlt's generic REST API source."
        ...
        def concur_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting SAP Concur ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            base = (self.datacenter_url or "https://us.api.concursolutions.com").rstrip("/")
            config = {
                "client": {
                    "base_url": f"{base}/v4",
            ...
            context.log.info("Creating SAP Concur REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting SAP Concur data...")
            load_info = pipeline.run(source)
            context.log.info(f"SAP Concur data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from SAP Concur.")
        ...
        return Definitions(assets=[concur_ingestion_asset])
