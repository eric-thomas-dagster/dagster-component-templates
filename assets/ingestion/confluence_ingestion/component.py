"""ConfluenceIngestionComponent -- Ingest Confluence Cloud pages and spaces using dlt's generic REST API source.

Ingest Confluence data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Confluence has no official
...
class ConfluenceIngestionComponent(Component, Model, Resolvable):
    ...
    site_domain: str = Field(description='Your Atlassian site subdomain (for yoursite.atlassian.net).')
    ...
    space_id: Optional[str] = Field(default=None, description='Optional space ID to scope pages/blogposts to a single space.')

    resources: str = Field(
        default="pages,spaces",
        description="Comma-separated list of resources to extract: pages, spaces, blogposts, attachments"
    )
    ...
    group_name: Optional[str] = Field(default="confluence_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Confluence Cloud pages and spaces using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["confluence", "python"]
        ...
        def confluence_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Confluence ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://{self.site_domain}.atlassian.net/wiki/api/v2",
                    "auth": {"type": "http_basic", "username": self.email, "password": self.api_token},
                },
                "resources": [],
            }
            _scoped_params = {"space-id": self.space_id} if self.space_id else {}
            _map = {
                "pages": {"name": "pages", "endpoint": {"path": "pages", "data_selector": "results", "params": _scoped_params}},
                "spaces": {"name": "spaces", "endpoint": {"path": "spaces", "data_selector": "results"}},
                "blogposts": {"name": "blogposts", "endpoint": {"path": "blogposts", "data_selector": "results", "params": _scoped_params}},
                "attachments": {"name": "attachments", "endpoint": {"path": "attachments", "data_selector": "results"}},
            }
            ...
            context.log.info("Creating Confluence REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Confluence data...")
            load_info = pipeline.run(source)
            context.log.info(f"Confluence data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Confluence.")
        ...
        return Definitions(assets=[confluence_ingestion_asset])
