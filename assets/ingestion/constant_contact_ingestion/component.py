"""ConstantContactIngestionComponent -- Ingest Constant Contact contact and list data using dlt's generic REST API source.

Ingest Constant Contact data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Constant Contact has no official
...
class ConstantContactIngestionComponent(Component, Model, Resolvable):
    ...
    client_id: str = Field(description='Constant Contact app client ID (used for the OAuth2 refresh-token exchange).')

    client_secret: str = Field(description='Constant Contact app client secret.')

    refresh_token: str = Field(description="OAuth2 refresh token obtained once via Constant Contact's OAuth consent flow -- no static API-key-only path exists for this API.")

    resources: str = Field(
        default="contacts,contact_lists",
        description="Comma-separated list of resources to extract: contacts, contact_lists, contact_custom_fields"
    )
    ...
    group_name: Optional[str] = Field(default="constant_contact_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Constant Contact contact and list data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["constant_contact", "python"]
        ...
        def constant_contact_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Constant Contact ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "contacts": {"name": "contacts", "endpoint": {"path": "contacts", "data_selector": "contacts"}},
                "contact_lists": {"name": "contact_lists", "endpoint": {"path": "contact_lists", "data_selector": "lists"}},
                "contact_custom_fields": {"name": "contact_custom_fields", "endpoint": {"path": "contact_custom_fields", "data_selector": "custom_fields"}},
            }
            ...
            context.log.info("Creating Constant Contact REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Constant Contact data...")
            load_info = pipeline.run(source)
            context.log.info(f"Constant Contact data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Constant Contact.")
        ...
        return Definitions(assets=[constant_contact_ingestion_asset])
