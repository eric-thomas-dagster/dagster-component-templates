"""ConvertKitIngestionComponent -- Ingest ConvertKit (rebranded 'Kit') subscriber, broadcast, and tag data using dlt's generic REST API source.

Ingest ConvertKit (Kit) data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). ConvertKit (Kit) has no official
...
class ConvertKitIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='ConvertKit (Kit) v4 API key, sent as the X-Kit-Api-Key header.')

    resources: str = Field(
        default="subscribers,tags",
        description="Comma-separated list of resources to extract: subscribers, broadcasts, tags, forms"
    )
    ...
    group_name: Optional[str] = Field(default="convertkit_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest ConvertKit (rebranded 'Kit') subscriber, broadcast, and tag data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["convertkit", "python"]
        ...
        def convertkit_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting ConvertKit (Kit) ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.kit.com/v4",
                    "auth": {"type": "api_key", "api_key": self.api_key, "name": "X-Kit-Api-Key", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "subscribers": {"name": "subscribers", "endpoint": {"path": "subscribers", "data_selector": "subscribers"}},
                "broadcasts": {"name": "broadcasts", "endpoint": {"path": "broadcasts", "data_selector": "broadcasts"}},
                "tags": {"name": "tags", "endpoint": {"path": "tags", "data_selector": "tags"}},
                "forms": {"name": "forms", "endpoint": {"path": "forms", "data_selector": "forms"}},
            }
            ...
            context.log.info("Creating ConvertKit (Kit) REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting ConvertKit (Kit) data...")
            load_info = pipeline.run(source)
            context.log.info(f"ConvertKit (Kit) data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from ConvertKit (Kit).")
        ...
        return Definitions(assets=[convertkit_ingestion_asset])
