"""CopperIngestionComponent -- Ingest Copper CRM data using dlt's generic REST API source.

Ingest Copper CRM data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Copper CRM has no official
...
class CopperIngestionComponent(Component, Model, Resolvable):
    """Ingest Copper CRM data using dlt's generic REST API source.
    ...
    api_key: str = Field(description='Copper API key. Use ${COPPER_API_KEY} for env vars.')

    user_email: str = Field(description='Email of the Copper user the API key belongs to (required by Copper as X-PW-UserEmail).')

    resources: str = Field(
        default="people,activities,projects",
        description="Comma-separated list of resources to extract: people, activities, projects"
    )
    ...
    group_name: Optional[str] = Field(default="copper_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Copper CRM data using dlt's generic REST API source."
        ...
        def copper_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Copper CRM ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.copper.com/developer_api/v1",
                    "auth": {"type": "api_key", "api_key": self.api_key, "name": "X-PW-AccessToken", "location": "header"},
                    "headers": {"X-PW-UserEmail": self.user_email, "Content-Type": "application/json"},
                },
                "resources": [],
            }
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": r, "method": "POST"}})
            ...
            context.log.info("Creating Copper CRM REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Copper CRM data...")
            load_info = pipeline.run(source)
            context.log.info(f"Copper CRM data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Copper CRM.")
        ...
        return Definitions(assets=[copper_ingestion_asset])
