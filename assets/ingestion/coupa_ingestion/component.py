"""CoupaIngestionComponent -- Ingest Coupa procurement data using dlt's generic REST API source.

Ingest Coupa data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Coupa has no official
...
class CoupaIngestionComponent(Component, Model, Resolvable):
    ...
    instance: str = Field(description='Coupa instance subdomain (for mycompany.coupahost.com).')

    client_id: str = Field(description='Coupa OAuth2 client_credentials client ID (integration user).')

    client_secret: str = Field(description='Coupa OAuth2 client_credentials client secret.')

    resources: str = Field(
        default="suppliers,purchase_orders",
        description="Comma-separated list of resources to extract: suppliers, purchase_orders, invoices, users, expense_reports"
    )
    ...
    group_name: Optional[str] = Field(default="coupa_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Coupa procurement data using dlt's generic REST API source."
        ...
        def coupa_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Coupa ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _token_resp = _requests.post(
                f"https://{self.instance}.coupahost.com/oauth2/token",
            ...
            config = {
                "client": {
                    "base_url": f"https://{self.instance}.coupahost.com/api",
                    "auth": {"type": "bearer", "token": _access_token},
                },
                "resources": [],
            }
            _map = {
                "suppliers": {"name": "suppliers", "endpoint": {"path": "suppliers", "data_selector": "$"}},
                "purchase_orders": {"name": "purchase_orders", "endpoint": {"path": "purchase_orders", "data_selector": "$"}},
                "invoices": {"name": "invoices", "endpoint": {"path": "invoices", "data_selector": "$"}},
                "users": {"name": "users", "endpoint": {"path": "users", "data_selector": "$"}},
                "expense_reports": {"name": "expense_reports", "endpoint": {"path": "expense_reports", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Coupa REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Coupa data...")
            load_info = pipeline.run(source)
            context.log.info(f"Coupa data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Coupa.")
        ...
        return Definitions(assets=[coupa_ingestion_asset])
