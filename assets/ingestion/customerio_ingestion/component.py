"""CustomerIOIngestionComponent -- Ingest Customer.io App API data (campaigns, segments, newsletters) using dlt's generic REST API source.

Ingest Customer.io data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Customer.io has no official
...
class CustomerIOIngestionComponent(Component, Model, Resolvable):
    ...
    app_api_key: str = Field(description='Customer.io App API bearer token.')

    resources: str = Field(
        default="campaigns,segments",
        description="Comma-separated list of resources to extract: campaigns, segments, newsletters"
    )
    ...
    group_name: Optional[str] = Field(default="customerio_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Customer.io App API data (campaigns, segments, newsletters) using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["customerio", "python"]
        ...
        def customerio_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Customer.io ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.customer.io/v1",
                    "auth": {"type": "bearer", "token": self.app_api_key},
                },
                "resources": [],
            }
            _map = {
                "campaigns": {"name": "campaigns", "endpoint": {"path": "campaigns", "data_selector": "campaigns"}},
                "segments": {"name": "segments", "endpoint": {"path": "segments", "data_selector": "segments"}},
                "newsletters": {"name": "newsletters", "endpoint": {"path": "newsletters", "data_selector": "newsletters"}},
            }
            ...
            context.log.info("Creating Customer.io REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Customer.io data...")
            load_info = pipeline.run(source)
            context.log.info(f"Customer.io data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Customer.io.")
        ...
        return Definitions(assets=[customerio_ingestion_asset])
