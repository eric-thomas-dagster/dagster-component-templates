...
| `view_ddl_overrides` | `Dict[str, str]` | — | Per-view body override. Keys are source qualified names (e.g. 'HR.V_LEGACY'), values are the SELECT body (NOT including CREATE VIEW). Used when source uses dialect-specific SQL the target can't run (Oracle CONNECT BY, MS… _(full docs in schema.json + component README)_ |
