"""DeelIngestionComponent -- Ingest Deel global payroll/contractor data using dlt's generic REST API source.

Ingest Deel data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Deel has no official
...
class DeelIngestionComponent(Component, Model, Resolvable):
    ...
    api_token: str = Field(description='Deel API bearer token. Use ${DEEL_API_TOKEN} for env vars.')

    client_id: Optional[str] = Field(default=None, description='Deel OAuth2 client ID (sent as x-client-id header; only needed for OAuth2 apps, not personal API tokens).')

    resources: str = Field(
        default="people,contracts",
        description="Comma-separated list of resources to extract: people, contracts, legal_entities, invoices"
    )
    ...
    group_name: Optional[str] = Field(default="deel_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Deel global payroll/contractor data using dlt's generic REST API source."
        ...
        def deel_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Deel ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.letsdeel.com/rest",
                    "auth": {"type": "bearer", "token": self.api_token},
                },
                "resources": [],
            }
            if self.client_id:
                config["client"]["headers"] = {"x-client-id": self.client_id}
            _map = {
                "people": "rest/people", "contracts": "rest/v2/contracts",
                "legal_entities": "rest/legal-entities", "invoices": "rest/invoices",
            }
            for r in resources_list:
                if r in _map:
                    config["resources"].append({"name": r, "endpoint": {"path": _map[r], "data_selector": "data"}})
            ...
            context.log.info("Creating Deel REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Deel data...")
            load_info = pipeline.run(source)
            context.log.info(f"Deel data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Deel.")
        ...
        return Definitions(assets=[deel_ingestion_asset])
