"""DoceboIngestionComponent -- Ingest Docebo LMS user, course, and enrollment data using dlt's generic REST API source.

Ingest Docebo data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Docebo has no official
...
class DoceboIngestionComponent(Component, Model, Resolvable):
    ...
    subdomain: str = Field(description='Docebo subdomain (for mycompany.docebosaas.com).')

    client_id: str = Field(description='Docebo OAuth2 client ID.')

    client_secret: str = Field(description='Docebo OAuth2 client secret.')

    resources: str = Field(
        default="users,courses",
        description="Comma-separated list of resources to extract: users, courses, enrollments"
    )
    ...
    group_name: Optional[str] = Field(default="docebo_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Docebo LMS user, course, and enrollment data using dlt's generic REST API source."
        ...
        def docebo_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Docebo ingestion, destination={destination or 'duckdb (in-memory)'}")

            import requests as _requests

            resources_list = [r.strip() for r in resources.split(",")]
            _base = f"https://{self.subdomain}.docebosaas.com"
            ...
            _map = {
                "users": {"name": "users", "endpoint": {"path": "manage/v1/user", "data_selector": "data.items"}},
                "courses": {"name": "courses", "endpoint": {"path": "learn/v1/courses", "data_selector": "data.items"}},
                "enrollments": {"name": "enrollments", "endpoint": {"path": "learn/v1/enrollments", "data_selector": "data.items"}},
            }
            ...
            context.log.info("Creating Docebo REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Docebo data...")
            load_info = pipeline.run(source)
            context.log.info(f"Docebo data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Docebo.")
        ...
        return Definitions(assets=[docebo_ingestion_asset])
