"""DocuSignIngestionComponent -- Ingest DocuSign envelope and template data using dlt's generic REST API source.

Ingest DocuSign data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). DocuSign has no official
...
class DocuSignIngestionComponent(Component, Model, Resolvable):
    ...
    integration_key: str = Field(description='DocuSign Integration Key (client ID) with an RSA keypair registered for JWT grant.')

    user_id: str = Field(description='GUID of the DocuSign user being impersonated (must have granted one-time JWT consent).')

    private_key: str = Field(description='RSA private key (PEM format) matching the public key registered on the Integration Key.')

    account_id: Optional[str] = Field(default=None, description='Specific DocuSign account ID to use. If unset, the default account from the JWT userinfo response is used.')

    use_demo_env: Optional[bool] = Field(default=None, description="Use DocuSign's demo/sandbox environment (account-d.docusign.com) instead of production.")

    resources: str = Field(
        default="envelopes",
        description="Comma-separated list of resources to extract: envelopes, templates, users"
    )
    ...
    group_name: Optional[str] = Field(default="docusign_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest DocuSign envelope and template data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["docusign", "python"]
        ...
        def docusign_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting DocuSign ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            try:
                ...
                raise ImportError(
                    "docusign_ingestion requires the 'pyjwt' and 'cryptography' packages for "
                    "JWT-grant auth. Install them (see requirements.txt)."
                ) from _e

            resources_list = [r.strip() for r in resources.split(",")]
            _auth_host = "account-d.docusign.com" if self.use_demo_env else "account.docusign.com"

            _now = int(_time.time())
            _assertion = _jwt.encode(
                {
            ...
            _userinfo_resp = _requests.get(
                f"https://{_auth_host}/oauth/userinfo",
                headers={"Authorization": f"Bearer {_access_token}"},
            )
            _userinfo_resp.raise_for_status()
            _accounts = _userinfo_resp.json().get("accounts", [])
            if self.account_id:
                _account = next((a for a in _accounts if a["account_id"] == self.account_id), None)
            else:
                _account = next((a for a in _accounts if a.get("is_default")), _accounts[0] if _accounts else None)
            if not _account:
                raise ValueError("Could not resolve a DocuSign account_id from the JWT userinfo response.")
            _base_uri = _account["base_uri"]
            _resolved_account_id = _account["account_id"]

            _envelope_params = {"from_date": "2000-01-01T00:00:00Z"}
            if context.has_partition_key:
                try:
                    _window = context.partition_time_window
                    _envelope_params = {
                        "from_date": _window.start.isoformat() + "Z",
                        "to_date": _window.end.isoformat() + "Z",
                    }
                except Exception:
                    pass
            ...
            config = {
                "client": {
                    "base_url": f"{_base_uri}/restapi/v2.1/accounts/{_resolved_account_id}",
                    "auth": {"type": "bearer", "token": _access_token},
                },
                "resources": [],
            }
            _map = {
                "envelopes": {"name": "envelopes", "endpoint": {"path": "envelopes", "data_selector": "envelopes", "params": _envelope_params}},
                "templates": {"name": "templates", "endpoint": {"path": "templates", "data_selector": "envelopeTemplates"}},
                "users": {"name": "users", "endpoint": {"path": "users", "data_selector": "users"}},
            }
            ...
            context.log.info("Creating DocuSign REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting DocuSign data...")
            load_info = pipeline.run(source)
            context.log.info(f"DocuSign data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from DocuSign.")
        ...
        return Definitions(assets=[docusign_ingestion_asset])
