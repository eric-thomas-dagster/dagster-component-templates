"""DropboxIngestionComponent -- Ingest Dropbox file/folder listings.

Dropbox's file-listing API is POST + JSON-body (`files/list_folder`,
`files/list_folder/continue`), not GET + query-params -- an awkward,
unverified fit for dlt's generic rest_api_source, which assumes REST
path/query-param semantics and auto-detected pagination styles. Also,
Dropbox deprecated long-lived static access tokens -- only a refresh_token
OAuth2 flow is supported now. Rather than force a bad fit, this connector
is hand-written with a plain @dlt.resource generator that POSTs directly
via `requests` and walks Dropbox's own cursor/has_more continuation
protocol, then hands that resource to a normal dlt pipeline -- so
destination flexibility (snowflake/bigquery/postgres/filesystem/duckdb/
etc, same as every other ingestion component in this repo) is preserved
even though the fetch mechanism differs. Matches the same reasoning
already applied to Linear and monday.com (GraphQL-only APIs) elsewhere
in this repo.
...
class DropboxIngestionComponent(Component, Model, Resolvable):
    ...
    client_id: str = Field(description="Dropbox app client ID (used for the OAuth2 refresh-token exchange).")
    client_secret: str = Field(description="Dropbox app client secret.")
    refresh_token: str = Field(description="OAuth2 refresh token obtained once via Dropbox's OAuth consent flow (token_access_type=offline) -- Dropbox deprecated long-lived static access tokens, so there is no simpler static-token path.")
    folder_path: str = Field(default="", description="Dropbox folder path to list, e.g. '/Reports'. Empty string lists the root.")
    recursive: bool = Field(default=True, description="List all nested subfolders' contents too, not just the immediate folder.")

    resources: str = Field(
        default="files",
        description="Comma-separated list of resources to extract: files"
    )
    ...
    group_name: Optional[str] = Field(default="dropbox_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Dropbox file/folder listings via a hand-written cursor-paginated fetch."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["dropbox", "python"]
        ...
        def dropbox_ingestion_asset(context: AssetExecutionContext):
            context.log.info(f"Starting Dropbox ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            def _files_resource():
                ...
                resp = requests.post(
                    ...
                    json={"path": folder_path, "recursive": recursive},
                    headers=headers,
                )
                resp.raise_for_status()
                payload = resp.json()
                yield payload["entries"]
            ...
            for r in resources_list:
                if r == "files":
                    dlt_resources.append(_files_resource())
            ...
            context.log.info("Extracting Dropbox data...")
            load_info = pipeline.run(dlt_resources)
            context.log.info(f"Dropbox data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Dropbox.")
        ...
        return Definitions(assets=[dropbox_ingestion_asset])
