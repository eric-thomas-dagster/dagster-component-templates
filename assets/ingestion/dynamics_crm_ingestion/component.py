"""DynamicsCrmIngestionComponent -- Ingest Microsoft Dynamics 365 CRM data (accounts, contacts, opportunities, leads) via the Dataverse Web API (OData v4) using dlt's generic REST API source.

Ingest Microsoft Dynamics 365 (Dataverse) data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Microsoft Dynamics 365 (Dataverse) has no official
...
class DynamicsCrmIngestionComponent(Component, Model, Resolvable):
    ...
    org_url: str = Field(description='Your Dynamics 365 organization URL.')

    tenant_id: str = Field(description='Azure AD (Entra ID) tenant ID.')

    client_id: str = Field(description='Azure AD app registration client ID (with API permissions granted for Dataverse).')

    client_secret: str = Field(description='Azure AD app registration client secret.')

    resources: str = Field(
        default="accounts,contacts",
        description="Comma-separated list of resources to extract: accounts, contacts, opportunities, leads"
    )
    ...
    group_name: Optional[str] = Field(default="dynamics_crm_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Microsoft Dynamics 365 CRM data (accounts, contacts, opportunities, leads) via the Dataverse Web API (OData v4) using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["dynamics_crm", "python"]
        ...
        def dynamics_crm_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Microsoft Dynamics 365 (Dataverse) ingestion, destination={destination or 'duckdb (in-memory)'}")

            import requests as _requests

            resources_list = [r.strip() for r in resources.split(",")]
            _org_url = self.org_url.rstrip("/")
            ...
            config = {
                "client": {
                    "base_url": f"{_org_url}/api/data/v9.2",
            ...
            _map = {
                "accounts": {"name": "accounts", "endpoint": {"path": "accounts", "data_selector": "value"}},
                "contacts": {"name": "contacts", "endpoint": {"path": "contacts", "data_selector": "value"}},
                "opportunities": {"name": "opportunities", "endpoint": {"path": "opportunities", "data_selector": "value"}},
                "leads": {"name": "leads", "endpoint": {"path": "leads", "data_selector": "value"}},
            }
            ...
            context.log.info("Creating Microsoft Dynamics 365 (Dataverse) REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Microsoft Dynamics 365 (Dataverse) data...")
            load_info = pipeline.run(source)
            context.log.info(f"Microsoft Dynamics 365 (Dataverse) data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Microsoft Dynamics 365 (Dataverse).")
        ...
        return Definitions(assets=[dynamics_crm_ingestion_asset])
