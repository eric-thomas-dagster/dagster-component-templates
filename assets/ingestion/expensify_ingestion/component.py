"""ExpensifyIngestionComponent -- Ingest Expensify expense report data using dlt's generic REST API source.

Ingest Expensify data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Expensify has no official
...
class ExpensifyIngestionComponent(Component, Model, Resolvable):
    ...
    partner_user_id: str = Field(description='Expensify partnerUserID. Use ${EXPENSIFY_PARTNER_USER_ID} for env vars.')

    partner_user_secret: str = Field(description='Expensify partnerUserSecret. Use ${EXPENSIFY_PARTNER_USER_SECRET} for env vars.')

    resources: str = Field(
        default="report_list,transaction_list",
        description="Comma-separated list of resources to extract: report_list, transaction_list, receipt_list, policy_list"
    )
    ...
    group_name: Optional[str] = Field(default="expensify_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Expensify expense report data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["expensify", "python"]
        ...
        def expensify_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Expensify ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            # Expensify's Integration Server takes credentials inside the JSON request body
            # (requestJobDescription), not a standard Authorization header -- so auth is baked
            # into each resource's json body via dlt's "json" request param, not a client-level
            # auth block.
            _creds = {"partnerUserID": self.partner_user_id, "partnerUserSecret": self.partner_user_secret}
            config = {
                "client": {"base_url": "https://integrations.expensify.com/Integration-Server/ExpensifyIntegrations"},
                "resources": [],
            }
            _map = {
                "report_list": "reportList", "transaction_list": "transactionList",
                "receipt_list": "receiptList", "policy_list": "policyList",
            }
                ...
                if r in _map:
                    config["resources"].append({
                        ...
                        "endpoint": {
                            "path": "api",
                            "method": "POST",
                            "data_selector": _map[r],
                            "json": {"type": "get", "credentials": _creds},
                        },
                    })
            ...
            context.log.info("Creating Expensify REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Expensify data...")
            load_info = pipeline.run(source)
            context.log.info(f"Expensify data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Expensify.")
        ...
        return Definitions(assets=[expensify_ingestion_asset])
