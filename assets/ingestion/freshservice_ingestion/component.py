"""FreshserviceIngestionComponent -- Ingest Freshservice ITSM ticket, requester, agent, and asset data using dlt's generic REST API source. Distinct product from Freshdesk.

Ingest Freshservice data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Freshservice has no official
...
class FreshserviceIngestionComponent(Component, Model, Resolvable):
    ...
    domain: str = Field(description='Freshservice domain.')

    api_key: str = Field(description="Freshservice API key, used as the HTTP Basic auth username (password is the literal string 'X').")

    resources: str = Field(
        default="tickets,requesters",
        description="Comma-separated list of resources to extract: tickets, requesters, agents, assets"
    )
    ...
    group_name: Optional[str] = Field(default="freshservice_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Freshservice ITSM ticket, requester, agent, and asset data using dlt's generic REST API source. Distinct product from Freshdesk."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["freshservice", "python"]
        ...
        def freshservice_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Freshservice ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "tickets": {"name": "tickets", "endpoint": {"path": "tickets", "data_selector": "tickets"}},
                "requesters": {"name": "requesters", "endpoint": {"path": "requesters", "data_selector": "requesters"}},
                "agents": {"name": "agents", "endpoint": {"path": "agents", "data_selector": "agents"}},
                "assets": {"name": "assets", "endpoint": {"path": "assets", "data_selector": "assets"}},
            }
            ...
            context.log.info("Creating Freshservice REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Freshservice data...")
            load_info = pipeline.run(source)
            context.log.info(f"Freshservice data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Freshservice.")
        ...
        return Definitions(assets=[freshservice_ingestion_asset])
