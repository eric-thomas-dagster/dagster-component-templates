"""FrontIngestionComponent -- Ingest Front support/inbox data using dlt's generic REST API source.

Ingest Front data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Front has no official
...
class FrontIngestionComponent(Component, Model, Resolvable):
    ...
    api_token: str = Field(description='Front API token. Use ${FRONT_API_TOKEN} for env vars.')

    resources: str = Field(
        default="conversations,contacts,accounts,teammates,inboxes",
        description="Comma-separated list of resources to extract: conversations, contacts, accounts, teammates, inboxes"
    )
    ...
    group_name: Optional[str] = Field(default="front_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Front support/inbox data using dlt's generic REST API source."
        ...
        def front_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Front ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": r, "data_selector": "_results"}})
            ...
            context.log.info("Creating Front REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Front data...")
            load_info = pipeline.run(source)
            context.log.info(f"Front data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Front.")
        ...
        return Definitions(assets=[front_ingestion_asset])
