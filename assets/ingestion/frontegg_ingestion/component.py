"""FronteggIngestionComponent -- Ingest Frontegg (auth-as-a-service) user and tenant data using dlt's generic REST API source.

Ingest Frontegg data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Frontegg has no official
...
class FronteggIngestionComponent(Component, Model, Resolvable):
    ...
    client_id: str = Field(description='Frontegg vendor client ID.')

    api_key: str = Field(description='Frontegg vendor API key/secret.')

    region: Optional[str] = Field(default=None, description="Frontegg region: 'us' (default), 'eu', 'ca', or 'au'.")

    resources: str = Field(
        default="users,tenants",
        description="Comma-separated list of resources to extract: users, tenants"
    )
    ...
    group_name: Optional[str] = Field(default="frontegg_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Frontegg (auth-as-a-service) user and tenant data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["frontegg", "python"]
        ...
        def frontegg_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Frontegg ingestion, destination={destination or 'duckdb (in-memory)'}")

            import requests as _requests

            resources_list = [r.strip() for r in resources.split(",")]
            _region_map = {"eu": "api.frontegg.com", "us": "api.us.frontegg.com", "ca": "api.ca.frontegg.com", "au": "api.au.frontegg.com"}
            _base = f"https://{_region_map.get(self.region or 'eu', 'api.frontegg.com')}"

            _token_resp = _requests.post(
                f"{_base}/auth/vendor",
                json={"clientId": self.client_id, "secret": self.api_key},
            )
            _token_resp.raise_for_status()
            _access_token = _token_resp.json()["token"]
            ...
            _map = {
                "users": {"name": "users", "endpoint": {"path": "identity/resources/users/v2", "data_selector": "items"}},
                "tenants": {"name": "tenants", "endpoint": {"path": "identity/resources/tenants/v1", "data_selector": "items"}},
            }
            ...
            context.log.info("Creating Frontegg REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Frontegg data...")
            load_info = pipeline.run(source)
            context.log.info(f"Frontegg data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Frontegg.")
        ...
        return Definitions(assets=[frontegg_ingestion_asset])
