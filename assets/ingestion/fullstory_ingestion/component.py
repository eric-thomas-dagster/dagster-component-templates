"""FullstoryIngestionComponent -- Ingest Fullstory user, segment, and settings data using dlt's generic REST API source.

Ingest Fullstory data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Fullstory has no official
...
class FullstoryIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='Fullstory API key, used as the HTTP Basic auth username (password is blank).')

    resources: str = Field(
        default="users,segments",
        description="Comma-separated list of resources to extract: users, segments"
    )
    ...
    group_name: Optional[str] = Field(default="fullstory_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Fullstory user, segment, and settings data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["fullstory", "python"]
        ...
        def fullstory_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Fullstory ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "users": {"name": "users", "endpoint": {"path": "v2/users", "data_selector": "$"}},
                "segments": {"name": "segments", "endpoint": {"path": "segments/v1", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Fullstory REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Fullstory data...")
            load_info = pipeline.run(source)
            context.log.info(f"Fullstory data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Fullstory.")
        ...
        return Definitions(assets=[fullstory_ingestion_asset])
