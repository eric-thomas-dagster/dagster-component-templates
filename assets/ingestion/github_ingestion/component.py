...
Ingest GitHub repository data using dlt (data load tool). Extracts issues,
pull requests, comments, and reactions.
...
class GitHubIngestionComponent(Component, Model, Resolvable):
    ...
    Extracts issues, pull requests, comments, and reactions from GitHub's
    REST API via dlt's verified `github_reactions` source. dlt handles
    pagination, rate limiting, and incremental loading automatically.

    Available resources: `issues`, `pull_requests`, `comments`, `reactions`.
    ...
    group_name: Optional[str] = Field(
        default="github", description="Asset group for organization"
    )
        ...
        def github_ingestion_asset(context: AssetExecutionContext):
            ...
            for resource_name in resources_list:
                if hasattr(source, resource_name):
                    ...
                    context.log.warning(
                        f"Resource '{resource_name}' not found in GitHub source; skipping."
                    )
            if not selected_resources:
                raise ValueError(
                    "No valid resources selected. Available: issues, pull_requests, comments, reactions."
                )
            ...
            base_metadata = {
                ...
                "resources_extracted": MetadataValue.json(resources_list),
            }

            # Decide return shape
            #   - persist_only OR non-SQL destination → MaterializeResult only
            #   - default DuckDB or SQL destination → query back into DataFrame
            ...
            all_data = []
            for resource_name in resources_list:
                if not hasattr(source, resource_name):
                    continue
                try:
