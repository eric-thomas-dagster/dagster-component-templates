"""GmailIngestionComponent -- Ingest Gmail messages/labels metadata via the Gmail API using dlt's generic REST API source.

Ingest Gmail data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Gmail has no official
...
class GmailIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Google OAuth2 access token scoped for gmail.readonly. Use ${GMAIL_ACCESS_TOKEN} for env vars.')

    query: Optional[str] = Field(default=None, description="Gmail search query to filter messages (e.g. 'after:2024/01/01'). Only used for the messages resource.")

    resources: str = Field(
        default="messages,labels",
        description="Comma-separated list of resources to extract: messages, labels, threads"
    )
    ...
    group_name: Optional[str] = Field(default="gmail_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Gmail messages/labels metadata via the Gmail API using dlt's generic REST API source."
        ...
        def gmail_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Gmail ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "messages": {"name": "messages", "endpoint": {"path": "messages", "data_selector": "messages", "params": ({"q": self.query} if self.query else {})}},
                "labels": {"name": "labels", "endpoint": {"path": "labels", "data_selector": "labels"}},
                "threads": {"name": "threads", "endpoint": {"path": "threads", "data_selector": "threads"}},
            }
            ...
            context.log.info("Creating Gmail REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Gmail data...")
            load_info = pipeline.run(source)
            context.log.info(f"Gmail data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Gmail.")
        ...
        return Definitions(assets=[gmail_ingestion_asset])
