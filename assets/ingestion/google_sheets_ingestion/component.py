...
Ingest Google Sheets data using dlt's verified `google_spreadsheet` source. Extracts named ranges or full sheets from a spreadsheet.
...
class GoogleSheetsIngestionComponent(Component, Model, Resolvable):
    ...
    Uses a Google service account JSON credentials dict to authenticate.
    ...
    sheet_names: List[str] = Field(description="List of sheet names or A1 ranges to extract from the spreadsheet")
    ...
    group_name: Optional[str] = Field(
        default="google_sheets", description="Asset group for organization"
    )
