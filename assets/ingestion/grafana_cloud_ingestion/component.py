"""GrafanaCloudIngestionComponent -- Ingest Grafana Cloud dashboard and datasource metadata using dlt's generic REST API source.

Ingest Grafana Cloud data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Grafana Cloud has no official
...
class GrafanaCloudIngestionComponent(Component, Model, Resolvable):
    ...
    stack_url: str = Field(description='Your Grafana Cloud stack URL (stack-specific -- no single fixed domain).')

    service_account_token: str = Field(description='Grafana Cloud service account token.')

    resources: str = Field(
        default="dashboards,datasources",
        description="Comma-separated list of resources to extract: dashboards, datasources"
    )
    ...
    group_name: Optional[str] = Field(default="grafana_cloud_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Grafana Cloud dashboard and datasource metadata using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["grafana_cloud", "python"]
        ...
        def grafana_cloud_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Grafana Cloud ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": self.stack_url.rstrip("/") + "/api",
                    "auth": {"type": "bearer", "token": self.service_account_token},
                },
                "resources": [],
            }
            _map = {
                "dashboards": {"name": "dashboards", "endpoint": {"path": "search", "data_selector": "$", "params": {"type": "dash-db"}}},
                "datasources": {"name": "datasources", "endpoint": {"path": "datasources", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Grafana Cloud REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Grafana Cloud data...")
            load_info = pipeline.run(source)
            context.log.info(f"Grafana Cloud data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Grafana Cloud.")
        ...
        return Definitions(assets=[grafana_cloud_ingestion_asset])
