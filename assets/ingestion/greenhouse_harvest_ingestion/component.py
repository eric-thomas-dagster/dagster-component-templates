"""GreenhouseHarvestIngestionComponent -- Ingest Greenhouse recruiting data using dlt's generic REST API source (Harvest API).

Ingest Greenhouse data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Greenhouse has no official
...
class GreenhouseHarvestIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='Greenhouse Harvest API key (HTTP Basic username, blank password). Use ${GREENHOUSE_API_KEY} for env vars.')

    resources: str = Field(
        default="applications,candidates,jobs,job_posts,offers",
        description="Comma-separated list of resources to extract: applications, candidates, jobs, job_posts, offers"
    )
    ...
    group_name: Optional[str] = Field(default="greenhouse_harvest_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Greenhouse recruiting data using dlt's generic REST API source (Harvest API)."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["greenhouse", "python"]
        ...
        def greenhouse_harvest_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Greenhouse ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            context.log.info("Creating Greenhouse REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Greenhouse data...")
            load_info = pipeline.run(source)
            context.log.info(f"Greenhouse data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Greenhouse.")
        ...
        return Definitions(assets=[greenhouse_harvest_ingestion_asset])
