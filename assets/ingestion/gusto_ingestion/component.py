"""GustoIngestionComponent -- Ingest Gusto HR/payroll data using dlt's generic REST API source.

Ingest Gusto data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Gusto has no official
...
class GustoIngestionComponent(Component, Model, Resolvable):
    ...
    company_id: str = Field(description='Gusto company UUID.')

    access_token: str = Field(description='Gusto OAuth2 access token. Use ${GUSTO_ACCESS_TOKEN} for env vars.')

    resources: str = Field(
        default="employees,payrolls,time_off_requests",
        description="Comma-separated list of resources to extract: employees, payrolls, time_off_requests"
    )
    ...
    group_name: Optional[str] = Field(default="gusto_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Gusto HR/payroll data using dlt's generic REST API source."
        ...
        def gusto_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Gusto ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            context.log.info("Creating Gusto REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Gusto data...")
            load_info = pipeline.run(source)
            context.log.info(f"Gusto data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Gusto.")
        ...
        return Definitions(assets=[gusto_ingestion_asset])
