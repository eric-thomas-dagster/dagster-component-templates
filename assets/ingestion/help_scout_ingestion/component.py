"""HelpScoutIngestionComponent -- Ingest Help Scout support data using dlt's generic REST API source.

Ingest Help Scout data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Help Scout has no official
...
class HelpScoutIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Help Scout OAuth2 access token. Use ${HELPSCOUT_ACCESS_TOKEN} for env vars.')

    resources: str = Field(
        default="conversations,customers,mailboxes,users",
        description="Comma-separated list of resources to extract: conversations, customers, mailboxes, users, tags"
    )
    ...
    group_name: Optional[str] = Field(default="help_scout_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Help Scout support data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["helpscout", "python"]
        ...
        def help_scout_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Help Scout ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": r, "data_selector": f"_embedded.{r}"}})
            ...
            context.log.info("Creating Help Scout REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Help Scout data...")
            load_info = pipeline.run(source)
            context.log.info(f"Help Scout data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Help Scout.")
        ...
        return Definitions(assets=[help_scout_ingestion_asset])
