"""HoneycombIngestionComponent -- Ingest Honeycomb dataset metadata using dlt's generic REST API source.

Ingest Honeycomb data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Honeycomb has no official
...
class HoneycombIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='Honeycomb API key.')

    eu_instance: Optional[bool] = Field(default=None, description='Set true if your Honeycomb environment is hosted in the EU (api.eu1.honeycomb.io).')

    resources: str = Field(
        default="datasets",
        description="Comma-separated list of resources to extract: datasets"
    )
    ...
    group_name: Optional[str] = Field(default="honeycomb_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Honeycomb dataset metadata using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["honeycomb", "python"]
        ...
        def honeycomb_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Honeycomb ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            _base = "https://api.eu1.honeycomb.io" if self.eu_instance else "https://api.honeycomb.io"
            config = {
                "client": {
                    "base_url": _base,
                    "auth": {"type": "api_key", "api_key": self.api_key, "name": "X-Honeycomb-Team", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "datasets": {"name": "datasets", "endpoint": {"path": "1/datasets", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Honeycomb REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Honeycomb data...")
            load_info = pipeline.run(source)
            context.log.info(f"Honeycomb data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Honeycomb.")
        ...
        return Definitions(assets=[honeycomb_ingestion_asset])
