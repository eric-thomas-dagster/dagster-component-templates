...
Ingest HubSpot CRM data (contacts, companies, deals, tickets, products, quotes, events) using dlt's verified `hubspot` source.
...
class HubSpotIngestionComponent(Component, Model, Resolvable):
    ...
    Available resources: `contacts`, `companies`, `deals`, `tickets`, `products`,
    `quotes`, `hubspot_events_for_objects`. Optionally include property change
    ...
    include_history: bool = Field(default=False, description="Include property change history for CRM objects")

    include_custom_props: bool = Field(default=True, description="Extract custom CRM object properties")
    ...
    group_name: Optional[str] = Field(
        default="hubspot", description="Asset group for organization"
    )
        ...
        def hubspot_ingestion_asset(context: AssetExecutionContext):
            ...
            for resource_name in resources_list:
                if hasattr(source, resource_name):
                    ...
                    context.log.warning(
                        f"Resource '{resource_name}' not found in HubSpot source; skipping."
                    )
            if not selected_resources:
                raise ValueError("No valid resources selected for HubSpot source.")
