"""InsightlyIngestionComponent -- Ingest Insightly CRM data using dlt's generic REST API source.

Ingest Insightly data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Insightly has no official
...
class InsightlyIngestionComponent(Component, Model, Resolvable):
    ...
    pod: str = Field(description="Insightly pod identifier (from your Insightly URL, e.g. 'na1').")

    api_key: str = Field(description='Insightly API key (HTTP Basic username, blank password). Use ${INSIGHTLY_API_KEY} for env vars.')

    resources: str = Field(
        default="contacts,leads,organisations,opportunities,projects",
        description="Comma-separated list of resources to extract: contacts, leads, organisations, opportunities, projects"
    )
    ...
    group_name: Optional[str] = Field(default="insightly_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Insightly CRM data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["insightly", "python"]
        ...
        def insightly_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Insightly ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://api.{self.pod}.insightly.com/v3.1",
                    "auth": {"type": "http_basic", "username": self.api_key, "password": ""},
                },
                "resources": [],
            }
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": r}})
            ...
            context.log.info("Creating Insightly REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Insightly data...")
            load_info = pipeline.run(source)
            context.log.info(f"Insightly data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Insightly.")
        ...
        return Definitions(assets=[insightly_ingestion_asset])
