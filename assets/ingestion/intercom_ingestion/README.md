...
| `access_token` | one of access_token OR resource_key | Intercom access token (from a private app). Use ${INTERCOM_ACCESS_TOKEN} for env vars. |
| `resource_key` | no | Resource key registered by an IntercomResourceComponent. When set, access_token is read from that resource at run time -- lets one Intercom credential serve both this connector and the intercom_contact_upsert reverse-ETL sink without configuring it twice. |
...
|---|---|---|---|
| `resource_key` | `str` | — | Optional resource key registered by an IntercomResourceComponent. When set, access_token is read from that resource at run time instead of access_token above -- lets one Intercom credential serve both this ingestion conn… _(full docs in schema.json + component README)_ |
...
| `group_name` | `str` | `"intercom_ingestion"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `access_token` | `str` | — | Intercom access token (from a private app). Use ${INTERCOM_ACCESS_TOKEN} for env vars. Required unless resource_key is set. |
| `resources` | `str` | `"contacts,conversations,companies,tags"` | Comma-separated list of resources to extract: contacts, conversations, companies, tags |
...
## Example — sharing credentials with an IntercomResourceComponent

```yaml
type: dagster_component_templates.IntercomIngestionComponent
attributes:
  asset_name: intercom_ingestion
  resource_key: intercom   # matches IntercomResourceComponent's default resource_key
  resources: "contacts,conversations,companies,tags"
```
