"""IntercomIngestionComponent -- Ingest Intercom support data using dlt's generic REST API source.

Ingest Intercom data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Intercom has no official
...
class IntercomIngestionComponent(Component, Model, Resolvable):
    ...
    resources: str = Field(
        ...
        description="Comma-separated list of resources to extract: contacts, conversations, companies, tags"
    )
    ...
    group_name: Optional[str] = Field(default="intercom_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["intercom", "python"]
        ...
        def intercom_ingestion_asset(context: AssetExecutionContext):
            ...
            _map = {
                ...
                "conversations": {"name": "conversations", "endpoint": {"path": "conversations", "data_selector": "conversations"}},
                "companies": {"name": "companies", "endpoint": {"path": "companies", "data_selector": "companies"}},
            ...
            context.log.info("Creating Intercom REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Intercom data...")
            load_info = pipeline.run(source)
            context.log.info(f"Intercom data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Intercom.")
        ...
        return Definitions(assets=[intercom_ingestion_asset])
