"""JiraServiceManagementIngestionComponent -- Ingest Jira Service Management service desk, request, and organization data using dlt's generic REST API source. Distinct product surface from core Jira.

Ingest Jira Service Management data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Jira Service Management has no official
...
class JiraServiceManagementIngestionComponent(Component, Model, Resolvable):
    ...
    site_domain: str = Field(description='Your Atlassian site subdomain (for mysite.atlassian.net).')

    email: str = Field(description='Atlassian account email for HTTP Basic auth.')

    api_token: str = Field(description='Atlassian API token, used as the Basic auth password.')

    resources: str = Field(
        default="servicedesks,requests",
        description="Comma-separated list of resources to extract: servicedesks, requests, organizations"
    )
    ...
    group_name: Optional[str] = Field(default="jira_service_management_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Jira Service Management service desk, request, and organization data using dlt's generic REST API source. Distinct product surface from core Jira."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["jira_service_management", "python"]
        ...
        def jira_service_management_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Jira Service Management ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://{self.site_domain}.atlassian.net/rest/servicedeskapi",
                    "auth": {"type": "http_basic", "username": self.email, "password": self.api_token},
                    "headers": {"Accept": "application/json"},
                },
                "resources": [],
            }
            _map = {
                "servicedesks": {"name": "servicedesks", "endpoint": {"path": "servicedesk", "data_selector": "values"}},
                "requests": {"name": "requests", "endpoint": {"path": "request", "data_selector": "values"}},
                "organizations": {"name": "organizations", "endpoint": {"path": "organization", "data_selector": "values"}},
            }
            ...
            context.log.info("Creating Jira Service Management REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Jira Service Management data...")
            load_info = pipeline.run(source)
            context.log.info(f"Jira Service Management data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Jira Service Management.")
        ...
        return Definitions(assets=[jira_service_management_ingestion_asset])
