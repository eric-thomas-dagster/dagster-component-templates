...
| `api_key` | one of api_key OR resource_key | Klaviyo private API key. Use ${KLAVIYO_API_KEY} for env vars. |
| `resource_key` | no | Resource key registered by a KlaviyoResourceComponent (defaults its own resource_key to `klaviyo`, not `klaviyo_resource`). When set, api_key is read from that resource at run time -- lets one Klaviyo credential serve both this connector and the dataframe_to_klaviyo reverse-ETL sink without configuring it twice. |
...
|---|---|---|---|
| `api_key` | `str` | — | Klaviyo private API key. Use ${KLAVIYO_API_KEY} for env vars. Required unless resource_key is set. |
| `resource_key` | `str` | — | Optional resource key registered by a KlaviyoResourceComponent (that component defaults its own resource_key to 'klaviyo', not 'klaviyo_resource'). When set, api_key is read from that resource at run time instead of api_… _(full docs in schema.json + component README)_ |
...
| `group_name` | `str` | `"klaviyo_ingestion"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `resources` | `str` | `"lists,campaigns,flows,profiles,metrics"` | Comma-separated list of resources to extract: lists, campaigns, flows, profiles, metrics |
...
## Example — sharing credentials with a KlaviyoResourceComponent

```yaml
type: dagster_component_templates.KlaviyoIngestionComponent
attributes:
  asset_name: klaviyo_ingestion
  resource_key: klaviyo   # matches KlaviyoResourceComponent's default resource_key
  resources: "lists,campaigns,flows,profiles,metrics"
```
