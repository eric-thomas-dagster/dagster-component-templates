"""KlaviyoIngestionComponent -- Ingest Klaviyo marketing data using dlt's generic REST API source.

Ingest Klaviyo data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Klaviyo has no official
...
class KlaviyoIngestionComponent(Component, Model, Resolvable):
    ...
    resources: str = Field(
        ...
        description="Comma-separated list of resources to extract: lists, campaigns, flows, profiles, metrics"
    )
    ...
    group_name: Optional[str] = Field(default="klaviyo_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["klaviyo", "python"]
        ...
        def klaviyo_ingestion_asset(context: AssetExecutionContext):
            ...
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": f"api/{r}", "data_selector": "data"}})
            ...
            context.log.info("Creating Klaviyo REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Klaviyo data...")
            load_info = pipeline.run(source)
            context.log.info(f"Klaviyo data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Klaviyo.")
        ...
        return Definitions(assets=[klaviyo_ingestion_asset])
