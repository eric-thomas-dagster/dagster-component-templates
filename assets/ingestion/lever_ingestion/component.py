"""LeverIngestionComponent -- Ingest Lever recruiting data using dlt's generic REST API source.

Ingest Lever data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Lever has no official
...
class LeverIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='Lever API key (HTTP Basic username, blank password). Use ${LEVER_API_KEY} for env vars.')

    resources: str = Field(
        default="opportunities,postings,users,stages",
        description="Comma-separated list of resources to extract: opportunities, postings, users, stages, archive_reasons"
    )
    ...
    group_name: Optional[str] = Field(default="lever_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Lever recruiting data using dlt's generic REST API source."
        ...
        def lever_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Lever ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            context.log.info("Creating Lever REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Lever data...")
            load_info = pipeline.run(source)
            context.log.info(f"Lever data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Lever.")
        ...
        return Definitions(assets=[lever_ingestion_asset])
