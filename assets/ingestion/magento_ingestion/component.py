"""MagentoIngestionComponent -- Ingest Magento/Adobe Commerce product, order, and customer data using dlt's generic REST API source.

Ingest Magento / Adobe Commerce data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Magento / Adobe Commerce has no official
...
class MagentoIngestionComponent(Component, Model, Resolvable):
    ...
    instance_url: str = Field(description='Your Magento/Adobe Commerce instance base URL (self-hosted -- no fixed SaaS domain).')

    admin_token: str = Field(description='Pre-generated Magento admin/integration access token (Admin > System > Integrations, or POST /V1/integration/admin/token).')

    resources: str = Field(
        default="products,orders",
        description="Comma-separated list of resources to extract: products, orders, customers"
    )
    ...
    group_name: Optional[str] = Field(default="magento_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Magento/Adobe Commerce product, order, and customer data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["magento", "python"]
        ...
        def magento_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Magento / Adobe Commerce ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": self.instance_url.rstrip("/") + "/rest/V1",
                    "auth": {"type": "bearer", "token": self.admin_token},
                },
                "resources": [],
            }
            _search_params = {"searchCriteria[pageSize]": 100}
            _map = {
                "products": {"name": "products", "endpoint": {"path": "products", "data_selector": "items", "params": _search_params}},
                "orders": {"name": "orders", "endpoint": {"path": "orders", "data_selector": "items", "params": _search_params}},
                "customers": {"name": "customers", "endpoint": {"path": "customers/search", "data_selector": "items", "params": _search_params}},
            }
            ...
            context.log.info("Creating Magento / Adobe Commerce REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Magento / Adobe Commerce data...")
            load_info = pipeline.run(source)
            context.log.info(f"Magento / Adobe Commerce data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Magento / Adobe Commerce.")
        ...
        return Definitions(assets=[magento_ingestion_asset])
