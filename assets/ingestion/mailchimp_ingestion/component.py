"""MailchimpIngestionComponent -- Ingest Mailchimp marketing data using dlt's generic REST API source.

Ingest Mailchimp data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Mailchimp has no official
...
class MailchimpIngestionComponent(Component, Model, Resolvable):
    ...
    resources: str = Field(
        ...
        description="Comma-separated list of resources to extract: lists, campaigns, automations, reports"
    )
    ...
    group_name: Optional[str] = Field(default="mailchimp_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["mailchimp", "python"]
        ...
        def mailchimp_ingestion_asset(context: AssetExecutionContext):
            ...
            _map = {
                "lists": {"name": "lists", "endpoint": {"path": "lists", "data_selector": "lists"}},
                "campaigns": {"name": "campaigns", "endpoint": {"path": "campaigns", "data_selector": "campaigns"}},
                "automations": {"name": "automations", "endpoint": {"path": "automations", "data_selector": "automations"}},
                "reports": {"name": "reports", "endpoint": {"path": "reports", "data_selector": "reports"}},
            }
            ...
            context.log.info("Creating Mailchimp REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Mailchimp data...")
            load_info = pipeline.run(source)
            context.log.info(f"Mailchimp data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Mailchimp.")
        ...
        return Definitions(assets=[mailchimp_ingestion_asset])
