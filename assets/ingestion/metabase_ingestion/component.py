"""MetabaseIngestionComponent -- Ingest Metabase database, card (question), and collection metadata using dlt's generic REST API source.

Ingest Metabase data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Metabase has no official
...
class MetabaseIngestionComponent(Component, Model, Resolvable):
    ...
    instance_url: str = Field(description='Base URL of your Metabase instance (self-hosted or Cloud).')

    api_key: str = Field(description='Metabase API key (newer versions; Settings > Admin > API Keys).')

    resources: str = Field(
        default="databases,cards,collections",
        description="Comma-separated list of resources to extract: databases, cards, collections"
    )
    ...
    group_name: Optional[str] = Field(default="metabase_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Metabase database, card (question), and collection metadata using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["metabase", "python"]
        ...
        def metabase_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Metabase ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": self.instance_url.rstrip("/") + "/api",
                    "auth": {"type": "api_key", "api_key": self.api_key, "name": "x-api-key", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "databases": {"name": "databases", "endpoint": {"path": "database", "data_selector": "data"}},
                "cards": {"name": "cards", "endpoint": {"path": "card", "data_selector": "$"}},
                "collections": {"name": "collections", "endpoint": {"path": "collection", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Metabase REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Metabase data...")
            load_info = pipeline.run(source)
            context.log.info(f"Metabase data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Metabase.")
        ...
        return Definitions(assets=[metabase_ingestion_asset])
