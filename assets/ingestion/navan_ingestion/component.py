"""NavanIngestionComponent -- Ingest Navan (travel & expense) data using dlt's generic REST API source.

Ingest Navan data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Navan has no official
...
class NavanIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Navan OAuth2 access token (client-credentials grant). Use ${NAVAN_ACCESS_TOKEN} for env vars.')

    resources: str = Field(
        default="trips,expenses",
        description="Comma-separated list of resources to extract: trips, expenses, users"
    )
    ...
    group_name: Optional[str] = Field(default="navan_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Navan (travel & expense) data using dlt's generic REST API source."
        ...
        def navan_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Navan ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.navan.com/v1",
            ...
            context.log.info("Creating Navan REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Navan data...")
            load_info = pipeline.run(source)
            context.log.info(f"Navan data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Navan.")
        ...
        return Definitions(assets=[navan_ingestion_asset])
