"""NetSuiteIngestionComponent -- Ingest Oracle NetSuite (SuiteTalk REST) data using dlt's generic REST API source.

Ingest NetSuite data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). NetSuite has no official
...
class NetSuiteIngestionComponent(Component, Model, Resolvable):
    ...
    account_id: str = Field(description='NetSuite account ID (used in the tenant-specific SuiteTalk hostname).')

    consumer_key: str = Field(description='OAuth1.0a consumer key from a NetSuite Integration record.')

    consumer_secret: str = Field(description='OAuth1.0a consumer secret.')

    token_id: str = Field(description='OAuth1.0a access token ID.')

    token_secret: str = Field(description='OAuth1.0a access token secret.')

    resources: str = Field(
        default="customer,invoice",
        description="Comma-separated list of resources to extract: customer, salesorder, invoice, vendor"
    )
    ...
    group_name: Optional[str] = Field(default="netsuite_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Oracle NetSuite (SuiteTalk REST) data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["netsuite", "python"]
        ...
        def netsuite_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting NetSuite ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            try:
                ...
                raise ImportError(
                    "netsuite_ingestion requires the 'requests-oauthlib' package for OAuth1.0a "
                    "signing. Install it (see requirements.txt)."
                ) from _e
            ...
            config = {
                "client": {
                    "base_url": f"https://{self.account_id}.suitetalk.api.netsuite.com/services/rest",
                    "auth": _auth,
                },
                "resources": [],
            }
            _map = {
                "customer": {"name": "customer", "endpoint": {"path": "record/v1/customer", "data_selector": "items"}},
                "salesorder": {"name": "salesorder", "endpoint": {"path": "record/v1/salesOrder", "data_selector": "items"}},
                "invoice": {"name": "invoice", "endpoint": {"path": "record/v1/invoice", "data_selector": "items"}},
                "vendor": {"name": "vendor", "endpoint": {"path": "record/v1/vendor", "data_selector": "items"}},
            }
            ...
            context.log.info("Creating NetSuite REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting NetSuite data...")
            load_info = pipeline.run(source)
            context.log.info(f"NetSuite data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from NetSuite.")
        ...
        return Definitions(assets=[netsuite_ingestion_asset])
