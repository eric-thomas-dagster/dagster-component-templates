"""OktaManagementIngestionComponent -- Ingest Okta org data (users, groups, apps) via the Management API using dlt's generic REST API source. Distinct from okta_system_log_ingestion, which covers audit/system logs only.

Ingest Okta (Management API) data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Okta (Management API) has no official
...
class OktaManagementIngestionComponent(Component, Model, Resolvable):
    ...
    org_url: str = Field(description='Your Okta org URL.')

    api_token: str = Field(description='Okta API token (SSWS), created under Security > API > Tokens.')

    resources: str = Field(
        default="users,groups",
        description="Comma-separated list of resources to extract: users, groups, apps"
    )
    ...
    group_name: Optional[str] = Field(default="okta_management_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Okta org data (users, groups, apps) via the Management API using dlt's generic REST API source. Distinct from okta_system_log_ingestion, which covers audit/system logs only."
        ...
        def okta_management_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Okta (Management API) ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": self.org_url.rstrip("/"),
                    "auth": {"type": "api_key", "api_key": f"SSWS {self.api_token}", "name": "Authorization", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "users": {"name": "users", "endpoint": {"path": "api/v1/users", "data_selector": "$"}},
                "groups": {"name": "groups", "endpoint": {"path": "api/v1/groups", "data_selector": "$"}},
                "apps": {"name": "apps", "endpoint": {"path": "api/v1/apps", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Okta (Management API) REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Okta (Management API) data...")
            load_info = pipeline.run(source)
            context.log.info(f"Okta (Management API) data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Okta (Management API).")
        ...
        return Definitions(assets=[okta_management_ingestion_asset])
