"""OmniIngestionComponent -- Ingest Omni BI account metadata (dashboards, models, queries) using dlt's generic REST API source.

Ingest Omni data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Omni has no official
...
class OmniIngestionComponent(Component, Model, Resolvable):
    ...
    organization: str = Field(description='Omni organization subdomain (from https://{organization}.omniapp.co).')

    api_key: str = Field(description='Omni API key. Use ${OMNI_API_KEY} for env vars.')

    resources: str = Field(
        default="dashboards,models",
        description="Comma-separated list of resources to extract: dashboards, models, queries"
    )
    ...
    group_name: Optional[str] = Field(default="omni_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Omni BI account metadata (dashboards, models, queries) using dlt's generic REST API source."
        ...
        def omni_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Omni ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://{self.organization}.omniapp.co/api/v1",
                    "auth": {"type": "bearer", "token": self.api_key},
                },
                "resources": [],
            }
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": r, "data_selector": "data"}})
            ...
            context.log.info("Creating Omni REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Omni data...")
            load_info = pipeline.run(source)
            context.log.info(f"Omni data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Omni.")
        ...
        return Definitions(assets=[omni_ingestion_asset])
