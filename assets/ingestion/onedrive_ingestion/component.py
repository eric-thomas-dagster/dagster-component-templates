"""OneDriveIngestionComponent -- Ingest OneDrive/SharePoint file listings via Microsoft Graph (app-only auth) using dlt's generic REST API source.

Ingest OneDrive / SharePoint (Microsoft Graph) data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). OneDrive / SharePoint (Microsoft Graph) has no official
...
class OneDriveIngestionComponent(Component, Model, Resolvable):
    ...
    client_id: str = Field(description='Azure AD app registration client ID (granted Files.Read.All or Sites.Read.All application permission, admin-consented).')

    client_secret: str = Field(description='Azure AD app registration client secret.')

    drive_id: str = Field(description="Target drive ID (a user's OneDrive or a SharePoint document library's drive ID).")

    resources: str = Field(
        default="files",
        description="Comma-separated list of resources to extract: files (root children of the configured drive)"
    )
    ...
    group_name: Optional[str] = Field(default="onedrive_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest OneDrive/SharePoint file listings via Microsoft Graph (app-only auth) using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["onedrive", "python"]
        ...
        def onedrive_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting OneDrive / SharePoint (Microsoft Graph) ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "files": {"name": "files", "endpoint": {"path": f"drives/{self.drive_id}/root/children", "data_selector": "value"}},
            }
            ...
            context.log.info("Creating OneDrive / SharePoint (Microsoft Graph) REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting OneDrive / SharePoint (Microsoft Graph) data...")
            load_info = pipeline.run(source)
            context.log.info(f"OneDrive / SharePoint (Microsoft Graph) data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from OneDrive / SharePoint (Microsoft Graph).")
        ...
        return Definitions(assets=[onedrive_ingestion_asset])
