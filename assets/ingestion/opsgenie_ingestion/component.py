"""OpsgenieIngestionComponent -- Ingest Opsgenie alerting/on-call data using dlt's generic REST API source.

Ingest Opsgenie data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Opsgenie has no official
...
class OpsgenieIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='Opsgenie API key. Use ${OPSGENIE_API_KEY} for env vars.')

    resources: str = Field(
        default="alerts,schedules,teams,users",
        description="Comma-separated list of resources to extract: alerts, schedules, teams, users"
    )
    ...
    group_name: Optional[str] = Field(default="opsgenie_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Opsgenie alerting/on-call data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["opsgenie", "python"]
        ...
        def opsgenie_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Opsgenie ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.opsgenie.com",
                    "auth": {"type": "api_key", "api_key": f"GenieKey {self.api_key}", "name": "Authorization", "location": "header"},
                },
                "resources": [],
            }
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": f"v2/{r}", "data_selector": "data"}})
            ...
            context.log.info("Creating Opsgenie REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Opsgenie data...")
            load_info = pipeline.run(source)
            context.log.info(f"Opsgenie data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Opsgenie.")
        ...
        return Definitions(assets=[opsgenie_ingestion_asset])
