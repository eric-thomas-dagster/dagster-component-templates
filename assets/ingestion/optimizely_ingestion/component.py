"""OptimizelyIngestionComponent -- Ingest Optimizely Feature/Web Experimentation data (projects, experiments, audiences) using dlt's generic REST API source.

Ingest Optimizely (Feature Experimentation) data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Optimizely (Feature Experimentation) has no official
...
class OptimizelyIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Optimizely Personal Access Token.')

    project_id: Optional[str] = Field(default=None, description='Project ID. Required for experiments/audiences resources; not needed for projects.')

    resources: str = Field(
        default="projects",
        description="Comma-separated list of resources to extract: projects (account-level); experiments, audiences (require project_id)"
    )
    ...
    group_name: Optional[str] = Field(default="optimizely_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Optimizely Feature/Web Experimentation data (projects, experiments, audiences) using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["optimizely", "python"]
        ...
        def optimizely_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Optimizely (Feature Experimentation) ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "projects": {"name": "projects", "endpoint": {"path": "projects", "data_selector": "$"}},
            }
            if self.project_id:
                _map.update({
                    "experiments": {"name": "experiments", "endpoint": {"path": "experiments", "data_selector": "$", "params": {"project_id": self.project_id}}},
                    "audiences": {"name": "audiences", "endpoint": {"path": "audiences", "data_selector": "$", "params": {"project_id": self.project_id}}},
                })
            for r in resources_list:
                if r in _map:
                    config["resources"].append(_map[r])
                else:
                    context.log.warning(f"Resource {r!r} requires project_id to be set; skipping.")
            ...
            context.log.info("Creating Optimizely (Feature Experimentation) REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Optimizely (Feature Experimentation) data...")
            load_info = pipeline.run(source)
            context.log.info(f"Optimizely (Feature Experimentation) data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Optimizely (Feature Experimentation).")
        ...
        return Definitions(assets=[optimizely_ingestion_asset])
