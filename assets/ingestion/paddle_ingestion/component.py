"""PaddleIngestionComponent -- Ingest Paddle (merchant-of-record billing) customer, subscription, and transaction data using dlt's generic REST API source.

Ingest Paddle data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Paddle has no official
...
class PaddleIngestionComponent(Component, Model, Resolvable):
    ...
    sandbox: Optional[bool] = Field(default=None, description="Set true to use Paddle's sandbox environment.")

    resources: str = Field(
        default="customers,subscriptions",
        description="Comma-separated list of resources to extract: customers, subscriptions, transactions"
    )
    ...
    group_name: Optional[str] = Field(default="paddle_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Paddle (merchant-of-record billing) customer, subscription, and transaction data using dlt's generic REST API source."
        ...
        def paddle_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Paddle ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            _base = "https://sandbox-api.paddle.com" if self.sandbox else "https://api.paddle.com"
            config = {
                "client": {
                    "base_url": _base,
            ...
            _map = {
                ...
                "transactions": {"name": "transactions", "endpoint": {"path": "transactions", "data_selector": "data"}},
            }
            ...
            context.log.info("Creating Paddle REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Paddle data...")
            load_info = pipeline.run(source)
            context.log.info(f"Paddle data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Paddle.")
        ...
        return Definitions(assets=[paddle_ingestion_asset])
