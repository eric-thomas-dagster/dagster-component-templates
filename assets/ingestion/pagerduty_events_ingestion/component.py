"""PagerDutyIngestionComponent -- Ingest PagerDuty incidents/services/schedules using dlt's generic REST API source.

Ingest PagerDuty data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). PagerDuty has no official
...
class PagerDutyIngestionComponent(Component, Model, Resolvable):
    ...
    api_token: str = Field(description='PagerDuty REST API token. Use ${PAGERDUTY_API_TOKEN} for env vars.')

    resources: str = Field(
        default="incidents,services,schedules",
        description="Comma-separated list of resources to extract: incidents, services, schedules, escalation_policies"
    )
    ...
    group_name: Optional[str] = Field(default="pagerduty_events_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest PagerDuty incidents/services/schedules using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["pagerduty", "python"]
        ...
        def pagerduty_events_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting PagerDuty ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.pagerduty.com",
                    "auth": {"type": "api_key", "api_key": f"Token token={self.api_token}", "name": "Authorization", "location": "header"},
            ...
            if context.has_partition_key:
                try:
                    ...
                    _incident_params["since"] = _window.start.isoformat() + "Z"
                    _incident_params["until"] = _window.end.isoformat() + "Z"
                except Exception:
                    pass
            for r in resources_list:
                _params = _incident_params if r == "incidents" else {}
                config["resources"].append({"name": r, "endpoint": {"path": r, "data_selector": r, "params": _params}})
            ...
            context.log.info("Creating PagerDuty REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting PagerDuty data...")
            load_info = pipeline.run(source)
            context.log.info(f"PagerDuty data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from PagerDuty.")
        ...
        return Definitions(assets=[pagerduty_events_ingestion_asset])
