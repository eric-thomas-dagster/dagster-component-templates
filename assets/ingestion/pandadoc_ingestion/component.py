"""PandaDocIngestionComponent -- Ingest PandaDoc document, template, and folder data using dlt's generic REST API source.

Ingest PandaDoc data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). PandaDoc has no official
...
class PandaDocIngestionComponent(Component, Model, Resolvable):
    ...
    resources: str = Field(
        ...
        description="Comma-separated list of resources to extract: documents, templates, folders"
    )
    ...
    group_name: Optional[str] = Field(default="pandadoc_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest PandaDoc document, template, and folder data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["pandadoc", "python"]
        ...
        def pandadoc_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting PandaDoc ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.pandadoc.com/public/v1",
                    "auth": {"type": "api_key", "api_key": f"API-Key {self.api_key}", "name": "Authorization", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "documents": {"name": "documents", "endpoint": {"path": "documents", "data_selector": "results"}},
                "templates": {"name": "templates", "endpoint": {"path": "templates", "data_selector": "results"}},
                "folders": {"name": "folders", "endpoint": {"path": "documents/folders", "data_selector": "results"}},
            }
            ...
            context.log.info("Creating PandaDoc REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting PandaDoc data...")
            load_info = pipeline.run(source)
            context.log.info(f"PandaDoc data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from PandaDoc.")
        ...
        return Definitions(assets=[pandadoc_ingestion_asset])
