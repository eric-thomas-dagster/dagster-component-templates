"""PaylocityIngestionComponent -- Ingest Paylocity payroll/HR data using dlt's generic REST API source.

Ingest Paylocity data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Paylocity has no official
...
class PaylocityIngestionComponent(Component, Model, Resolvable):
    ...
    company_id: str = Field(description='Paylocity company ID.')

    access_token: str = Field(description='Paylocity OAuth2 access token (client-credentials grant). Use ${PAYLOCITY_ACCESS_TOKEN} for env vars.')

    resources: str = Field(
        default="employees",
        description="Comma-separated list of resources to extract: employees, earnings, deductions"
    )
    ...
    group_name: Optional[str] = Field(default="paylocity_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Paylocity payroll/HR data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["paylocity", "python"]
        ...
        def paylocity_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Paylocity ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": f"companies/{self.company_id}/{r}"}})
            ...
            context.log.info("Creating Paylocity REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Paylocity data...")
            load_info = pipeline.run(source)
            context.log.info(f"Paylocity data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Paylocity.")
        ...
        return Definitions(assets=[paylocity_ingestion_asset])
