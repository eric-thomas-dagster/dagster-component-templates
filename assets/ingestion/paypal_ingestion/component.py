"""PayPalIngestionComponent -- Ingest PayPal transaction and invoice data (REST API, not Braintree) using dlt's generic REST API source.

Ingest PayPal data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). PayPal has no official
...
class PayPalIngestionComponent(Component, Model, Resolvable):
    ...
    resources: str = Field(
        ...
        description="Comma-separated list of resources to extract: transactions (requires a bounded date range, max ~31 days), invoices"
    )
    ...
    group_name: Optional[str] = Field(default="paypal_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest PayPal transaction and invoice data (REST API, not Braintree) using dlt's generic REST API source."
        ...
        def paypal_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting PayPal ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            if context.has_partition_key:
                try:
                    ...
                    _tx_params["start_date"] = _window.start.isoformat() + "Z"
                    _tx_params["end_date"] = _window.end.isoformat() + "Z"
                except Exception:
                    pass
            ...
            _map = {
                "transactions": {"name": "transactions", "endpoint": {"path": "v1/reporting/transactions", "data_selector": "transaction_details", "params": _tx_params}},
                "invoices": {"name": "invoices", "endpoint": {"path": "v2/invoicing/invoices", "data_selector": "items"}},
            }
            ...
            context.log.info("Creating PayPal REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting PayPal data...")
            load_info = pipeline.run(source)
            context.log.info(f"PayPal data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from PayPal.")
        ...
        return Definitions(assets=[paypal_ingestion_asset])
