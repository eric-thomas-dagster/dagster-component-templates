"""PendoIngestionComponent -- Ingest Pendo product analytics metadata (features, guides, accounts) using dlt's generic REST API source.

Ingest Pendo data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Pendo has no official
...
class PendoIngestionComponent(Component, Model, Resolvable):
    ...
    integration_key: str = Field(description='Pendo integration key.')

    eu_tenant: Optional[bool] = Field(default=None, description='Set true if your Pendo tenant is hosted in the EU (app.eu.pendo.io).')

    resources: str = Field(
        default="features,guides",
        description="Comma-separated list of resources to extract: features, guides, accounts"
    )
    ...
    group_name: Optional[str] = Field(default="pendo_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Pendo product analytics metadata (features, guides, accounts) using dlt's generic REST API source."
        ...
        def pendo_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Pendo ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            _base = "https://app.eu.pendo.io/api/v1" if self.eu_tenant else "https://app.pendo.io/api/v1"
            config = {
                "client": {
                    "base_url": _base,
                    "auth": {"type": "api_key", "api_key": self.integration_key, "name": "x-pendo-integration-key", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "features": {"name": "features", "endpoint": {"path": "feature", "data_selector": "$"}},
                "guides": {"name": "guides", "endpoint": {"path": "guide", "data_selector": "$"}},
                "accounts": {"name": "accounts", "endpoint": {"path": "account", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating Pendo REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Pendo data...")
            load_info = pipeline.run(source)
            context.log.info(f"Pendo data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Pendo.")
        ...
        return Definitions(assets=[pendo_ingestion_asset])
