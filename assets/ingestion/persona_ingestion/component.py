"""PersonaIngestionComponent -- Ingest Persona identity-verification inquiry, account, and case data using dlt's generic REST API source.

Ingest Persona data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Persona has no official
...
class PersonaIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='Persona API key.')

    resources: str = Field(
        default="inquiries,accounts",
        description="Comma-separated list of resources to extract: inquiries, accounts, cases"
    )
    ...
    group_name: Optional[str] = Field(default="persona_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Persona identity-verification inquiry, account, and case data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["persona", "python"]
        ...
        def persona_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Persona ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "inquiries": {"name": "inquiries", "endpoint": {"path": "inquiries", "data_selector": "data"}},
                "accounts": {"name": "accounts", "endpoint": {"path": "accounts", "data_selector": "data"}},
                "cases": {"name": "cases", "endpoint": {"path": "cases", "data_selector": "data"}},
            }
            ...
            context.log.info("Creating Persona REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Persona data...")
            load_info = pipeline.run(source)
            context.log.info(f"Persona data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Persona.")
        ...
        return Definitions(assets=[persona_ingestion_asset])
