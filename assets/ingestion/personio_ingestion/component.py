...
Ingest Personio HR data (employees, absences, attendances, recruiting) using dlt's verified `personio` source.
...
class PersonioIngestionComponent(Component, Model, Resolvable):
    """Component for ingesting Personio HR data using dlt.

    Available resources: `employees`, `absences`, `attendances`, `recruiting`.
    Authenticates via Personio API client credentials.
    ...
    group_name: Optional[str] = Field(
        default="personio", description="Asset group for organization"
    )
        ...
        def personio_ingestion_asset(context: AssetExecutionContext):
            ...
            for resource_name in resources_list:
                if hasattr(source, resource_name):
                    ...
                    context.log.warning(
                        f"Resource '{resource_name}' not found in Personio source; skipping."
                    )
            if not selected_resources:
                raise ValueError("No valid resources selected for Personio source.")
