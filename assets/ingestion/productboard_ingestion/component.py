"""ProductboardIngestionComponent -- Ingest Productboard notes, features, components, and objectives using dlt's generic REST API source.

Ingest Productboard data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Productboard has no official
...
class ProductboardIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Productboard API access token.')

    resources: str = Field(
        default="notes,features",
        description="Comma-separated list of resources to extract: notes, features, components, objectives"
    )
    ...
    group_name: Optional[str] = Field(default="productboard_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Productboard notes, features, components, and objectives using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["productboard", "python"]
        ...
        def productboard_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Productboard ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            if context.has_partition_key:
                try:
                    ...
                    _notes_params["dateFrom"] = _window.start.date().isoformat()
                    _notes_params["dateTo"] = _window.end.date().isoformat()
                except Exception:
                    pass
            _map = {
                "notes": {"name": "notes", "endpoint": {"path": "notes", "data_selector": "data", "params": _notes_params}},
                "features": {"name": "features", "endpoint": {"path": "features", "data_selector": "data"}},
                "components": {"name": "components", "endpoint": {"path": "components", "data_selector": "data"}},
                "objectives": {"name": "objectives", "endpoint": {"path": "objectives", "data_selector": "data"}},
            }
            ...
            context.log.info("Creating Productboard REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Productboard data...")
            load_info = pipeline.run(source)
            context.log.info(f"Productboard data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Productboard.")
        ...
        return Definitions(assets=[productboard_ingestion_asset])
