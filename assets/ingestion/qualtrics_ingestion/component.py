"""QualtricsIngestionComponent -- Ingest Qualtrics survey and mailing list data using dlt's generic REST API source.

Ingest Qualtrics data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Qualtrics has no official
...
class QualtricsIngestionComponent(Component, Model, Resolvable):
    ...
    datacenter_id: str = Field(description='Qualtrics datacenter ID (found in your account settings or the Qualtrics URL).')

    api_token: str = Field(description='Qualtrics API token.')

    directory_id: Optional[str] = Field(default=None, description='Directory (Contact List) ID. Required for the mailing_lists resource.')

    resources: str = Field(
        default="surveys",
        description="Comma-separated list of resources to extract: surveys (account-level); mailing_lists (requires directory_id)"
    )
    ...
    group_name: Optional[str] = Field(default="qualtrics_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Qualtrics survey and mailing list data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["qualtrics", "python"]
        ...
        def qualtrics_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Qualtrics ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": f"https://{self.datacenter_id}.qualtrics.com/API/v3",
                    "auth": {"type": "api_key", "api_key": self.api_token, "name": "X-API-TOKEN", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "surveys": {"name": "surveys", "endpoint": {"path": "surveys", "data_selector": "result.elements"}},
            }
            if self.directory_id:
                _map["mailing_lists"] = {"name": "mailing_lists", "endpoint": {"path": f"directories/{self.directory_id}/mailinglists", "data_selector": "result.elements"}}
            for r in resources_list:
                if r in _map:
                    config["resources"].append(_map[r])
                elif r == "mailing_lists":
                    context.log.warning("Resource 'mailing_lists' requires directory_id to be set; skipping.")
            ...
            context.log.info("Creating Qualtrics REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Qualtrics data...")
            load_info = pipeline.run(source)
            context.log.info(f"Qualtrics data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Qualtrics.")
        ...
        return Definitions(assets=[qualtrics_ingestion_asset])
