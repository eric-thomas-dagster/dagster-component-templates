"""RampIngestionComponent -- Ingest Ramp corporate card/spend data using dlt's generic REST API source.

Ingest Ramp data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Ramp has no official
...
class RampIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Ramp OAuth2 access token (client-credentials grant). Use ${RAMP_ACCESS_TOKEN} for env vars.')

    resources: str = Field(
        default="transactions,cards",
        description="Comma-separated list of resources to extract: transactions, cards, users, reimbursements"
    )
    ...
    group_name: Optional[str] = Field(default="ramp_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Ramp corporate card/spend data using dlt's generic REST API source."
        ...
        def ramp_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Ramp ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            context.log.info("Creating Ramp REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Ramp data...")
            load_info = pipeline.run(source)
            context.log.info(f"Ramp data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Ramp.")
        ...
        return Definitions(assets=[ramp_ingestion_asset])
