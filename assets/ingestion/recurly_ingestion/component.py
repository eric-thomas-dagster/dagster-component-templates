"""RecurlyIngestionComponent -- Ingest Recurly subscription billing data using dlt's generic REST API source.

Ingest Recurly data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Recurly has no official
...
class RecurlyIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='Recurly private API key, used as the HTTP Basic auth username (password is blank).')

    resources: str = Field(
        default="accounts,subscriptions,invoices",
        description="Comma-separated list of resources to extract: accounts, subscriptions, invoices, transactions, coupons"
    )
    ...
    group_name: Optional[str] = Field(default="recurly_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Recurly subscription billing data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["recurly", "python"]
        ...
        def recurly_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Recurly ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            if context.has_partition_key:
                try:
                    ...
                    _date_params = {"begin_time": _window.start.isoformat(), "end_time": _window.end.isoformat()}
                except Exception:
                    pass
            _map = {
                "accounts": {"name": "accounts", "endpoint": {"path": "accounts", "data_selector": "data"}},
                "subscriptions": {"name": "subscriptions", "endpoint": {"path": "subscriptions", "data_selector": "data"}},
                "invoices": {"name": "invoices", "endpoint": {"path": "invoices", "data_selector": "data", "params": _date_params}},
                "transactions": {"name": "transactions", "endpoint": {"path": "transactions", "data_selector": "data", "params": _date_params}},
                "coupons": {"name": "coupons", "endpoint": {"path": "coupons", "data_selector": "data"}},
            }
            ...
            context.log.info("Creating Recurly REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Recurly data...")
            load_info = pipeline.run(source)
            context.log.info(f"Recurly data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Recurly.")
        ...
        return Definitions(assets=[recurly_ingestion_asset])
