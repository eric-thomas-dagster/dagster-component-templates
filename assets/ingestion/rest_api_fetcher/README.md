...
| `api_url` | `str` | URL of the API endpoint to fetch data from |

### Execution

| Field | Type | Default | Description |
|---|---|---|---|
| `params` | `Union[str, Dict[str, Any]]` | — | Query parameters — YAML dict or JSON string (dict form recommended). |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `partition_type` | `str` | — | Partition type: 'daily', 'weekly', 'monthly', 'hourly', 'static', 'multi', or None. The partition key is exposed to api_url and params via {partition_key} (always), plus {partition_date} / {partition_date_next} for time-based partitions. |
...
| `headers` | `Union[str, Dict[str, str]]` | — | HTTP headers — either a YAML dict ({Authorization: 'Bearer t'}) or a JSON string ('{"Authorization": "Bearer t"}'). Dict form is recommended (avoids YAML-loader template-resolution gotchas with single-brace strings). |
| `body` | `str` | — | JSON string of request body for POST/PUT requests |
| `auth_type` | `str` | — | Authentication type: 'basic', 'bearer', or None |
| `auth_username` | `str` | — | Username for basic authentication (use ${USERNAME} for env var) |
| `auth_password` | `str` | — | Password for basic authentication (use ${PASSWORD} for env var) |
| `auth_token` | `str` | — | Bearer token for authentication (use ${API_TOKEN} for env var) |
| `json_path` | `str` | — | JSON path to extract data from response (e.g., 'data.results') |
| `cache_results` | `bool` | `false` | Whether to cache results locally |
| `cache_path` | `str` | — | Path to cache file (required if cache_results is True) |
| `verify_ssl` | `bool` | `true` | Whether to verify SSL certificates |
| `include_preview_metadata` | `bool` | `false` | Include sample data preview in metadata when output_format is 'dataframe' (first 5 rows as markdown table and interactive preview) |
