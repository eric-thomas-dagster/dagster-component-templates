...
| Field | Type | Default | Description |
|---|---|---|---|
| `output_format` | `str` | `"dataframe"` | Output format: 'dataframe' (default — parsed JSON wrapped in a pandas DataFrame ready for downstream transforms), 'json' (raw Python dict/list — use only if a custom asset consumes it directly), 'csv', 'parquet', or 'tex… _(full docs in schema.json + component README)_ |
...
| `response_path` | `str` | — | JMESPath expression selecting the array of records to iterate. Used with `column_map` to normalize a nested / heterogeneous API response into a tabular batch. Example: `output.rateReplyDetails` or `data.results[?status==… _(full docs in schema.json + component README)_ |
| `column_map` | `Dict[str, str]` | — | Dict of `{output_column: jmespath_expression}` evaluated against each element of the array selected by `response_path`. Produces one row per array element with named columns extracted from arbitrary JSON shapes. Example… _(full docs in schema.json + component README)_ |
| `endpoints` | `Dict[str, Dict[str, Any]]` | — | Per-partition-value endpoint dispatch — dict keyed by the value of `partition_static_dim` (multi-partition mode) or by the partition key itself (single-dim static mode). When set, the top-level `api_url` / `method` / `he… _(full docs in schema.json + component README)_ |
| `sinks` | `List[Dict[str, Any]]` | — | Optional list of sinks that write the fetched DataFrame to a warehouse table (in addition to returning it as the asset value for the IO manager). Each sink is a dict — supported keys: `kind: table`, `resource_key: <name>… _(full docs in schema.json + component README)_ |
