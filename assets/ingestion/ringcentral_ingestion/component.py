"""RingCentralIngestionComponent -- Ingest RingCentral call logs, messages, and extension data using dlt's generic REST API source.

Ingest RingCentral data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). RingCentral has no official
...
class RingCentralIngestionComponent(Component, Model, Resolvable):
    ...
    client_id: str = Field(description='RingCentral app client ID.')

    client_secret: str = Field(description='RingCentral app client secret.')

    jwt_token: str = Field(description='Pre-issued RingCentral JWT credential (from an admin-created Custom Rest API app with JWT flow enabled).')

    resources: str = Field(
        default="call_logs,extensions",
        description="Comma-separated list of resources to extract: extensions, call_logs, messages, teams"
    )
    ...
    group_name: Optional[str] = Field(default="ringcentral_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest RingCentral call logs, messages, and extension data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["ringcentral", "python"]
        ...
        def ringcentral_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting RingCentral ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "extensions": {"name": "extensions", "endpoint": {"path": "account/~/extension", "data_selector": "records"}},
                "call_logs": {"name": "call_logs", "endpoint": {"path": "account/~/call-log", "data_selector": "records"}},
                "messages": {"name": "messages", "endpoint": {"path": "account/~/extension/~/message-store", "data_selector": "records"}},
                "teams": {"name": "teams", "endpoint": {"path": "team-messaging/v1/teams", "data_selector": "records"}},
            }
            ...
            context.log.info("Creating RingCentral REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting RingCentral data...")
            load_info = pipeline.run(source)
            context.log.info(f"RingCentral data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from RingCentral.")
        ...
        return Definitions(assets=[ringcentral_ingestion_asset])
