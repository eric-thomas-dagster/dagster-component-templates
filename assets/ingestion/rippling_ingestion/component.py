"""RippleIngestionComponent -- Ingest Rippling HR/workforce data using dlt's generic REST API source.

Ingest Rippling data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Rippling has no official
...
class RippleIngestionComponent(Component, Model, Resolvable):
    ...
    api_token: str = Field(description='Rippling API bearer token. Use ${RIPPLING_API_TOKEN} for env vars.')

    resources: str = Field(
        default="workers,departments",
        description="Comma-separated list of resources to extract: workers, users, departments, job_requisitions, compensation"
    )
    ...
    group_name: Optional[str] = Field(default="rippling_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Rippling HR/workforce data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["rippling", "python"]
        ...
        def rippling_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Rippling ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "workers": "workers/", "users": "users/", "departments": "departments/",
                "job_requisitions": "job-requisitions/", "compensation": "compensation/",
            }
            for r in resources_list:
                if r in _map:
                    config["resources"].append({"name": r, "endpoint": {"path": _map[r], "data_selector": "results"}})
            ...
            context.log.info("Creating Rippling REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Rippling data...")
            load_info = pipeline.run(source)
            context.log.info(f"Rippling data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Rippling.")
        ...
        return Definitions(assets=[rippling_ingestion_asset])
