...
Ingest Salesforce CRM objects using dlt's verified `salesforce` source.
...
class SalesforceIngestionComponent(Component, Model, Resolvable):
    ...
    Extracts standard or custom objects (Account, Opportunity, Contact, Lead,
    Campaign, Task, Event, etc.) via the Salesforce API. Authenticates with
    ...
    group_name: Optional[str] = Field(
        default="salesforce", description="Asset group for organization"
    )
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        def salesforce_ingestion_asset(context: AssetExecutionContext):
            ...
            selected_resources = []
            for resource_name in sf_objects:
                attr = resource_name.lower()
                if hasattr(source, attr):
                    selected_resources.append(getattr(source, attr))
                elif hasattr(source, resource_name):
                    selected_resources.append(getattr(source, resource_name))
                else:
                    context.log.warning(
                        f"Object '{resource_name}' not found in Salesforce source; skipping."
                    )
            if not selected_resources:
                raise ValueError("No valid Salesforce objects selected.")

            load_info = pipeline.run(selected_resources)
            context.log.info(f"Salesforce data loaded: {load_info}")

            resource_keys = [o.lower() for o in sf_objects]
