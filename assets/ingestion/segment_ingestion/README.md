...
| `resource_key` | optional | Resource key registered by a SegmentResourceComponent. When set, credentials are read from that resource at run time instead of access_token. **Caveat:** SegmentResourceComponent's `write_key_env_var` normally holds a Tracking-API write key, not a workspace Config-API access token -- these are different Segment credential types; only use this if the env var actually holds a workspace token. |
...
|---|---|---|---|
| `resource_key` | `str` | — | Optional resource key registered by a SegmentResourceComponent. When set, credentials are read from that resource at run time instead of access_token above, letting one registered Segment resource serve both this ingesti… _(full docs in schema.json + component README)_ |
