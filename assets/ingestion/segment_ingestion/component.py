"""SegmentIngestionComponent -- Ingest Segment CDP workspace configuration data (sources, destinations, catalog) using dlt's generic REST API source.

Ingest Segment data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Segment has no official
...
class SegmentIngestionComponent(Component, Model, Resolvable):
    ...
    resources: str = Field(
        ...
        description="Comma-separated list of resources to extract: sources, destinations, catalog_sources"
    )
    ...
    group_name: Optional[str] = Field(default="segment_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["segment", "python"]
        ...
        def segment_ingestion_asset(context: AssetExecutionContext):
            ...
            _map = {
                "sources": {"name": "sources", "endpoint": {"path": "sources", "data_selector": "sources"}},
                "destinations": {"name": "destinations", "endpoint": {"path": "destinations", "data_selector": "destinations"}},
                "catalog_sources": {"name": "catalog_sources", "endpoint": {"path": "catalog/sources", "data_selector": "sources"}},
            }
            ...
            context.log.info("Creating Segment REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Segment data...")
            load_info = pipeline.run(source)
            context.log.info(f"Segment data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Segment.")
        ...
        return Definitions(assets=[segment_ingestion_asset])
