...
| `connection_string_env_var` | `str` | Env var with Service Bus connection string |
...
|---|---|---|---|
| `max_messages` | `int` | `5000` | Max messages to consume per run |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `queue_name` | `str` | — | Queue name (set queue_name OR topic_name+subscription_name) |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `subscription_name` | `str` | — | Subscription name (required with topic_name) |
| `schema_name` | `str` | — | Destination schema name |
| `max_wait_seconds` | `float` | `5.0` | Max seconds to wait per receive call |
