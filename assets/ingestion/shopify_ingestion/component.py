...
Ingest Shopify e-commerce data (customers, orders, products) using dlt's verified `shopify_dlt` source.
...
class ShopifyIngestionComponent(Component, Model, Resolvable):
    ...
    Available resources: `customers`, `orders`, `products`. Filters orders by
    status and supports start-date based incremental loading.
    ...
    start_date: Optional[str] = Field(default="2000-01-01", description="Start date for incremental loading (YYYY-MM-DD)")

    order_status: Optional[str] = Field(default="any", description="Filter orders by status (open, closed, cancelled, any)")
    ...
    group_name: Optional[str] = Field(
        default="shopify", description="Asset group for organization"
    )
        ...
        def shopify_ingestion_asset(context: AssetExecutionContext):
            ...
            for resource_name in resources_list:
                if hasattr(source, resource_name):
                    ...
                    context.log.warning(
                        f"Resource '{resource_name}' not found in Shopify source; skipping."
                    )
            if not selected_resources:
                raise ValueError("No valid resources selected for Shopify source.")
