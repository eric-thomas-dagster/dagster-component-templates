...
Ingest Slack workspace data (messages, channels, users) using dlt's verified `slack` source.
...
class SlackIngestionComponent(Component, Model, Resolvable):
    ...
    Available resources: `messages`, `channels`, `users`. Authenticates with a
    ...
    group_name: Optional[str] = Field(
        default="slack", description="Asset group for organization"
    )
        ...
        def slack_ingestion_asset(context: AssetExecutionContext):
            ...
            for resource_name in resources_list:
                if hasattr(source, resource_name):
                    ...
                    context.log.warning(
                        f"Resource '{resource_name}' not found in Slack source; skipping."
                    )
            if not selected_resources:
                raise ValueError("No valid resources selected for Slack source.")
