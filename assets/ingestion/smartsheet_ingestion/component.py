"""SmartsheetIngestionComponent -- Ingest Smartsheet sheet metadata, users, reports, and (optionally) one sheet's row data using dlt's generic REST API source.

Ingest Smartsheet data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Smartsheet has no official
...
class SmartsheetIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Smartsheet API access token (personal or from an OAuth2 app).')

    sheet_id: Optional[str] = Field(default=None, description="Specific sheet ID to pull row data from (enables the sheet_rows resource). Find it via the sheets resource or the sheet's own URL.")

    resources: str = Field(
        default="sheets,users",
        description="Comma-separated list of resources to extract: sheets (metadata list), users, reports, sheet_rows (requires sheet_id)"
    )
    ...
    group_name: Optional[str] = Field(default="smartsheet_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Smartsheet sheet metadata, users, reports, and (optionally) one sheet's row data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["smartsheet", "python"]
        ...
        def smartsheet_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Smartsheet ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "sheets": {"name": "sheets", "endpoint": {"path": "sheets", "data_selector": "data"}},
                "users": {"name": "users", "endpoint": {"path": "users", "data_selector": "data"}},
                "reports": {"name": "reports", "endpoint": {"path": "reports", "data_selector": "data"}},
            }
            if self.sheet_id:
                _map["sheet_rows"] = {"name": "sheet_rows", "endpoint": {"path": f"sheets/{self.sheet_id}", "data_selector": "rows"}}
            for r in resources_list:
                if r in _map:
                    config["resources"].append(_map[r])
                elif r == "sheet_rows":
                    context.log.warning("Resource 'sheet_rows' requires sheet_id to be set; skipping.")
            ...
            context.log.info("Creating Smartsheet REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Smartsheet data...")
            load_info = pipeline.run(source)
            context.log.info(f"Smartsheet data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Smartsheet.")
        ...
        return Definitions(assets=[smartsheet_ingestion_asset])
