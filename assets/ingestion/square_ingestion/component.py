"""SquareIngestionComponent -- Ingest Square customer, catalog, and payment data using dlt's generic REST API source.

Ingest Square data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Square has no official
...
class SquareIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Square access token (Personal Access Token or OAuth2 token).')

    resources: str = Field(
        default="customers,payments",
        description="Comma-separated list of resources to extract: customers, catalog, payments"
    )
    ...
    group_name: Optional[str] = Field(default="square_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Square customer, catalog, and payment data using dlt's generic REST API source."
        ...
        def square_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Square ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "customers": {"name": "customers", "endpoint": {"path": "customers", "data_selector": "customers"}},
                "catalog": {"name": "catalog", "endpoint": {"path": "catalog/list", "data_selector": "objects"}},
                "payments": {"name": "payments", "endpoint": {"path": "payments", "data_selector": "payments"}},
            }
            ...
            context.log.info("Creating Square REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Square data...")
            load_info = pipeline.run(source)
            context.log.info(f"Square data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Square.")
        ...
        return Definitions(assets=[square_ingestion_asset])
