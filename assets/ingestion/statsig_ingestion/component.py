"""StatsigIngestionComponent -- Ingest Statsig feature gate and experiment configuration data using dlt's generic REST API source.

Ingest Statsig data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Statsig has no official
...
class StatsigIngestionComponent(Component, Model, Resolvable):
    ...
    project_id: Optional[str] = Field(default=None, description='Project ID. Required for feature_gates/dynamic_configs/experiments resources; not needed for projects.')

    resources: str = Field(
        default="projects",
        description="Comma-separated list of resources to extract: projects (org-level); feature_gates, dynamic_configs, experiments (require project_id)"
    )
    ...
    group_name: Optional[str] = Field(default="statsig_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Statsig feature gate and experiment configuration data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["statsig", "python"]
        ...
        def statsig_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Statsig ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://statsigapi.net/console/v1",
                    "auth": {"type": "api_key", "api_key": self.console_api_key, "name": "STATSIG-API-KEY", "location": "header"},
                },
                "resources": [],
            }
            _map = {
                "projects": {"name": "projects", "endpoint": {"path": "projects", "data_selector": "data"}},
            }
            if self.project_id:
                _map.update({
                    "feature_gates": {"name": "feature_gates", "endpoint": {"path": "gates", "data_selector": "data", "params": {"project_id": self.project_id}}},
                    "dynamic_configs": {"name": "dynamic_configs", "endpoint": {"path": "dynamic_configs", "data_selector": "data", "params": {"project_id": self.project_id}}},
                    "experiments": {"name": "experiments", "endpoint": {"path": "experiments", "data_selector": "data", "params": {"project_id": self.project_id}}},
                })
            ...
            context.log.info("Creating Statsig REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Statsig data...")
            load_info = pipeline.run(source)
            context.log.info(f"Statsig data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Statsig.")
        ...
        return Definitions(assets=[statsig_ingestion_asset])
