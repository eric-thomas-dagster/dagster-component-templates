"""StatuspageIngestionComponent -- Ingest Atlassian Statuspage incident/component data using dlt's generic REST API source.

Ingest Statuspage data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Statuspage has no official
...
class StatuspageIngestionComponent(Component, Model, Resolvable):
    ...
    page_id: str = Field(description='Statuspage page ID.')

    api_key: str = Field(description='Statuspage API key. Use ${STATUSPAGE_API_KEY} for env vars.')

    resources: str = Field(
        default="incidents,components",
        description="Comma-separated list of resources to extract: incidents, components, metrics, subscribers"
    )
    ...
    group_name: Optional[str] = Field(default="statuspage_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Atlassian Statuspage incident/component data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["statuspage", "python"]
        ...
        def statuspage_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Statuspage ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.statuspage.io/v1",
                    "auth": {"type": "api_key", "api_key": f"OAuth {self.api_key}", "name": "Authorization", "location": "header"},
                },
                "resources": [],
            }
            for r in resources_list:
                config["resources"].append({"name": r, "endpoint": {"path": f"pages/{self.page_id}/{r}.json"}})
            ...
            context.log.info("Creating Statuspage REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Statuspage data...")
            load_info = pipeline.run(source)
            context.log.info(f"Statuspage data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Statuspage.")
        ...
        return Definitions(assets=[statuspage_ingestion_asset])
