...
| `group_name` | `str` | `"stripe"` | Asset group for organization |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `resources` | `str` | `"customers,subscriptions,charges"` | Comma-separated list: customers, subscriptions, charges, invoices, products, prices, payment_intents, balance_transactions, events |
| `start_date` | `str` | — | Start date for data extraction (YYYY-MM-DD). Defaults to all historical data. |
| `end_date` | `str` | — | End date for data extraction (YYYY-MM-DD). Defaults to today. |
| `incremental` | `bool` | `false` | Use incremental loading (append mode) instead of full refresh |
