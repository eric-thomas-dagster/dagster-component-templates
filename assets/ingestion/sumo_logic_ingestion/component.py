"""SumoLogicIngestionComponent -- Ingest Sumo Logic collector and source metadata using dlt's generic REST API source. Log/message search data requires an async job/poll pattern not implemented here -- this connector covers collector/source metadata only.

Ingest Sumo Logic data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Sumo Logic has no official
...
class SumoLogicIngestionComponent(Component, Model, Resolvable):
    ...
    access_id: str = Field(description='Sumo Logic access ID, used as the HTTP Basic auth username.')

    access_key: str = Field(description='Sumo Logic access key, used as the HTTP Basic auth password.')

    deployment: Optional[str] = Field(default=None, description="Sumo Logic deployment pod: 'us1' (default), 'us2', 'au', 'ca', 'de', 'eu', 'fed', 'in', or 'jp'. Must match your account.")

    collector_id: Optional[str] = Field(default=None, description='Specific collector ID to list sources for (enables the sources resource).')

    resources: str = Field(
        default="collectors",
        description="Comma-separated list of resources to extract: collectors (account-level); sources (requires collector_id)"
    )
    ...
    group_name: Optional[str] = Field(default="sumo_logic_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Sumo Logic collector and source metadata using dlt's generic REST API source. Log/message search data requires an async job/poll pattern not implemented here -- this connector covers collector/source metadata only."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["sumo_logic", "python"]
        ...
        def sumo_logic_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Sumo Logic ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            _deployment = self.deployment or "us1"
            _base = "https://api.sumologic.com/api" if _deployment == "us1" else f"https://api.{_deployment}.sumologic.com/api"

            config = {
                "client": {
                    "base_url": _base,
                    "auth": {"type": "http_basic", "username": self.access_id, "password": self.access_key},
                },
                "resources": [],
            }
            _map = {
                "collectors": {"name": "collectors", "endpoint": {"path": "v1/collectors", "data_selector": "collectors"}},
            }
            if self.collector_id:
                _map["sources"] = {"name": "sources", "endpoint": {"path": f"v1/collectors/{self.collector_id}/sources", "data_selector": "sources"}}
            for r in resources_list:
                if r in _map:
                    config["resources"].append(_map[r])
                elif r == "sources":
                    context.log.warning("Resource 'sources' requires collector_id to be set; skipping.")
            ...
            context.log.info("Creating Sumo Logic REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Sumo Logic data...")
            load_info = pipeline.run(source)
            context.log.info(f"Sumo Logic data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Sumo Logic.")
        ...
        return Definitions(assets=[sumo_logic_ingestion_asset])
