"""SurveyMonkeyIngestionComponent -- Ingest SurveyMonkey surveys and responses using dlt's generic REST API source.

Ingest SurveyMonkey data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). SurveyMonkey has no official
...
class SurveyMonkeyIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='SurveyMonkey OAuth2 access token. Use ${SURVEYMONKEY_ACCESS_TOKEN} for env vars.')

    survey_id: Optional[str] = Field(default=None, description="Survey ID to pull responses for (required if 'responses' is in resources).")

    resources: str = Field(
        default="surveys",
        description="Comma-separated list of resources to extract: surveys, responses (responses requires survey_id)"
    )
    ...
    group_name: Optional[str] = Field(default="surveymonkey_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest SurveyMonkey surveys and responses using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["surveymonkey", "python"]
        ...
        def surveymonkey_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting SurveyMonkey ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            if "surveys" in resources_list:
                config["resources"].append({"name": "surveys", "endpoint": {"path": "surveys", "data_selector": "data"}})
            if "responses" in resources_list:
                if not self.survey_id:
                    raise ValueError("resources includes 'responses' but survey_id is not set.")
                config["resources"].append({"name": "responses", "endpoint": {"path": f"surveys/{self.survey_id}/responses/bulk", "data_selector": "data"}})
            ...
            context.log.info("Creating SurveyMonkey REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting SurveyMonkey data...")
            load_info = pipeline.run(source)
            context.log.info(f"SurveyMonkey data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from SurveyMonkey.")
        ...
        return Definitions(assets=[surveymonkey_ingestion_asset])
