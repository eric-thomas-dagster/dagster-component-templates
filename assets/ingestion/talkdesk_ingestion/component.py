"""TalkdeskIngestionComponent -- Ingest Talkdesk contact center data (contacts, campaigns, users) using dlt's generic REST API source.

Ingest Talkdesk data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Talkdesk has no official
...
class TalkdeskIngestionComponent(Component, Model, Resolvable):
    ...
    account_subdomain: str = Field(description='Talkdesk account subdomain (for mycompany.talkdesk.com), used to obtain the OAuth2 token.')

    client_id: str = Field(description='Talkdesk OAuth2 client_credentials client ID.')

    client_secret: str = Field(description='Talkdesk OAuth2 client_credentials client secret.')

    resources: str = Field(
        default="contacts,campaigns",
        description="Comma-separated list of resources to extract: contacts, campaigns, users"
    )
    ...
    group_name: Optional[str] = Field(default="talkdesk_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Talkdesk contact center data (contacts, campaigns, users) using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["talkdesk", "python"]
        ...
        def talkdesk_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Talkdesk ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _token_resp = _requests.post(
                f"https://{self.account_subdomain}.talkdesk.com/oauth/token",
            ...
            _map = {
                ...
                "campaigns": {"name": "campaigns", "endpoint": {"path": "campaigns", "data_selector": "campaigns"}},
            ...
            context.log.info("Creating Talkdesk REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Talkdesk data...")
            load_info = pipeline.run(source)
            context.log.info(f"Talkdesk data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Talkdesk.")
        ...
        return Definitions(assets=[talkdesk_ingestion_asset])
