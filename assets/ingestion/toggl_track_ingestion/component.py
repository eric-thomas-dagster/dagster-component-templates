"""TogglTrackIngestionComponent -- Ingest Toggl Track time entries, projects, and clients using dlt's generic REST API source.

Ingest Toggl Track data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Toggl Track has no official
...
class TogglTrackIngestionComponent(Component, Model, Resolvable):
    ...
    api_token: str = Field(description="Toggl Track API token, used as the HTTP Basic auth username (password is the literal string 'api_token').")

    workspace_id: Optional[str] = Field(default=None, description='Toggl workspace ID. Required for projects/clients resources; not needed for time_entries.')

    resources: str = Field(
        default="time_entries",
        description="Comma-separated list of resources to extract: time_entries (account-level); projects, clients (require workspace_id)"
    )
    ...
    group_name: Optional[str] = Field(default="toggl_track_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Toggl Track time entries, projects, and clients using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["toggl_track", "python"]
        ...
        def toggl_track_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Toggl Track ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.track.toggl.com/api/v9",
                    "auth": {"type": "http_basic", "username": self.api_token, "password": "api_token"},
                },
                "resources": [],
            }
            _time_params = {}
            if context.has_partition_key:
                try:
                    ...
                    _time_params["since"] = int(_window.start.timestamp())
                    _time_params["before"] = _window.end.date().isoformat()
                except Exception:
                    pass
            _map = {
                "time_entries": {"name": "time_entries", "endpoint": {"path": "me/time_entries", "data_selector": "$", "params": _time_params}},
            }
            if self.workspace_id:
                _map.update({
                    "projects": {"name": "projects", "endpoint": {"path": f"workspaces/{self.workspace_id}/projects", "data_selector": "$"}},
                    "clients": {"name": "clients", "endpoint": {"path": f"workspaces/{self.workspace_id}/clients", "data_selector": "$"}},
                })
            for r in resources_list:
                if r in _map:
                    config["resources"].append(_map[r])
                else:
                    context.log.warning(f"Resource {r!r} requires workspace_id to be set; skipping.")
            ...
            context.log.info("Creating Toggl Track REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Toggl Track data...")
            load_info = pipeline.run(source)
            context.log.info(f"Toggl Track data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Toggl Track.")
        ...
        return Definitions(assets=[toggl_track_ingestion_asset])
