"""TrelloIngestionComponent -- Ingest Trello boards/cards data using dlt's generic REST API source.

Ingest Trello data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Trello has no official
...
class TrelloIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='Trello API key. Use ${TRELLO_API_KEY} for env vars.')

    api_token: str = Field(description='Trello API token. Use ${TRELLO_API_TOKEN} for env vars.')

    resources: str = Field(
        default="boards,cards,lists",
        description="Comma-separated list of resources to extract: boards, cards, lists, actions"
    )
    ...
    group_name: Optional[str] = Field(default="trello_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Trello boards/cards data using dlt's generic REST API source."
        ...
        def trello_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Trello ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://api.trello.com/1",
                    "auth": {"type": "api_key", "api_key": self.api_token, "name": "token", "location": "query"},
                    "params": {"key": self.api_key},
                },
                "resources": [],
            }
            _map = {
                "boards": {"name": "boards", "endpoint": {"path": "members/me/boards"}},
                "cards": {"name": "cards", "endpoint": {"path": "members/me/cards"}},
                "lists": {"name": "lists", "endpoint": {"path": "members/me/boards", "params": {"lists": "open"}}},
            }
            ...
            context.log.info("Creating Trello REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Trello data...")
            load_info = pipeline.run(source)
            context.log.info(f"Trello data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Trello.")
        ...
        return Definitions(assets=[trello_ingestion_asset])
