"""TypeformIngestionComponent -- Ingest Typeform forms and responses using dlt's generic REST API source.

Ingest Typeform data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Typeform has no official
...
class TypeformIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Typeform personal access token. Use ${TYPEFORM_ACCESS_TOKEN} for env vars.')

    form_id: Optional[str] = Field(default=None, description="Specific form ID to pull responses for (required if 'responses' is in resources).")

    resources: str = Field(
        default="forms",
        description="Comma-separated list of resources to extract: forms, responses (responses requires form_id)"
    )
    ...
    group_name: Optional[str] = Field(default="typeform_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Typeform forms and responses using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["typeform", "python"]
        ...
        def typeform_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Typeform ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            if "forms" in resources_list:
                config["resources"].append({"name": "forms", "endpoint": {"path": "forms", "data_selector": "items"}})
            if "responses" in resources_list:
                if not self.form_id:
                    raise ValueError("resources includes 'responses' but form_id is not set.")
                config["resources"].append({"name": "responses", "endpoint": {"path": f"forms/{self.form_id}/responses", "data_selector": "items"}})
            ...
            context.log.info("Creating Typeform REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Typeform data...")
            load_info = pipeline.run(source)
            context.log.info(f"Typeform data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Typeform.")
        ...
        return Definitions(assets=[typeform_ingestion_asset])
