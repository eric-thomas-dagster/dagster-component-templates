"""VercelIngestionComponent -- Ingest Vercel deployment, project, and domain data using dlt's generic REST API source.

Ingest Vercel data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Vercel has no official
...
class VercelIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Vercel personal or team access token.')

    team_id: Optional[str] = Field(default=None, description='Vercel team ID. Required if the token has access to multiple teams.')

    resources: str = Field(
        default="deployments,projects",
        description="Comma-separated list of resources to extract: deployments, projects, domains"
    )
    ...
    group_name: Optional[str] = Field(default="vercel_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Vercel deployment, project, and domain data using dlt's generic REST API source."
        ...
        def vercel_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Vercel ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            _team_params = {"teamId": self.team_id} if self.team_id else {}
            config = {
                "client": {
            ...
            _deploy_params = dict(_team_params)
            if context.has_partition_key:
                try:
                    _window = context.partition_time_window
                    _deploy_params["since"] = int(_window.start.timestamp() * 1000)
                    _deploy_params["until"] = int(_window.end.timestamp() * 1000)
                except Exception:
                    pass
            _map = {
                "deployments": {"name": "deployments", "endpoint": {"path": "v6/deployments", "data_selector": "deployments", "params": _deploy_params}},
                "projects": {"name": "projects", "endpoint": {"path": "v9/projects", "data_selector": "projects", "params": _team_params}},
                "domains": {"name": "domains", "endpoint": {"path": "v5/domains", "data_selector": "domains", "params": _team_params}},
            }
            ...
            context.log.info("Creating Vercel REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Vercel data...")
            load_info = pipeline.run(source)
            context.log.info(f"Vercel data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Vercel.")
        ...
        return Definitions(assets=[vercel_ingestion_asset])
