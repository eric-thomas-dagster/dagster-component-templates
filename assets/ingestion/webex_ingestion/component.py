"""WebexIngestionComponent -- Ingest Cisco Webex room, meeting, and people data using dlt's generic REST API source.

Ingest Webex data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Webex has no official
...
class WebexIngestionComponent(Component, Model, Resolvable):
    ...
    bot_token: str = Field(description='Webex bot access token (static, long-lived) -- the recommended non-interactive auth path. A personal access token also works but expires after ~12 hours.')

    resources: str = Field(
        default="rooms,people",
        description="Comma-separated list of resources to extract: rooms, meetings, people"
    )
    ...
    group_name: Optional[str] = Field(default="webex_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Cisco Webex room, meeting, and people data using dlt's generic REST API source."
        ...
        def webex_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Webex ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://webexapis.com/v1",
                    "auth": {"type": "bearer", "token": self.bot_token},
                },
                "resources": [],
            }
            _map = {
                "rooms": {"name": "rooms", "endpoint": {"path": "rooms", "data_selector": "items"}},
                "meetings": {"name": "meetings", "endpoint": {"path": "meetings", "data_selector": "items"}},
                "people": {"name": "people", "endpoint": {"path": "people", "data_selector": "items"}},
            }
            ...
            context.log.info("Creating Webex REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Webex data...")
            load_info = pipeline.run(source)
            context.log.info(f"Webex data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Webex.")
        ...
        return Definitions(assets=[webex_ingestion_asset])
