"""WebflowIngestionComponent -- Ingest Webflow CMS data using dlt's generic REST API source.

Ingest Webflow data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Webflow has no official
...
class WebflowIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Webflow API access token. Use ${WEBFLOW_ACCESS_TOKEN} for env vars.')

    site_id: Optional[str] = Field(default=None, description="Site ID to scope collections to (required if 'collections' is in resources).")

    collection_id: Optional[str] = Field(default=None, description="Collection ID to pull items for (required if 'items' is in resources).")

    resources: str = Field(
        default="sites,collections",
        description="Comma-separated list of resources to extract: sites, collections, items (items requires collection_id)"
    )
    ...
    group_name: Optional[str] = Field(default="webflow_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Webflow CMS data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["webflow", "python"]
        ...
        def webflow_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Webflow ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            if "sites" in resources_list:
                config["resources"].append({"name": "sites", "endpoint": {"path": "sites", "data_selector": "sites"}})
            if "collections" in resources_list:
                if not self.site_id:
                    raise ValueError("resources includes 'collections' but site_id is not set.")
                config["resources"].append({"name": "collections", "endpoint": {"path": f"sites/{self.site_id}/collections", "data_selector": "collections"}})
            if "items" in resources_list:
                if not self.collection_id:
                    raise ValueError("resources includes 'items' but collection_id is not set.")
                config["resources"].append({"name": "items", "endpoint": {"path": f"collections/{self.collection_id}/items", "data_selector": "items"}})
            ...
            context.log.info("Creating Webflow REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Webflow data...")
            load_info = pipeline.run(source)
            context.log.info(f"Webflow data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Webflow.")
        ...
        return Definitions(assets=[webflow_ingestion_asset])
