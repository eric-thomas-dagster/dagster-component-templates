"""WooCommerceIngestionComponent -- Ingest WooCommerce store data using dlt's generic REST API source.

Ingest WooCommerce data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). WooCommerce has no official
...
class WooCommerceIngestionComponent(Component, Model, Resolvable):
    ...
    store_url: str = Field(description='Base URL of your WooCommerce store (the /wp-json/wc/v3 path is appended automatically).')

    consumer_key: str = Field(description='WooCommerce REST API consumer key, used as the HTTP Basic auth username.')

    consumer_secret: str = Field(description='WooCommerce REST API consumer secret, used as the HTTP Basic auth password.')

    resources: str = Field(
        default="products,orders",
        description="Comma-separated list of resources to extract: products, orders, customers, coupons"
    )
    ...
    group_name: Optional[str] = Field(default="woocommerce_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest WooCommerce store data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["woocommerce", "python"]
        ...
        def woocommerce_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting WooCommerce ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": self.store_url.rstrip("/") + "/wp-json/wc/v3",
                    "auth": {"type": "http_basic", "username": self.consumer_key, "password": self.consumer_secret},
                },
                "resources": [],
            }
            _order_params = {}
            if context.has_partition_key:
                try:
                    ...
                    _order_params = {"after": _window.start.isoformat(), "before": _window.end.isoformat()}
                except Exception:
                    pass
            _map = {
                "products": {"name": "products", "endpoint": {"path": "products", "data_selector": "$"}},
                "orders": {"name": "orders", "endpoint": {"path": "orders", "data_selector": "$", "params": _order_params}},
                "customers": {"name": "customers", "endpoint": {"path": "customers", "data_selector": "$"}},
                "coupons": {"name": "coupons", "endpoint": {"path": "coupons", "data_selector": "$"}},
            }
            ...
            context.log.info("Creating WooCommerce REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting WooCommerce data...")
            load_info = pipeline.run(source)
            context.log.info(f"WooCommerce data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from WooCommerce.")
        ...
        return Definitions(assets=[woocommerce_ingestion_asset])
