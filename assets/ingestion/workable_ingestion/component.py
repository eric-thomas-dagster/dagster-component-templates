...
Ingest Workable recruiting data (jobs, candidates, stages, members) using dlt's verified `workable` source.
...
class WorkableIngestionComponent(Component, Model, Resolvable):
    """Component for ingesting Workable recruiting data using dlt.

    Available resources: `jobs`, `candidates`, `stages`, `members`.
    ...
    api_token: str = Field(description="Workable API token for authentication")

    subdomain: str = Field(description="Workable subdomain (e.g., 'your-company' from your-company.workable.com)")

    resources: List[str] = Field(default=["jobs", "candidates"], description="Workable resources to extract (jobs, candidates, stages, members)")
    ...
    group_name: Optional[str] = Field(
        default="workable", description="Asset group for organization"
    )
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or f"Workable data ({', '.join(resources_list)})"
        ...
        def workable_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.workable import workable_source

            context.log.info(
                f"Starting Workable ingestion: subdomain={subdomain}, resources={resources_list}, "
            ...
            for resource_name in resources_list:
                if hasattr(source, resource_name):
                    ...
                    context.log.warning(
                        f"Resource '{resource_name}' not found in Workable source; skipping."
                    )
            if not selected_resources:
                raise ValueError("No valid resources selected for Workable source.")
