"""WorkdayIngestionComponent -- Ingest Workday HCM worker data using dlt's generic REST API source.

Ingest Workday data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Workday has no official
...
class WorkdayIngestionComponent(Component, Model, Resolvable):
    ...
    resources: str = Field(
        default="workers",
        description="Comma-separated list of resources to extract: workers (only resource currently wired up -- see confidence note)"
    )
    ...
    group_name: Optional[str] = Field(default="workday_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["workday", "python"]
        ...
        def workday_ingestion_asset(context: AssetExecutionContext):
            ...
            context.log.info(f"Starting Workday ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "workers": {"name": "workers", "endpoint": {"path": "workers", "data_selector": "data"}},
            }
            for r in resources_list:
                if r in _map:
                    ...
                    context.log.warning(
                        f"Resource {r!r} is not wired up in this low-confidence Workday connector; "
                        f"only 'workers' is currently mapped. Extend _map in component.py for other "
                        f"Workday REST endpoints specific to your tenant/module."
                    )
            ...
            context.log.info("Creating Workday REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Workday data...")
            load_info = pipeline.run(source)
            context.log.info(f"Workday data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Workday.")
        ...
        return Definitions(assets=[workday_ingestion_asset])
