"""WorkOSIngestionComponent -- Ingest WorkOS SSO/directory-sync data using dlt's generic REST API source.

Ingest WorkOS data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). WorkOS has no official
...
class WorkOSIngestionComponent(Component, Model, Resolvable):
    ...
    api_key: str = Field(description='WorkOS API key (starts with sk_).')

    resources: str = Field(
        default="connections,organizations",
        description="Comma-separated list of resources to extract: connections, organizations, directory_users, directory_groups, audit_logs"
    )
    ...
    group_name: Optional[str] = Field(default="workos_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest WorkOS SSO/directory-sync data using dlt's generic REST API source."
        ...
        def workos_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting WorkOS ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "connections": {"name": "connections", "endpoint": {"path": "connections", "data_selector": "data"}},
                "organizations": {"name": "organizations", "endpoint": {"path": "organizations", "data_selector": "data"}},
                "directory_users": {"name": "directory_users", "endpoint": {"path": "directory_users", "data_selector": "data"}},
                "directory_groups": {"name": "directory_groups", "endpoint": {"path": "directory_groups", "data_selector": "data"}},
                "audit_logs": {"name": "audit_logs", "endpoint": {"path": "audit_logs/actions", "data_selector": "data"}},
            }
            ...
            context.log.info("Creating WorkOS REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting WorkOS data...")
            load_info = pipeline.run(source)
            context.log.info(f"WorkOS data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from WorkOS.")
        ...
        return Definitions(assets=[workos_ingestion_asset])
