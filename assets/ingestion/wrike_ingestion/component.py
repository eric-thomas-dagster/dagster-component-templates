"""WrikeIngestionComponent -- Ingest Wrike task, folder, and contact data using dlt's generic REST API source.

Ingest Wrike data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Wrike has no official
...
class WrikeIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Wrike permanent access token (Apps & Integrations > API).')

    resources: str = Field(
        default="tasks,folders",
        description="Comma-separated list of resources to extract: tasks, folders, contacts"
    )
    ...
    group_name: Optional[str] = Field(default="wrike_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Wrike task, folder, and contact data using dlt's generic REST API source."
        ...
        def wrike_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Wrike ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "tasks": {"name": "tasks", "endpoint": {"path": "tasks", "data_selector": "data"}},
                "folders": {"name": "folders", "endpoint": {"path": "folders", "data_selector": "data"}},
            ...
            context.log.info("Creating Wrike REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Wrike data...")
            load_info = pipeline.run(source)
            context.log.info(f"Wrike data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Wrike.")
        ...
        return Definitions(assets=[wrike_ingestion_asset])
