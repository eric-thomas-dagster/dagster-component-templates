"""XeroIngestionComponent -- Ingest Xero accounting data using dlt's generic REST API source.

Ingest Xero data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Xero has no official
...
class XeroIngestionComponent(Component, Model, Resolvable):
    ...
    client_id: str = Field(description='Xero app client ID (used for the OAuth2 refresh-token exchange).')

    client_secret: str = Field(description='Xero app client secret.')

    refresh_token: str = Field(description="OAuth2 refresh token obtained once via Xero's OAuth consent flow (PKCE).")

    resources: str = Field(
        default="Invoices,Contacts",
        description="Comma-separated list of resources to extract: Invoices, Contacts, Accounts, BankTransactions"
    )
    ...
    group_name: Optional[str] = Field(default="xero_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Xero accounting data using dlt's generic REST API source."
        ...
        def xero_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Xero ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _conn_resp = _requests.get(
                "https://api.xero.com/connections",
                headers={"Authorization": f"Bearer {_access_token}"},
            )
            _conn_resp.raise_for_status()
            _connections = _conn_resp.json()
            _tenant_id = _connections[0]["tenantId"] if _connections else None

            config = {
                "client": {
                    "base_url": "https://api.xero.com/api.xro/2.0",
                    "auth": {"type": "bearer", "token": _access_token},
                    "headers": {"Xero-tenant-id": _tenant_id, "Accept": "application/json"},
                },
                "resources": [],
            }
            _map = {
                "Invoices": {"name": "Invoices", "endpoint": {"path": "Invoices", "data_selector": "Invoices"}},
                "Contacts": {"name": "Contacts", "endpoint": {"path": "Contacts", "data_selector": "Contacts"}},
                "Accounts": {"name": "Accounts", "endpoint": {"path": "Accounts", "data_selector": "Accounts"}},
                "BankTransactions": {"name": "BankTransactions", "endpoint": {"path": "BankTransactions", "data_selector": "BankTransactions"}},
            }
            ...
            context.log.info("Creating Xero REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Xero data...")
            load_info = pipeline.run(source)
            context.log.info(f"Xero data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Xero.")
        ...
        return Definitions(assets=[xero_ingestion_asset])
