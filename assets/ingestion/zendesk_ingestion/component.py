...
Ingest Zendesk support data (tickets, users, organizations, groups) using dlt's verified `zendesk` source.
...
class ZendeskIngestionComponent(Component, Model, Resolvable):
    """Component for ingesting Zendesk support data using dlt.

    Available resources: `tickets`, `users`, `organizations`, `groups`.
    Authenticates via subdomain + email + API token.
    ...
    group_name: Optional[str] = Field(
        default="zendesk", description="Asset group for organization"
    )
        ...
        def zendesk_ingestion_asset(context: AssetExecutionContext):
            ...
            for resource_name in resources_list:
                if hasattr(source, resource_name):
                    ...
                    context.log.warning(
                        f"Resource '{resource_name}' not found in Zendesk source; skipping."
                    )
            if not selected_resources:
                raise ValueError("No valid resources selected for Zendesk source.")
