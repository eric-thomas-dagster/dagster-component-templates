"""ZohoCrmIngestionComponent -- Ingest Zoho CRM records using dlt's generic REST API source.

Ingest Zoho CRM data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Zoho CRM has no official
...
class ZohoCrmIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Zoho OAuth2 access token. Use ${ZOHO_ACCESS_TOKEN} for env vars.')

    resources: str = Field(
        default="Leads,Contacts,Accounts,Deals",
        description="Comma-separated list of resources to extract: Zoho CRM module API names, e.g. Leads, Contacts, Accounts, Deals"
    )
    ...
    group_name: Optional[str] = Field(default="zoho_crm_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Zoho CRM records using dlt's generic REST API source."
        ...
        def zoho_crm_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Zoho CRM ingestion, destination={destination or 'duckdb (in-memory)'}")

            resources_list = [r.strip() for r in resources.split(",")]
            config = {
                "client": {
                    "base_url": "https://www.zohoapis.com/crm/v3",
                    "auth": {"type": "api_key", "api_key": f"Zoho-oauthtoken {self.access_token}", "name": "Authorization", "location": "header"},
                },
                "resources": [],
            }
            for r in resources_list:
                config["resources"].append({"name": r.lower(), "endpoint": {"path": r, "data_selector": "data"}})
            ...
            context.log.info("Creating Zoho CRM REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Zoho CRM data...")
            load_info = pipeline.run(source)
            context.log.info(f"Zoho CRM data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Zoho CRM.")
        ...
        return Definitions(assets=[zoho_crm_ingestion_asset])
