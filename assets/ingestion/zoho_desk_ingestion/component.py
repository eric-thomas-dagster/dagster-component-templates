"""ZohoDeskIngestionComponent -- Ingest Zoho Desk ticket, contact, and agent data using dlt's generic REST API source.

Ingest Zoho Desk data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Zoho Desk has no official
...
class ZohoDeskIngestionComponent(Component, Model, Resolvable):
    ...
    client_id: str = Field(description='Zoho app client ID (used for the OAuth2 refresh-token exchange).')

    client_secret: str = Field(description='Zoho app client secret.')

    refresh_token: str = Field(description="OAuth2 refresh token obtained once via Zoho's OAuth consent flow.")

    org_id: str = Field(description='Zoho Desk organization ID (required on most API calls).')

    datacenter: Optional[str] = Field(default=None, description="Zoho datacenter TLD suffix: 'com' (default), 'eu', 'in', 'com.au', or 'jp'.")

    resources: str = Field(
        default="tickets,contacts",
        description="Comma-separated list of resources to extract: tickets, contacts, agents, departments"
    )
    ...
    group_name: Optional[str] = Field(default="zoho_desk_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Zoho Desk ticket, contact, and agent data using dlt's generic REST API source."
        ...
        if not _inferred_kinds:
            _inferred_kinds = ["zoho_desk", "python"]
        ...
        def zoho_desk_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Zoho Desk ingestion, destination={destination or 'duckdb (in-memory)'}")

            import requests as _requests

            resources_list = [r.strip() for r in resources.split(",")]
            _dc = self.datacenter or "com"
            ...
            config = {
                "client": {
                    "base_url": f"https://desk.zoho.{_dc}/api/v1",
                    "auth": {"type": "api_key", "api_key": f"Zoho-oauthtoken {_access_token}", "name": "Authorization", "location": "header"},
                    "headers": {"orgId": self.org_id},
                },
                "resources": [],
            }
            _map = {
                "tickets": {"name": "tickets", "endpoint": {"path": "tickets", "data_selector": "data"}},
                "contacts": {"name": "contacts", "endpoint": {"path": "contacts", "data_selector": "data"}},
                "agents": {"name": "agents", "endpoint": {"path": "agents", "data_selector": "data"}},
                "departments": {"name": "departments", "endpoint": {"path": "departments", "data_selector": "data"}},
            }
            ...
            context.log.info("Creating Zoho Desk REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Zoho Desk data...")
            load_info = pipeline.run(source)
            context.log.info(f"Zoho Desk data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Zoho Desk.")
        ...
        return Definitions(assets=[zoho_desk_ingestion_asset])
