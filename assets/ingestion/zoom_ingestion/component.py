"""ZoomIngestionComponent -- Ingest Zoom meetings/webinars/users data using dlt's generic REST API source.

Ingest Zoom data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Zoom has no official
...
class ZoomIngestionComponent(Component, Model, Resolvable):
    ...
    access_token: str = Field(description='Zoom OAuth2 access token (Server-to-Server OAuth app). Use ${ZOOM_ACCESS_TOKEN} for env vars.')

    user_id: Optional[str] = Field(default=None, description="Zoom user ID to scope meetings/webinars to. Defaults to 'me' (the token's own user).")

    resources: str = Field(
        default="users,meetings,webinars",
        description="Comma-separated list of resources to extract: users, meetings, upcoming_meetings, webinars"
    )
    ...
    group_name: Optional[str] = Field(default="zoom_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Zoom meetings/webinars/users data using dlt's generic REST API source."
        ...
        def zoom_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Zoom ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                ...
                "meetings": {"name": "meetings", "endpoint": {"path": f"users/{uid}/meetings", "data_selector": "meetings"}},
                "upcoming_meetings": {"name": "upcoming_meetings", "endpoint": {"path": f"users/{uid}/upcoming_meetings", "data_selector": "meetings"}},
                "webinars": {"name": "webinars", "endpoint": {"path": f"users/{uid}/webinars", "data_selector": "webinars"}},
            }
            ...
            context.log.info("Creating Zoom REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Zoom data...")
            load_info = pipeline.run(source)
            context.log.info(f"Zoom data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Zoom.")
        ...
        return Definitions(assets=[zoom_ingestion_asset])
