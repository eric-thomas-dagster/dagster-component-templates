"""ZuoraIngestionComponent -- Ingest Zuora subscription billing data (accounts, subscriptions, invoices) using dlt's generic REST API source.

Ingest Zuora data using dlt's generic REST API source
(dlt.sources.rest_api.rest_api_source). Zuora has no official
...
class ZuoraIngestionComponent(Component, Model, Resolvable):
    ...
    base_url: str = Field(description='Your Zuora REST endpoint (region/environment-specific, e.g. rest.na.zuora.com, rest.eu.zuora.com, or a sandbox host).')

    client_id: str = Field(description='Zuora OAuth client ID.')

    client_secret: str = Field(description='Zuora OAuth client secret.')

    resources: str = Field(
        default="accounts,subscriptions",
        description="Comma-separated list of resources to extract: accounts, subscriptions, invoices"
    )
    ...
    group_name: Optional[str] = Field(default="zuora_ingestion", description="Asset group for organization")
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        ...
        description = self.description or "Ingest Zuora subscription billing data (accounts, subscriptions, invoices) using dlt's generic REST API source."
        ...
        def zuora_ingestion_asset(context: AssetExecutionContext):
            from dlt.sources.rest_api import rest_api_source

            context.log.info(f"Starting Zuora ingestion, destination={destination or 'duckdb (in-memory)'}")
            ...
            _map = {
                "accounts": {"name": "accounts", "endpoint": {"path": "v1/accounts", "data_selector": "accounts"}},
                "subscriptions": {"name": "subscriptions", "endpoint": {"path": "v1/subscriptions", "data_selector": "subscriptions"}},
                "invoices": {"name": "invoices", "endpoint": {"path": "v1/invoices", "data_selector": "invoices"}},
            }
            ...
            context.log.info("Creating Zuora REST API source...")
            source = rest_api_source(config)

            context.log.info("Extracting Zuora data...")
            load_info = pipeline.run(source)
            context.log.info(f"Zuora data loaded: {load_info}")
            ...
            if not all_data:
                context.log.warning("No data extracted from Zuora.")
        ...
        return Definitions(assets=[zuora_ingestion_asset])
