...
Ingest document data from MongoDB collections using dlt's MongoDB source.

## Overview

This component uses [dlt (data load tool)](https://dlthub.com) to extract data from MongoDB.
...
**Available Resources:**
Collections, Documents (any MongoDB schema)
...
See `schema.json` for complete list of authentication and configuration parameters specific to MongoDB.
...
Refer to `schema.json` and the [dlt documentation](https://dlthub.com/docs) for authentication requirements specific to MongoDB.
...
- `dlt[mongodb]` - Installs dlt with MongoDB source
