...
Ingest HR data from Personio (employees, attendance, time-off) using dlt's Personio source.

## Overview

This component uses [dlt (data load tool)](https://dlthub.com) to extract data from Personio.
...
**Available Resources:**
Employees, Attendance, Time Off, Absences, Projects
...
See `schema.json` for complete list of authentication and configuration parameters specific to Personio.
...
Refer to `schema.json` and the [dlt documentation](https://dlthub.com/docs) for authentication requirements specific to Personio.
...
- `dlt[personio]` - Installs dlt with Personio source
