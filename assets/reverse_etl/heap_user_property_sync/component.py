"""DataFrame -> Heap user property sync (write-only, no upsert semantics).

Calls Heap's add_user_properties endpoint once per row to attach/overwrite
custom properties on a user. Heap's API has no read-back or dedup
concept -- this simply calls the endpoint for every row every run; Heap
itself handles the property overwrite idempotently server-side.

Pairs with:
  - ``heap`` resource -- write-only Data API (required)"""
from typing import Any, Dict, List, Optional

import dagster as dg
from pydantic import Field
class HeapUserPropertySyncComponent(dg.Component, dg.Model, dg.Resolvable):
    """Push computed user properties from an upstream DataFrame into Heap.
    ...
    resource_key: str = Field(
        default="heap",
        description="Resource key registered by HeapResourceComponent.",
    )

    identity_column: str = Field(description="Upstream column holding the Heap identity (whatever value your heap.identify() calls use, typically an email or internal user ID).")

    properties_map: Dict[str, str] = Field(description="Upstream column -> Heap user property name.")

    max_rows: int = Field(default=10000, description="Overall safety cap on rows per run.")

    group_name: Optional[str] = Field(default="heap", description="Dagster asset group name.")
    description: Optional[str] = Field(default=None, description="Asset description.")
    owners: Optional[List[str]] = Field(default=None, description="Asset owners.")
    tags: Optional[Dict[str, str]] = Field(default=None, description="Catalog tags.")
    kinds: Optional[List[str]] = Field(default=None, description="Asset kinds (auto-includes 'heap').")
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        ...
        if bool(self.upstream_asset_key) == bool(self.source):
            raise ValueError(
                "HeapUserPropertySyncComponent: supply exactly one of "
        ...
        def _resolve_source_df(exec_ctx):
            ...
            raise ValueError(f"HeapUserPropertySyncComponent source kind={kind!r} not supported (sql / csv / inline)")
        ...
        def _run_write(context, upstream):
            ...
            required_cols = {_self.identity_column} | set(_self.properties_map.keys())
            ...
            for _, row in df.iterrows():
                identity = _row_value(row[_self.identity_column])
                if not identity:
                    skipped_no_key += 1
                    continue
                properties = {}
                for col, heap_field in _self.properties_map.items():
                    v = _row_value(row[col])
                    if v is not None:
                        properties[heap_field] = v
                if not properties:
                    continue
                try:
                    svc.add_user_properties(str(identity), properties)
                    success_count += 1
                except Exception as e:  # noqa: BLE001
                    errors.append(f"{identity}: {type(e).__name__}: {e}")

            context.log.info(
                f"Heap user property sync: {success_count} succeeded, {len(errors)} errors, "
                f"{skipped_no_key} skipped (missing {_self.identity_column})."
            )
            if errors:
                context.log.error("First few errors:\n" + "\n".join(errors[:5]))
            metadata = {
                "rows_synced": dg.MetadataValue.int(success_count),
        ...
        common_kwargs = dict(
            ...
            owners=_self.owners,
            tags=_self.tags,
            description=_self.description or ("Push computed user properties from a DataFrame into Heap (write-only, no dedup semantics)."),
        )
