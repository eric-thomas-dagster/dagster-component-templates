...
Ingest CRM data from Salesforce (accounts, contacts, opportunities, leads) using dlt's Salesforce source.

## Overview

This component uses [dlt (data load tool)](https://dlthub.com) to extract data from Salesforce.
...
**Available Resources:**
Accounts, Contacts, Opportunities, Leads, Activities, Tasks
...
See `schema.json` for complete list of authentication and configuration parameters specific to Salesforce.
...
Refer to `schema.json` and the [dlt documentation](https://dlthub.com/docs) for authentication requirements specific to Salesforce.
...
- `dlt[salesforce]` - Installs dlt with Salesforce source
