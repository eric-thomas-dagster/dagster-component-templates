...
> **Also see [`CollibraExportJobComponent`](../../../jobs/collibra_export_job/)** — an op-shaped, schedulable single-job alternative that does the whole extract-and-push in one tick without adding lineage assets to your graph. Usually the better default for "just sync my Dagster asset graph to Collibra nightly." Reach for this asset-chain shape when you want the lineage payload versioned as a first-class Dagster asset or want automation-condition-driven pushes wired into your graph.
...
|---|---|---|---|
| `asset_name` | `str` | `"lineage_to_collibra"` | Output sink asset name |
...
| `catalog_url` | `str` | `"https://example.collibra.com"` | Catalog endpoint base URL. |
| `api_token_env` | `str` | `"COLLIBRA_API_TOKEN"` | Env var holding the API token / OAuth bearer. |
