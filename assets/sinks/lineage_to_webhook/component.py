...
Sink that POSTs the raw upstream lineage_graph to any HTTP webhook (Slack, internal endpoint, n8n). Optional Bearer auth.
"""
import json
import os
...
class LineageToWebhookComponent(dg.Component, dg.Model, dg.Resolvable):
    """Lineage → HTTP Webhook — sink asset that depends on lineage_graph and pushes to webhook."""

    asset_name: str = Field(default="lineage_to_webhook", description="Output sink asset name")
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        ...
        def lineage_sink(context: dg.AssetExecutionContext, upstream: dict) -> dg.MaterializeResult:
            ...
            if only_push_on_change and last_hash and last_hash == current_hash:
                ...
                context.log.info(
                    f"Lineage unchanged (hash={current_hash[:8]}), skipping push to webhook. "
            ...
            return dg.MaterializeResult(metadata={
                ...
                "catalog": dg.MetadataValue.text("webhook"),
