"""DataFrame -> Pipedrive person upsert (search-then-write).

Pipedrive has no native upsert endpoint: this sink searches `/persons/search`
by `key_field` via the resource's `upsert_person`, then PUTs the match or
POSTs a new person.
...
class PipedrivePersonUpsertComponent(dg.Component, dg.Model, dg.Resolvable):
    """Upsert rows from an upstream DataFrame into Pipedrive persons.
    ...
    resource_key: str = Field(
        ...
        description="Resource key registered by PipedriveResourceComponent.",
    )

    key_field: str = Field(description="Pipedrive field key to search on (e.g. 'email', or a custom field hash key).")

    key_column: str = Field(description="Upstream column holding the value to match key_field on. MUST be present in fields_map values.")

    fields_map: Dict[str, str] = Field(description="Upstream column -> Pipedrive person field key.")

    max_rows: int = Field(default=10000, description="Overall safety cap on rows per run.")

    group_name: Optional[str] = Field(default="pipedrive", description="Dagster asset group name.")
    description: Optional[str] = Field(default=None, description="Asset description.")
    owners: Optional[List[str]] = Field(default=None, description="Asset owners.")
    tags: Optional[Dict[str, str]] = Field(default=None, description="Catalog tags.")
    kinds: Optional[List[str]] = Field(default=None, description="Asset kinds (auto-includes 'pipedrive').")
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        ...
        if bool(self.upstream_asset_key) == bool(self.source):
            raise ValueError(
                "PipedrivePersonUpsertComponent: supply exactly one of "
                "`upstream_asset_key` OR `source:` (got both or neither)."
            )

        if self.key_field not in self.fields_map.values():
            raise ValueError(
                f"PipedrivePersonUpsertComponent: key_field={self.key_field!r} not in "
        ...
        def _resolve_source_df(exec_ctx):
            ...
            raise ValueError(f"PipedrivePersonUpsertComponent source kind={kind!r} not supported (sql / csv / inline)")
        ...
        def _run_write(context, upstream):
            ...
            for _, row in df.iterrows():
                key_value = _row_value(row[_self.key_column])
                if key_value is None:
                    skipped_no_key += 1
                    continue
                fields = {}
                for col, pd_field in _self.fields_map.items():
                    v = _row_value(row[col])
                    if v is not None:
                        fields[pd_field] = v
                try:
                    svc.upsert_person(_self.key_field, str(key_value), fields)
                    success_count += 1
                except Exception as e:  # noqa: BLE001
                    errors.append(f"{key_value}: {type(e).__name__}: {e}")

            context.log.info(
                f"Pipedrive person upsert: {success_count} succeeded, {len(errors)} errors, "
                f"{skipped_no_key} skipped (missing {_self.key_column}) -- matched on {_self.key_field}."
            )
            if errors:
        ...
        common_kwargs = dict(
            ...
            owners=_self.owners,
            tags=_self.tags,
            description=_self.description or (f"Upsert DataFrame rows into Pipedrive persons (match on {_self.key_field})."),
        )
