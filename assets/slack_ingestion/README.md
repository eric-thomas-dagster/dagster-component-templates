...
Ingest communication data from Slack (messages, channels, users) using dlt's Slack source.

## Overview

This component uses [dlt (data load tool)](https://dlthub.com) to extract data from Slack.
...
**Available Resources:**
Messages, Channels, Users, Threads, Reactions
...
See `schema.json` for complete list of authentication and configuration parameters specific to Slack.
...
Refer to `schema.json` and the [dlt documentation](https://dlthub.com/docs) for authentication requirements specific to Slack.
...
- `dlt[slack]` - Installs dlt with Slack source
