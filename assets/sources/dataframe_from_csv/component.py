...
class DataframeFromCsvComponent(Component, Model, Resolvable):
    ...
    sinks: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description=(
            "Optional list of sinks that write the loaded DataFrame to a warehouse "
...
group_name=group_name,
        ...
        def _asset(context: AssetExecutionContext) -> pd.DataFrame:
            ...
            # This is a source -- there's no upstream DataFrame to compare
            # against, every column here originates in this CSV -- except a
            # synthetic one this component adds itself (filename_column_name),
            # which has no real "source" column to point lineage at.
            _effective_lineage = column_lineage
            if not _effective_lineage:
                _derived_cols = {filename_column_name} if add_filename_column else set()
                _effective_lineage = {
                    col.name: [col.name] for col in _col_schema.columns
                    if col.name not in _derived_cols
                }
            ...
            # DataFrame source even when a sink is also configured.
            if sinks:
                ...
                for sink in sinks:
                    ...
                    if kind != "table":
                        raise ValueError(
                            f"dataframe_from_csv sinks: only kind=table is supported "
                            f"(got {kind!r})"
                        )
                    ...
                    with _acquire() as conn:
                        ...
                        if (
                            ...
                            conn.register("_csv_sink_batch", sink_df)
                            try:
                                if sink_mode == "upsert_on_match":
                                    conn.execute(
                                        f"CREATE TABLE IF NOT EXISTS {qualified} AS "
                                        f"SELECT * FROM _csv_sink_batch WHERE 1=0"
                                    )
                                    ...
                                    try:
                                        conn.execute(
                                            ...
                                            f"(SELECT DISTINCT {match_tuple} FROM _csv_sink_batch)"
                                        )
                                        conn.execute(
                                            f"INSERT INTO {qualified} "
                                            f"SELECT * FROM _csv_sink_batch"
                                        )
                                        conn.execute("COMMIT")
                                    except Exception:
                                ...
                                try:
                                    conn.unregister("_csv_sink_batch")
