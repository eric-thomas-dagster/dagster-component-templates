...
|---|---|---|
| `name` | `str` | Short label for logs (also used as the asset's MCP server tag). |
...
| `headers_env` | `Dict[str, str]` | — | http / sse: map of header_name → env_var_name (value read from env). |
