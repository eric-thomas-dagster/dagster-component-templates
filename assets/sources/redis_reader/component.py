...
class RedisReaderComponent(Component, Model, Resolvable):
    ...
    sinks: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description=(
            "Optional list of sinks that write the read DataFrame to a warehouse "
        ...
        def redis_reader_asset(context: AssetExecutionContext) -> pd.DataFrame:
            ...
            if sinks:
                ...
                for sink in sinks:
                    ...
                    if kind != "table":
                        raise ValueError(
                            f"redis_reader sinks: only kind=table is supported "
                            f"(got {kind!r})"
                        )
                    ...
                    with _acquire() as conn:
                        ...
                        if (
                            ...
                            conn.register("_redis_sink_batch", sink_df)
                            try:
                                if sink_mode == "upsert_on_match":
                                    conn.execute(
                                        f"CREATE TABLE IF NOT EXISTS {qualified} AS "
                                        f"SELECT * FROM _redis_sink_batch WHERE 1=0"
                                    )
                                    ...
                                    try:
                                        conn.execute(
                                            ...
                                            f"(SELECT DISTINCT {match_tuple} FROM _redis_sink_batch)"
                                        )
                                        conn.execute(
                                            f"INSERT INTO {qualified} "
                                            f"SELECT * FROM _redis_sink_batch"
                                        )
                                        conn.execute("COMMIT")
                                    except Exception:
                                ...
                                try:
                                    conn.unregister("_redis_sink_batch")
