...
| `input_column` | `Union[str, int]` | Column containing the input bytes / text |
| `operation` | `str` | One of: to_base64, from_base64, to_hex, from_hex, to_text, to_bytes |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `output_column` | `Union[str, int]` | — | Output column name (defaults to overwriting input_column) |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `encoding` | `str` | `"utf-8"` | Text codec for to_text / to_bytes operations (default utf-8) |
| `error_handling` | `str` | `"coerce"` | How to handle conversion errors per row: 'coerce' (set None and log; default), 'raise' (fail asset). |
