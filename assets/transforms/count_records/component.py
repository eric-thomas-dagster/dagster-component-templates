...
class CountRecordsComponent(Component, Model, Resolvable):
    ...
    include_null_counts: bool = Field(
        default=False,
        description="Also compute and log null counts per column as asset metadata",
    )
    ...
    def get_description(cls) -> str:
        return "Count rows in a DataFrame, optionally grouped by one or more columns."
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        include_null_counts = self.include_null_counts
        ...
        _comp_name = "count_records"  # component directory name
        _kind_map = {
...
group_name=group_name,
            description=CountRecordsComponent.get_description(),
        )
        def _asset(context: AssetExecutionContext, upstream: pd.DataFrame) -> pd.DataFrame:
            ...
            if group_by:
                result = df.groupby(group_by).size().reset_index(name=count_column)
                context.log.info(f"Counted {len(result)} groups by {group_by}")
            else:
                result = pd.DataFrame({count_column: [len(df)]})
                context.log.info(f"Total row count: {len(df)}")
            ...
            if include_null_counts:
                ...
                context.add_output_metadata(
                    {
                        "null_counts": MetadataValue.json(
                            null_counts.to_dict(orient="records")
                        )
                    }
                )
