...
| `mode` | `str` | Renaming mode: first_row / add_prefix / add_suffix / replace / mapping / mapping_from_column |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `prefix` | `str` | — | For mode=add_prefix |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `columns` | `List[Union[str, int]]` | — | Restrict renaming to these columns. None = apply to all columns. |
| `suffix` | `str` | — | For mode=add_suffix |
| `pattern` | `str` | — | For mode=replace (regex) |
| `replacement` | `str` | — | For mode=replace (replacement string; backrefs supported) |
| `mapping` | `Dict[str, Union[str, int]]` | — | For mode=mapping: explicit {old: new} dict |
| `mapping_asset_key` | `str` | — | For mode=mapping_from_column: secondary asset providing the rename pairs |
| `mapping_key_column` | `Union[str, int]` | — | For mode=mapping_from_column: column in mapping_asset_key with the OLD names |
| `mapping_value_column` | `Union[str, int]` | — | For mode=mapping_from_column: column in mapping_asset_key with the NEW names |
