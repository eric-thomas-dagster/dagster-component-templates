...
class FieldMapperComponent(Component, Model, Resolvable):
    ...
    mapping: Dict[str, Union[str, int]] = Field(
        description="Mapping of old_column_name to new_column_name, e.g. {'_id': 'id', 'createdAt': 'created_at'}"
    )
    drop_unmapped: bool = Field(
        default=False,
        description="If true, only keep columns present in the mapping (plus passthrough columns).",
    )
    passthrough: Optional[List[str]] = Field(
        default=None,
        description="Columns to keep as-is even when drop_unmapped=true.",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        passthrough = self.passthrough or []
        ...
        _comp_name = "field_mapper"  # component directory name
        _kind_map = {
...
group_name=group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            if drop_unmapped:
                # Keep renamed columns + passthrough columns
                kept = set(mapping.values()) | set(passthrough)
                cols_to_keep = [c for c in df.columns if c in kept]
                df = df[cols_to_keep]
