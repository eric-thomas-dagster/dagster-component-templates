...
@dataclass
class HtmlParserComponent(Component, Model, Resolvable):
    """Strip HTML tags from text columns or extract specific elements using BeautifulSoup."""
    ...
    mode: str = Field(
        default="strip_tags",
        description="Processing mode: 'strip_tags', 'extract_text', 'extract_links', or 'extract_tables'.",
    )
    parser: str = Field(
        default="html.parser",
        description="BeautifulSoup parser: 'html.parser', 'lxml', or 'html5lib'.",
    )
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "html_parser"  # component directory name
        _kind_map = {
...
group_name=group_name,
        ...
        def _asset(context: AssetExecutionContext, upstream: pd.DataFrame) -> pd.DataFrame:
            ...
            valid_modes = ("strip_tags", "extract_text", "extract_links", "extract_tables")
            if mode not in valid_modes:
                raise ValueError(f"mode must be one of {valid_modes}, got: {mode}")

            df = upstream.copy()

            def process_value(html_val) -> object:
            ...
            for col in columns:
                if col not in df.columns:
                    ...
                    continue
                result_series = df[col].apply(process_value)
                if new_column_suffix:
                    df[f"{col}{new_column_suffix}"] = result_series
                else:
