...
@dataclass
class MakeColumnsComponent(Component, Model, Resolvable):
    """Reshape a single column of values into a wide multi-column format."""
    ...
    fill_value: Optional[str] = Field(
        default=None,
        description="Fill value for incomplete last row when values don't divide evenly",
    )
    ...
    def get_description(cls) -> str:
        return "Reshape a single column of values into a wide multi-column format."
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "make_columns"  # component directory name
        _kind_map = {
...
group_name=group_name,
            description=MakeColumnsComponent.get_description(),
        )
        def _asset(context: AssetExecutionContext, upstream: pd.DataFrame) -> pd.DataFrame:
            ...
            # Pad to a multiple of n_columns
            pad = (-len(values)) % n_columns
            values += [fill_value] * pad
            ...
            context.log.info(
                f"Reshaped {len(upstream)} values into {len(result)} rows x {n_columns} columns"
            )
