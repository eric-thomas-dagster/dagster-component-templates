...
class MultiFieldFormulaComponent(Component, Model, Resolvable):
    ...
    expression: str = Field(
        description=(
            "Formula expression using {col} as a placeholder for each column. "
            "e.g. 'log({col} + 1)' or '{col}.str.upper()'"
        )
    )
    columns: List[str] = Field(description="Columns to apply the expression to")
    output_prefix: Optional[str] = Field(
        default=None,
        description="If set, create new columns named '{prefix}{col}' instead of overwriting",
    )
    output_suffix: Optional[str] = Field(
        default=None,
        description="If set, create new columns named '{col}{suffix}'",
    )
    ...
    def get_description(cls) -> str:
        return "Apply a formula expression to multiple columns at once."
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "multi_field_formula"  # component directory name
        _kind_map = {
...
group_name=group_name,
            description=MultiFieldFormulaComponent.get_description(),
        )
        def _asset(context: AssetExecutionContext, upstream: pd.DataFrame) -> pd.DataFrame:
            ...
            for col in columns:
                if col not in df.columns:
                    ...
                    continue
                expr = expression.replace("{col}", f"upstream['{col}']")
                out_col = f"{output_prefix or ''}{col}{output_suffix or ''}"
                try:
                    df[out_col] = eval(
                        expr,
                        {"upstream": df, "df": df, "pd": pd, "np": np, col: df[col]},
                    )
