...
Compute period-over-period diff and percent-change on a value column,
optionally grouped (e.g. per customer / region) and ordered by a time
...
class PctChangeComponent(Component, Model, Resolvable):
    """Compute period-over-period diff and percent change.

    Right for week-over-week / month-over-month growth metrics. Optionally
    groups by one or more columns (e.g. per customer / region) and orders
    by a time column so the lag-1 comparison is on consecutive periods.

    Output columns (added to the input DataFrame):
      <value_column>_diff      — value vs previous period
      <value_column>_pct       — percent change vs previous period (0..1 by default;
                                  set as_percent=true to multiply by 100)
    """
    ...
    as_percent: bool = Field(
        default=False,
        description="If true, multiply pct by 100 (so 5.0 = 5%). Default: ratio form (0.05 = 5%).",
    )
    fill_first: bool = Field(
        default=False,
        description="Fill first-row NaN values (no prior period) with 0. Default: leave as NaN.",
    )
    ...
    def get_description(cls) -> str:
        return "Compute period-over-period diff and percent change on a value column, optionally grouped + ordered."
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "pct_change"  # component directory name
        _kind_map = {
...
group_name=group_name,
            description=PctChangeComponent.get_description(),
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> Any:
            ...
            if value_column not in df.columns:
                raise ValueError(
                    f"value_column='{value_column}' not present in upstream DataFrame "
                    f"(columns: {list(df.columns)})"
                )

            # Order rows first (within each group, if grouped) so the lag-1 diff
            # compares consecutive periods.
            if order_by:
                if order_by not in df.columns:
                    raise ValueError(
                        f"order_by='{order_by}' not present in upstream DataFrame"
                    )
                sort_cols = (group_by or []) + [order_by]
