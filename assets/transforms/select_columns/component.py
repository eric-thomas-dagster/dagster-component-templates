...
class SelectColumnsComponent(Component, Model, Resolvable):
    ...
    reorder: bool = Field(
        default=True,
        description="If columns is set, reorder to match that order",
    )
    ...
    def get_description(cls) -> str:
        return "Select, drop, and rename columns in a DataFrame."
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "select_columns"  # component directory name
        _kind_map = {
...
group_name=group_name,
            description=SelectColumnsComponent.get_description(),
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> pd.DataFrame:
            ...
            if drop_columns:
                ...
                context.log.info(f"Dropped columns: {drop_columns}")

            if columns and reorder:
                available = [c for c in columns if c in df.columns]
                skipped = [c for c in columns if c not in df.columns]
                if skipped:
                    context.log.warning(f"Columns not found, skipped: {skipped}")
                df = df[available]
