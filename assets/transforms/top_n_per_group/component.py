...
Keep the top N rows per group, sorted by a value column. Equivalent to
SQL's `ROW_NUMBER() OVER (PARTITION BY ... ORDER BY ... DESC) <= N`, or
...
class TopNPerGroupComponent(Component, Model, Resolvable):
    """Keep the top N rows per group, sorted by a value column.

    Equivalent to SQL `ROW_NUMBER() OVER (PARTITION BY ... ORDER BY ... DESC) <= N`.
    Pass `ascending: true` for bottom N. Optionally adds a rank column to the output.
    """
    ...
    sort_method: str = Field(
        default="first",
        description="Tie-breaker rank method: 'first' (input order), 'dense', 'min', 'max', 'average'.",
    )
    ...
    def get_description(cls) -> str:
        return "Keep the top N rows per group, sorted by a value column."
    ...
    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        ...
        _comp_name = "top_n_per_group"  # component directory name
        _kind_map = {
...
group_name=group_name,
            description=TopNPerGroupComponent.get_description(),
        ...
        def _asset(context: AssetExecutionContext, upstream: Any) -> Any:
            ...
            if missing_groups:
                raise ValueError(
                    f"group_by columns not in upstream: {missing_groups} "
