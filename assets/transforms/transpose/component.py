...
class TransposeComponent(Component, Model, Resolvable):
    ...
    def get_description(cls) -> str:
        return "Transpose a DataFrame — rows become columns."

    def build_defs(self, load_context: ComponentLoadContext) -> Definitions:
        asset_name = self.asset_name
        upstream_asset_key = self.upstream_asset_key
        index_column = self.index_column
        reset_column_name = self.reset_column_name
        ...
        _comp_name = "transpose"  # component directory name
        _kind_map = {
...
group_name=group_name,
            description=TransposeComponent.get_description(),
        )
        def _asset(context: AssetExecutionContext, upstream: pd.DataFrame) -> pd.DataFrame:
            ...
            original_shape = upstream.shape

            if index_column:
                result = (
                    upstream.set_index(index_column)
                    .T.reset_index()
                    .rename(columns={"index": reset_column_name})
                )
            else:
                result = (
                    upstream.T.reset_index().rename(columns={"index": reset_column_name})
                )

            context.log.info(
                f"Transposed DataFrame from {original_shape} to {result.shape}"
            )
