...
| `type_map` | `Dict[str, str]` | Mapping of column_name to target type. Valid values: 'int', 'float', 'str', 'bool', 'datetime', 'date', 'json'. |
...
| `errors` | `str` | `"coerce"` | What to do when conversion fails: 'raise', 'coerce' (NaN/NaT), or 'ignore'. |
| `datetime_format` | `str` | — | Optional strptime format string for datetime parsing, e.g. '%Y-%m-%d'. |
