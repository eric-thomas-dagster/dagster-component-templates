...
DEFAULT_QUERY = """query AuditLogEntries($cursor: String, $limit: Int, $startTime: Float, $endTime: Float) {
...
class DagsterPlusAuditLogIngestionComponent(dg.Component, dg.Model, dg.Resolvable):
    ...
    result_path: str = Field(default='auditLogs.entries', description="Dot-path to the list of records inside the GraphQL response")
    cursor_path: Optional[str] = Field(default='auditLogs.cursor', description="Dot-path to the next-page cursor (None if not paginated)")
    has_more_path: Optional[str] = Field(default='auditLogs.hasMore', description="Dot-path to the boolean has-more flag")
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        ...
        @dg.asset(
            ...
            description=self.description or "Pull Dagster+ audit-log entries (logins, deployment changes, permission edits) via the Dagster+ GraphQL API.",
