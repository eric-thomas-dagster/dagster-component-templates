...
DEFAULT_QUERY = """query RunsList($cursor: String, $limit: Int, $startTime: Float, $endTime: Float) {
...
class DagsterPlusRunEventsIngestionComponent(dg.Component, dg.Model, dg.Resolvable):
    """Pull Dagster+ run records (every job run, status, duration, tags) via the Dagster+ GraphQL API."""
    ...
    result_path: str = Field(default='runsOrError.results', description="Dot-path to the list of records inside the GraphQL response")
