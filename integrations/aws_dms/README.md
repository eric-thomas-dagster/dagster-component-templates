...
| Field | Type | Default | Description |
|---|---|---|---|
| `group_name` | `str` | `"aws_dms"` | Asset group name for all imported assets |
| `description` | `str` | — | Description for the AWS DMS component |
...
| `import_replication_tasks` | `bool` | `true` | Import replication tasks as materializable assets |
| `import_zero_etl_integrations` | `bool` | `false` | Import Zero ETL integrations as observable assets |
| `exclude_name_pattern` | `str` | — | Regex pattern to exclude entities by name |
| `replication_task_type` | `str` | `"start-replication"` | Task start type: start-replication, resume-processing, or reload-target |
| `generate_sensor` | `bool` | `true` | Generate observation sensor for replication task runs |
