...
|---|---|---|
| `workspace_url` | `str` | Databricks workspace URL (e.g., https://dbc-abc123.cloud.databricks.com) |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `group_name` | `str` | `"databricks"` | Group name for all imported assets |
| `description` | `str` | — | Description for the Databricks workspace component |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `filter_by_tags` | `str` | — | Comma-separated list of tags to filter entities |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `import_jobs` | `bool` | `true` | Import Databricks jobs as assets |
| `import_notebooks` | `bool` | `false` | Import standalone notebooks as assets |
| `import_dlt_pipelines` | `bool` | `false` | Import Delta Live Tables (DLT) pipelines as materializable assets that can be triggered from Dagster |
| `import_model_endpoints` | `bool` | `false` | Import ML model serving endpoints as observable assets |
| `exclude_name_pattern` | `str` | — | Regex pattern to exclude entities by name |
| `notebook_base_path` | `str` | — | Base path for importing notebooks (e.g., /Users/user@company.com/production) |
| `generate_sensor` | `bool` | `true` | Create a sensor to observe runs from Databricks |
| `assets_by_job_name` | `dict` | — | Override AssetSpec for specific Databricks jobs, keyed by job name. Each value may be a single override dict or a list of dicts (to produce multiple Dagster assets from one job). Supported keys per override: key, description, group_name, metadata, tags, kinds, deps. |
