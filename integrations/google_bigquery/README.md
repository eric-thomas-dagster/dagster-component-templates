...
|---|---|---|---|
| `poll_interval_seconds` | `int` | `60` | How often (in seconds) the sensor should check for completed queries |

### Catalog metadata

| Field | Type | Default | Description |
|---|---|---|---|
| `group_name` | `str` | `"bigquery"` | Group name for all imported assets |
| `description` | `str` | — | Description for the BigQuery component |
...
| `filter_by_labels` | `str` | — | Comma-separated list of label keys to filter entities |
...
| `dataset_id` | `str` | — | BigQuery dataset ID to filter entities (optional) |
| `location` | `str` | `"US"` | BigQuery location/region (e.g., US, EU, us-central1) |
...
| `import_materialized_views` | `bool` | `false` | Import materialized views as materializable assets |
| `import_transfer_jobs` | `bool` | `false` | Import BigQuery Data Transfer Service jobs as materializable assets |
| `import_tables` | `bool` | `false` | Import tables as observable assets (monitor metadata) |
