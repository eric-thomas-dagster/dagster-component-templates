...
| `asset_overrides` | `Dict[str, AssetOverride]` | — | Per-asset overrides keyed by the emitted asset's name (e.g. `datastream_mysql_to_bq`, `connection_profile_mysql_source`). Today supports `depends_on: [upstream_key, ...]` to add Dagster asset dependencies. Matches the pa… _(full docs in schema.json + component README)_ |
