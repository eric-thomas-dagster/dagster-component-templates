...
| `asset_overrides` | `Dict[str, AssetOverride]` | — | Per-asset overrides keyed by the emitted asset's name (e.g. `training_job_recommender_v2`, `pipeline_customer_scoring`). Today supports `depends_on: [upstream_key, ...]` to add Dagster asset dependencies. Matches the pat… _(full docs in schema.json + component README)_ |
