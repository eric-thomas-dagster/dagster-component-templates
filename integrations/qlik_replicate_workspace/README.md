...
|---|---|---|---|
| `wait_for_completion` | `bool` | `true` | If true, poll Qlik EM until the task reaches a terminal state. |
| `poll_interval_seconds` | `int` | `15` | Poll interval while waiting for terminal state. |
| `timeout_seconds` | `int` | `3600` | Give up waiting after this many seconds (asset fails). |
