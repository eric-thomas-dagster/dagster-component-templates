"""SnowflakeDynamicTableComponent — define a Snowflake DYNAMIC TABLE as Dagster YAML.
...
class SnowflakeDynamicTableComponent(Component, Model, Resolvable):
    """Define a Snowflake DYNAMIC TABLE as Dagster YAML. Materialization runs CREATE OR REPLACE DYNAMIC TABLE."""
    ...
    dt_name: str = Field(description="Dynamic table name (unquoted identifier)")
    target_lag: str = Field(description='"5 minutes" / "1 hour" / "DOWNSTREAM"')
    sql: str = Field(description="The dynamic table's SELECT statement.")
    initialize: str = Field(default="ON_SCHEDULE", description="'ON_CREATE' or 'ON_SCHEDULE'. Default ON_SCHEDULE avoids immediate-refresh privilege issues.")
    refresh_mode: str = Field(default="AUTO", description="'AUTO' / 'INCREMENTAL' / 'FULL'")
    ...
    def _build_ddl(self) -> str:
        body = f"CREATE OR REPLACE DYNAMIC TABLE {self.database}.{self.schema_name}.{self.dt_name}\n"
        body += f"  TARGET_LAG = '{self.target_lag}'\n"
        body += f"  WAREHOUSE = {self.warehouse}\n"
        body += f"  INITIALIZE = {self.initialize}\n"
        body += f"  REFRESH_MODE = {self.refresh_mode}\n"
        if self.cluster_by:
    ...
    def get_description(cls) -> str:
        return "Define a Snowflake DYNAMIC TABLE as Dagster YAML. Materialization runs CREATE OR REPLACE DYNAMIC TABLE."
