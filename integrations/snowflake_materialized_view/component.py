"""SnowflakeMaterializedViewComponent — define a Snowflake MATERIALIZED VIEW as Dagster YAML.

Define a Snowflake MATERIALIZED VIEW. Requires Enterprise+ edition. Materialization runs CREATE OR REPLACE MATERIALIZED VIEW.
...
class SnowflakeMaterializedViewComponent(Component, Model, Resolvable):
    """Define a Snowflake MATERIALIZED VIEW. Requires Enterprise+ edition. Materialization runs CREATE OR REPLACE MATERIALIZED VIEW."""
    ...
    mv_name: str = Field(description="Materialized view name (unquoted identifier)")
    sql: str = Field(description="The MV's SELECT statement. Standard MV restrictions apply (single source table, no joins, etc.).")
    ...
    def _build_ddl(self) -> str:
        body = f"CREATE OR REPLACE MATERIALIZED VIEW {self.database}.{self.schema_name}.{self.mv_name}\n"
        if self.cluster_by:
    ...
    def get_description(cls) -> str:
        return "Define a Snowflake MATERIALIZED VIEW. Requires Enterprise+ edition. Materialization runs CREATE OR REPLACE MATERIALIZED VIEW."
