            ...
            if self.import_stored_procedures:
                try:
                    ...
                    for proc in procedures:
                        ...
                        #
                        # Two execution-shape variants — mirrors _make_task_asset:
                        #   - No config_schema_v → @asset(context) with hardcoded
                        #     args_v baked into CALL proc(args...)
                        #     (current behavior; backwards-compatible).
                        #   - config_schema_v set → @asset(context, config:
                        #     <generated dg.Config subclass>) so Dagster renders
                        #     the launchpad form. Field values are passed to
                        #     CALL proc(...) positionally in YAML insertion
                        #     order (Python dicts preserve it). args_v ignored.
                        def _run_proc_body(context, self_v, proc_name_v, db_v, schema_v, args_list):
                            """Shared body: CALL the proc with args_list (positional)."""
                            conn = self_v._create_connection()
                            cursor = conn.cursor()
                            try:
                                args_sql = ", ".join(
                        ...
                        def _make_proc_asset(
                            proc_name_v, db_v, schema_v, proc_kwargs_v, self_v,
                            args_v, config_schema_v=None,
                        ):
                            if config_schema_v:
                                # Launchpad-overridable path. Reuses the same
                                # _build_task_config_class builder — it's
                                # generic over the entity name; "Task" in the
                                # class name is fine for procs too (cosmetic).
                                ConfigClass = _build_task_config_class(proc_name_v, config_schema_v)
                                arg_order = list(config_schema_v.keys())

                                @asset(**proc_kwargs_v)
                                def _procedure_asset(context: AssetExecutionContext, config: ConfigClass):  # type: ignore[valid-type]
                                    """CALL stored procedure (config from launchpad)."""
                                    cfg = config.model_dump()
                                    runtime_args = [cfg[k] for k in arg_order]
                                    return _run_proc_body(
                                        context, self_v, proc_name_v, db_v, schema_v,
                                        runtime_args,
                                    )
                            ...
                            def _procedure_asset(context: AssetExecutionContext):
                                """CALL stored procedure with hardcoded args."""
                                return _run_proc_body(
                                    context, self_v, proc_name_v, db_v, schema_v,
                                    args_v or [],
                                )
                            ...
                            for inst in override["instances"]:
                                ...
                                if inst_args and inst_config_schema:
                                    raise ValueError(
                                        ...
                                        f"`config_schema:` on the same asset — pick one. "
                                        f"`config_schema:` exposes a launchpad form so the "
                                        f"defaults appear pre-filled (and customers can "
                                        f"override at materialize time); `args:` bakes the "
                                        f"values in."
                                    )
                                inst_kwargs: dict = dict(
                                    name=re.sub(r'[^a-zA-Z0-9_]', '_', inst_name.lower()),
                        ...
                        if proc_args and proc_config_schema:
                            raise ValueError(
                                ...
                                f"`args:` with overridable `config_schema:` on "
                                f"the same asset — pick one. `config_schema:` "
                                f"exposes a launchpad form (defaults pre-filled, "
                                f"customers override at materialize time)."
                            )
