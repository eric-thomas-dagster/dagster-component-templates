...
| `dagster_plus_url` | `str` | Dagster+ deployment URL (e.g. https://acme.dagster.cloud/prod). GraphQL endpoint is appended. |
| `warehouse` | `str` | Target warehouse type: snowflake \| bigquery \| postgres |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `table` | `str` | `"dagster_audit_log"` | — |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `schedule` | `str` | `"0 * * * *"` | Cron schedule; default hourly. |
| `dagster_plus_token_env` | `str` | `"DAGSTER_CLOUD_API_TOKEN"` | Env var with Dagster+ user token. |
| `snowflake_account` | `str` | — | — |
| `snowflake_user` | `str` | — | — |
| `snowflake_password_env` | `str` | `"SNOWFLAKE_PASSWORD"` | — |
