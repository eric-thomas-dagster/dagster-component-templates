...
| `webhook_url` | `str` | `"https://example.com/lineage-webhook"` | Full webhook URL to POST the lineage payload to. |
| `api_token_env` | `str` | — | Optional env var with a Bearer token. Omit for unauthenticated webhooks. |
| `only_export_on_change` | `bool` | `true` | — |
| `fail_on_webhook_error` | `bool` | `true` | — |
