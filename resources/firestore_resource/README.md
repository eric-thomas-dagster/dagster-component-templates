# Google Firestore Resource

Register a Google Cloud Firestore resource for serverless document database access

A Dagster resource component that provides a Google Cloud Firestore `ConfigurableResource` backed by `google-cloud-firestore`.

## Installation

```
pip install google-cloud-firestore
```

<!-- FIELDS:START - auto-generated by tools/regen_readme_fields.py -->

## Fields

### Required

| Field | Type | Description |
|---|---|---|
| `project` | `str` | GCP project ID |

### Connection

| Field | Type | Default | Description |
|---|---|---|---|
| `resource_key` | `str` | `"firestore_resource"` | Key used to register this resource. Other components reference it via resource_key. |
| `database` | `str` | `"(default)"` | Firestore database ID. '(default)' targets the default database. |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `gcp_credentials_env_var` | `str` | — | Environment variable holding service account JSON. Leave empty to use Application Default Credentials. |

<!-- FIELDS:END -->

## Configuration

```yaml
type: firestore_resource.component.FirestoreResourceComponent
attributes:
  resource_key: firestore_resource
  project: my-gcp-project
  gcp_credentials_env_var: GCP_SERVICE_ACCOUNT_JSON
  database: "(default)"
```

## Auth

Set the environment variable named in `gcp_credentials_env_var` to the full JSON content of a GCP service account key file. If omitted, Application Default Credentials (ADC) are used — suitable for Workload Identity or local `gcloud auth application-default login` setups.