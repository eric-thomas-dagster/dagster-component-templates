...
|---|---|---|
| `base_url_env_var` | `str` | Env var with the AIS server base URL (no /jderest path). |
| `username_env_var` | `str` | Env var with JDE username. |
| `password_env_var` | `str` | Env var with JDE password. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `api_path_prefix` | `str` | `"/jderest/v3/orchestrator"` | Orchestrator API path. v3 for JDE Tools 9.2.7+, v2 for older. |
