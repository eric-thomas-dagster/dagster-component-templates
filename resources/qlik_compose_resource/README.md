...
|---|---|---|
| `base_url_env_var` | `str` | Env var with the Compose base URL (no /api path). |
...
| `username_env_var` | `str` | — | Env var with username (session auth). |
| `password_env_var` | `str` | — | Env var with password (session auth). |
| `api_token_env_var` | `str` | — | Env var with API token (preferred for prod). |
