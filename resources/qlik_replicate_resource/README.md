...
|---|---|---|
| `base_url_env_var` | `str` | Env var with the Enterprise Manager base URL, e.g. 'https://qlikem.acme.com'. Do NOT include the '/attunityenterprisemanager/api/v1' path — the resource appends it. |

### Connection

| Field | Type | Default | Description |
|---|---|---|---|
| `resource_key` | `str` | `"qlik_replicate_resource"` | Resource key. Other components reference it via this name. |
| `username_env_var` | `str` | — | Env var with Enterprise Manager username (session-based auth). Required unless api_token_env_var is set. |
| `password_env_var` | `str` | — | Env var with Enterprise Manager password (session-based auth). Required unless api_token_env_var is set. |
| `api_token_env_var` | `str` | — | Env var with a Qlik Enterprise Manager API token. Preferred for production / CI. When set, username/password fields are ignored. |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `verify_ssl` | `bool` | `true` | Enable TLS certificate verification. Set false only for self-signed dev servers. |
