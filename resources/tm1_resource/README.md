...
|---|---|---|
| `base_url_env_var` | `str` | Env var with the TM1 server base URL, e.g. 'https://tm1.acme.com:5495'. Include the port. Do NOT include '/api/v1' — the resource appends it. |
| `username_env_var` | `str` | Env var with TM1 username. |
| `password_env_var` | `str` | Env var with TM1 password. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `resource_key` | `str` | `"tm1_resource"` | Resource key. Other components reference it via this name. |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `cam_namespace_env_var` | `str` | — | Env var with CAM namespace (Cognos SSO). Omit for native TM1 auth. |
