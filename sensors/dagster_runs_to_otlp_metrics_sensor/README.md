...
| `otlp_endpoint_env_var` | `str` | Env var with OTLP/HTTP endpoint base URL. The /v1/metrics path is appended. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `otlp_headers_env_var` | `str` | — | Env var with comma-separated HTTP headers (k=v,k=v). |
| `service_name` | `str` | `"dagster"` | OTLP resource attribute `service.name`. |
| `location_label` | `str` | `"default"` | OTLP resource attribute `service.instance.id`. |
| `monitor_jobs` | `List[str]` | — | If set, only emit for these job names. None = all. |
