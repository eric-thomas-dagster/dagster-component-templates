...
| Field | Type | Default | Description |
|---|---|---|---|
| `statsd_host` | `str` | `"127.0.0.1"` | StatsD server host. For Datadog Agent on the same host, 127.0.0.1. |
| `statsd_port` | `int` | `8125` | StatsD UDP port (default 8125 for both statsd and Datadog Agent). |
| `metric_prefix` | `str` | `"dagster"` | Prefix applied to every metric name (e.g. 'dagster.runs.total'). |
| `dogstatsd_tags` | `bool` | `true` | When true, append `\|#k:v,k:v` tag block (Datadog Agent, modern statsd, etcd). When false, omit tags entirely (plain statsd protocol). |
| `extra_tags` | `dict` | — | Static tags applied to every metric (e.g. {env: prod, team: data}). Merged with per-event tags. |
