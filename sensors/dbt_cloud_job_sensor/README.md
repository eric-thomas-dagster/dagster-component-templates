...
| `job_id` | `int` | dbt Cloud job ID to monitor |
| `api_token_env_var` | `str` | Env var containing the dbt Cloud API token |
| `job_name` | `str` | Dagster job to trigger when dbt run completes |
