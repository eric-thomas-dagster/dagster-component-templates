...
| `bootstrap_servers` | `str` | Comma-separated list of Kafka broker addresses (e.g., 'broker1:9092,broker2:9092') |
| `topic` | `str` | Name of the Kafka topic to monitor |
| `group_id` | `str` | Kafka consumer group ID for this sensor |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `max_messages_per_poll` | `int` | `100` | Maximum number of messages to consume per sensor evaluation |
| `poll_timeout_seconds` | `float` | `5.0` | Seconds to wait for messages during each poll call |
| `security_protocol` | `str` | `"PLAINTEXT"` | Kafka security protocol (PLAINTEXT, SSL, SASL_PLAINTEXT, SASL_SSL) |
