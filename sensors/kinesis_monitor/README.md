...
| `stream_name` | `str` | Name of the Kinesis Data Stream to monitor |
| `job_name` | `str` | Name of the job to trigger when records are detected |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `max_records_per_shard` | `int` | `100` | Maximum number of records to read per shard per evaluation |
| `initial_position` | `str` | `"LATEST"` | Where to start reading on first run: LATEST (new records only) or TRIM_HORIZON (all records) |
