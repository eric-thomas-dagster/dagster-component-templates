...
Sensor that writes the Dagster lineage graph as JSON to a local or mounted path. For demos, debugging, audit trails.
...
class LineageToFileComponent(dg.Component, dg.Model, dg.Resolvable):
    """Lineage → JSON File — sensor that pushes the Dagster asset lineage graph to file.
    ...
    sensor_name: str = Field(default="lineage_to_file_sync")
    ...
    demo_export_path: Optional[str] = Field(
        default="data/exports/lineage_to_file.json",
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        ...
        @dg.sensor(
            ...
            description=f"Sync the Dagster asset lineage graph to file. Only pushes when the graph changes.",
        )
        def lineage_sensor(context: dg.SensorEvaluationContext):
            ...
            if demo_mode and export_path:
                ...
                context.log.info(f"Pushed lineage to file")
