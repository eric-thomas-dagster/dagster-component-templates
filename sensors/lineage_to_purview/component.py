...
Sensor that exports the Dagster asset lineage graph to Microsoft Purview Data Map (Apache Atlas v2 entity bulk API).
...
def _transform(payload):
    ...
    for i, edge in enumerate(payload["edges"]):
        up_qn = f"{qn_prefix}/{edge['upstream']}"
        dn_qn = f"{qn_prefix}/{edge['downstream']}"
        edge_id = f"{edge['upstream']}->{edge['downstream']}"
        entities.append({
            "typeName": "Process",
            "attributes": {
                "qualifiedName": f"{qn_prefix}/process/{edge['upstream']}__to__{edge['downstream']}",
...
class LineageToPurviewComponent(dg.Component, dg.Model, dg.Resolvable):
    """Lineage → Microsoft Purview — sensor that pushes the Dagster asset lineage graph to purview.
    ...
    sensor_name: str = Field(default="lineage_to_purview_sync")
    ...
    demo_export_path: Optional[str] = Field(
        default="data/exports/lineage_to_purview.json",
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        ...
        @dg.sensor(
            ...
            description=f"Sync the Dagster asset lineage graph to purview. Only pushes when the graph changes.",
        )
        def lineage_sensor(context: dg.SensorEvaluationContext):
            ...
            if demo_mode and export_path:
                ...
                context.log.info(f"Pushed lineage to purview")
