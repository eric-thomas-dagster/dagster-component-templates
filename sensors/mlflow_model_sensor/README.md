...
| `tracking_uri_env_var` | `str` | Name of the environment variable that holds the MLflow Tracking URI (e.g., MLFLOW_TRACKING_URI). The variable must be set at runtime. |
| `model_name` | `str` | Registered model name to watch in the MLflow Model Registry. |
| `target_job` | `str` | Name of the Dagster job to trigger when a new version is detected. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `target_stage` | `str` | `"Production"` | Model Registry stage transition that triggers a run. Common values: 'Production', 'Staging', 'Archived'. |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `run_config` | `dict` | — | Optional extra run config merged into the triggered job's run_config. Keys are merged under the top-level 'ops' key alongside the model metadata injected by the sensor. |
