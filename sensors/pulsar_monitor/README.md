...
| `service_url` | `str` | Pulsar service URL (e.g., 'pulsar://localhost:6650' or 'pulsar+ssl://host:6651') |
| `topic` | `str` | Pulsar topic name (e.g., 'persistent://public/default/my-topic') |
| `subscription_name` | `str` | Durable subscription name. Pulsar tracks the position per subscription. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `subscription_type` | `str` | `"Shared"` | Subscription type: Exclusive, Shared, Failover, or Key_Shared |
| `max_messages_per_poll` | `int` | `100` | Maximum number of messages to consume per evaluation |
| `receive_timeout_ms` | `int` | `5000` | Milliseconds to wait for each message before stopping the poll |
| `jwt_token_env_var` | `str` | — | Name of the environment variable containing a JWT auth token |
