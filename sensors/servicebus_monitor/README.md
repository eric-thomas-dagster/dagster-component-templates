...
|---|---|---|---|
| `connection_string_env_var` | `str` | — | Name of the environment variable containing the Service Bus connection string. If not provided, DefaultAzureCredential is used with fully_qualified_namespace. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `queue_name` | `str` | — | Name of the Service Bus queue to monitor. Provide this OR topic_name. |
| `topic_name` | `str` | — | Name of the Service Bus topic to monitor. Requires subscription_name. |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `fully_qualified_namespace` | `str` | — | Service Bus namespace hostname (e.g., 'mynamespace.servicebus.windows.net'). Required when not using a connection string. |
| `subscription_name` | `str` | — | Name of the topic subscription (required when using topic_name) |
| `max_messages_per_poll` | `int` | `100` | Maximum number of messages to receive per evaluation |
