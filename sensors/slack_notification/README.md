...
| `slack_token` | `str` | Slack Bot token (use ${SLACK_TOKEN} for env var) |
| `channel_id` | `str` | Slack channel ID to monitor |
...
|---|---|---|---|
| `minimum_interval_seconds` | `int` | `60` | How often to check for new messages |
| `default_status` | `str` | `"RUNNING"` | Default sensor status (RUNNING or STOPPED) |

### Other

| Field | Type | Default | Description |
|---|---|---|---|
| `keyword_filter` | `str` | — | Only trigger on messages containing this keyword |
| `user_filter` | `str` | — | Only trigger on messages from this user ID |
