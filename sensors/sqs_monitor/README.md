...
| `queue_url` | `str` | Full URL of the SQS queue to poll (e.g., 'https://sqs.us-east-1.amazonaws.com/123456789/my-queue') |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `max_messages_per_poll` | `int` | `10` | Maximum number of messages to receive per evaluation (1–10, SQS limit) |
| `visibility_timeout_seconds` | `int` | `30` | Seconds a message stays hidden from other consumers while being processed |
| `region_name` | `str` | — | AWS region name. Uses default boto3 region if not set. |
