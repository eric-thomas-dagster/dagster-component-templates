...
| `job_name` | `str` | Job to watch + restart. For asset-based deployments this is typically the default `__ASSET_JOB`. Set explicitly if you're using a named job. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `asset_selection` | `list` | — | Optional list of asset keys to include in the launched RunRequest. Narrow the run to just the streaming asset (recommended). Omit to materialize whatever the job's default selection is. |
| `minimum_interval_seconds` | `int` | `60` | How often to check run health. |
| `default_status` | `str` | `"running"` | Sensor default status ('running' \| 'stopped'). |
