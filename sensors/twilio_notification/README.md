...
| `account_sid_env_var` | `str` | Name of the environment variable that holds the Twilio Account SID (e.g., TWILIO_ACCOUNT_SID). |
| `auth_token_env_var` | `str` | Name of the environment variable that holds the Twilio Auth Token (e.g., TWILIO_AUTH_TOKEN). |
| `from_number` | `str` | The Twilio phone number to send from (e.g., '+15551234567'). For WhatsApp use the plain E.164 number — the 'whatsapp:' prefix is added automatically when channel='whatsapp'. |
| `to_numbers` | `List[str]` | List of recipient phone numbers in E.164 format (e.g., '+15559876543'). |

### Sensor configuration

| Field | Type | Default | Description |
|---|---|---|---|
| `monitored_assets` | `List[str]` | — | List of asset key strings to watch (e.g., ['my_asset', 'namespace/other']). When None (default) all asset events are monitored. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `channel` | `str` | `"sms"` | Messaging channel to use. 'sms' sends a standard SMS; 'whatsapp' prefixes both from/to numbers with 'whatsapp:' automatically. |
| `message_template` | `str` | `"Dagster alert: {asset_key} {event_type} at {timestamp}"` | Template string for the notification message. Available format variables: {asset_key}, {event_type}, {timestamp}, {run_id}. |
| `on_failure` | `bool` | `true` | Send a notification when an asset materialization fails. |
| `on_success` | `bool` | `false` | Send a notification when an asset materialization succeeds. |
